...
battle (!x1, !y1) (!x2, !y2) =
  let i1 = (\(!a, !b) -> a + min 1 b) $ divMod x1 y2
      i2 = (\(!a, !b) -> a + min 1 b) $ divMod x2 y1
      !_ = traceShow ((x1, y1), (x2, y2), (i1, i2)) ()
   in i1 > i2
...
main = do
  ...
  !sorted <- VU.modify (VAI.sortBy (comparing (Down . uncurry (*) . snd))) . VU.zip (vRange 1 n) <$> VU.replicateM n (tuple2 <$> getLineIntList)
  let !iResult = fst $ VU.head sorted

  let !sortedXs = VU.map snd sorted
  let (!x, !xs) = (VU.head sortedXs, VU.tail sortedXs)

  -- the strongest monster has to win all the others
  let !result = VU.all (battle x) xs
  print $ if result then iResult else -1
