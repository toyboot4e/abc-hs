...
main = do
  [n] <- getLineIntList
  ((!x1, !x2), !xs) <- bimap VU.head VU.tail . dupe <$> VU.replicateM n (tuple2 <$> getLineIntList)

  let !res1 = VU.foldl' (\acc (!x, !y) -> max (gcd acc x) (gcd acc y)) x1 xs
  let !res2 = VU.foldl' (\acc (!x, !y) -> max (gcd acc x) (gcd acc y)) x2 xs
