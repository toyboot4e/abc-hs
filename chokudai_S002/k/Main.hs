...
main = do
  ...
  !input <- replicateM nInput getTuple2

  -- REMARK: duplicate item should not be chosen by others
  let !imInput = concatMap (\(!a, !b) -> if a == b then [(a, 10000000)] else [(a, 1), (b, 1)]) input
  let !im = IM.fromListWith (+) imInput :: IM.IntMap Int

  -- TODO: proof
  let !result = foldl' greedy s0 input
      s0 = IS.empty
      greedy :: IS.IntSet -> (Int, Int) -> IS.IntSet
      greedy !is (!x, !y) = IS.insert chosen is
        where
          !b1 = IS.member x is
          !b2 = IS.member y is
          !i1 = fromJust $ IM.lookup x im
          !i2 = fromJust $ IM.lookup y im
          chosen
            | b1 = y
            | b2 = x
            | i1 < i2 = x
            | otherwise = y
  -- !_ = traceShow ((x, y), (i1, i2), im) ()

  -- let !_ = traceShow (result) ()
  print $ IS.size result
