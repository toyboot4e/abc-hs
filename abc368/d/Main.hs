...
solve = do
  ...
  vs <- U.map pred <$> intsU'
  let bvs = U.accumulate (||) (U.replicate n False) $ U.map (,True) vs

  when (G.length vs == 1) $ do
    printBSB '1'
    liftIO exitSuccess

  let !gr = buildSG n $ swapDupeU es

  degs <- UM.replicate n (0 :: Int)
  forM_ [0 .. n - 1] $ \u -> do
    U.forM_ (gr `adj` u) $ \v -> do
      UM.modify degs succ v

  exists <- UM.replicate n True
  heap <- newMinBinaryHeap (3 * n)

  degs' <- U.unsafeFreeze degs
  U.iforM_ degs' $ \v deg -> do
    insertBH heap (deg, v)

  let onDelete v = do
        let !_ = dbg ("delete", v)
        GM.write exists v False
        U.forM_ (gr `adj` v) $ \v' -> do
          b <- GM.read exists v'
          when b $ do
            GM.modify degs pred v'
            vDeg <- GM.read degs v'
            insertBH heap (vDeg, v')

  fix $ \loop -> do
    whenJustM (deleteBH heap) $ \(!deg, !v) -> do
      deg' <- GM.read degs v
      b <- GM.read exists v
      when (deg' == deg && deg <= 1 && b) $ do
        when (deg <= 1) $ do
          unless (bvs G.! v) $ do
            onDelete v
          loop

  exists' <- U.unsafeFreeze exists
  printBSB $ G.length $ G.filter id exists'
