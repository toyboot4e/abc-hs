...
solve = do
  (!n, !m, !x) <- ints3'
  !input <- U.replicateM m ((,,,) <$> int1' <*> int1' <*> int' <*> int')

  res <- UM.replicate m (0 :: Int)
  GM.write res 0 x

  let !gr = buildWSG n $ U.imap (\i (!u, !v, !s, !e) -> (u, v, (i, s, e))) input

  let (!_, !v0, !s0_, !e0_) = input G.! 0

  heap <- newMinBinaryHeap (2 * m)
  insertBH heap (x + e0_, 0, v0, e0_, x)
  done <- UM.replicate m False

  fix $ \loop -> do
    whenJustM (deleteBH heap) $ \(!_, !iEdge, !u, !e0, !delay) -> do
      unlessM (GM.exchange done iEdge True) $ do
        let !_ = dbg (iEdge, u)
        U.forM_ (gr `adjW` u) $ \(!v, (i, !s, !e)) -> do
          when (e0 <= s) $ do
            let !e' = e0 + delay
            let !delay' = e' - s
            f <- GM.read res i
            let !_ = dbg ("delay", f, delay')
            when (f < delay') $ do
              GM.write res i delay'
              GM.write done i False
              insertBH heap (e + delay', i, v, e, delay')
      loop
