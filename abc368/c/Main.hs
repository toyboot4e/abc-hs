...
input' :: [Int]
input' = scanl' (+) (0 :: Int) input

calc :: Int -> Int -> Int
calc n x
  | x `mod` 5 == 0 = n + x3
  | otherwise =
      -- let (!l, !r) = splitAt (offset + 1) input'
      --     !r0 = last l
      --     candidates = takeWhile (< rest + r0) r
      --  in n + (length candidates + 1) + x3
      let candidates = note (n, x3, rest) . takeWhile (< rest) $ scanl' (+) (input !! offset) $ drop (offset+1) input
       in n + (length candidates + 1) + x3
  where
    !offset = n `mod` 3
    !rest = x `mod` 5
...
solve = do
  n <- int'
  xs <- intsU'
  printBSB $ U.foldl' (calc . note "acc") (0 :: Int) xs
