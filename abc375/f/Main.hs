...
solve = do
  ...
  let excludes :: S.Set (Int, Int)
      excludes =
        S.fromList . U.toList $
          U.mapMaybe
            ( \case
                (1, !iEdge, !_) ->
                  let (!u, !v, !_) = es G.! iEdge
                   in Just (min u v, max u v)
                _ -> Nothing
            )
            qs

  -- TODO: need not to dupe the edges??
  let wEdges = swapDupeW es
  ...
  U.forM_ wEdges $ \(!v1, !v2, !w) -> do
    -- when (S.member (min v1 v2, max v1 v2) excludes) $ do
    --   let !_ = dbg ("exc", v1, v2)
    --   return ()

    when (S.notMember (min v1 v2, max v1 v2) excludes) $ do
  ...
  res <- (`U.mapMaybeM` U.reverse qs) $ \case
    (1, !iEdge, !_) -> do
      let (!a, !b, !c) = es G.! iEdge
      -- TODO: update the graph

      -- P11
      -- https://img.atcoder.jp/arc035/editorial.pdf

      -- TODO: only consider better edges?

      forM_ [0 .. nVerts - 1] $ \i -> do
        forM_ [0 .. nVerts - 1] $ \j -> do
          ij <- GM.read vec $ index bnd (i, j)
          ia <- GM.read vec $ index bnd (i, a)
          bj <- GM.read vec $ index bnd (b, j)
          ib <- GM.read vec $ index bnd (i, b)
          aj <- GM.read vec $ index bnd (a, j)

          let c1
               | ia == undef || bj == undef = undef
               | otherwise = ia + bj + c
          let c2
               | ib == undef || aj == undef = undef
               | otherwise = ib + aj + c
          let choose a b
                | a == undef = b
                | b == undef = a
          ...
          let !cost' = ij `choose` c1 `choose` c2
          GM.write vec (index bnd (i, j)) cost'

      return Nothing
    (2, !u, !v) -> do
      res <- GM.read vec $ index bnd (u, v)
      return $ Just res

  putBSB $ unlinesBSB $ U.reverse res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc375/tasks/abc375_f
main :: IO ()
main = runIO solve
