...
solve = do
  ...
  let !dict = U.uniq $ U.modify (VAI.sortBy compare) xs
  let !is = U.map (bindex dict) xs
  -- let !is2 = is U.++ is

  let len = U.length dict
  let freq = U.accumulate (+) (U.replicate len (0 :: Int)) $ U.map (,1 :: Int) is
  let csum = csum1D $ freq U.++ freq
  let eval l =
        let !r' = fromMaybe l . bisectR l (l + len - 1) $ \r -> csum +! (l, r) < c
            -- !_ = dbg (is2 G.! (l + 1), is2 G.! l)
            !w =
              let !prevX = fromMaybe (G.last dict - m) $ dict G.!? (l - 1)
                  !x = dict G.! l
               in x - prevX 
         in w * (csum +! (l, r'))

  printBSB $ U.sum $ U.generate len eval

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc429/tasks/abc429_d
main :: IO ()
main = runIO solve
