...
solve = do
  !n <- int'
  !xs <- intsU'
  let !freq = U.accumulate (+) (U.replicate (n + 1) (0 :: Int)) $ U.map (, 1) xs
  let eval f
        | f < 2 = 0
        | otherwise = f * (f - 1) `div` 2 * (n - f)

  printBSB . U.sum $ U.map eval freq

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc429/tasks/abc429_c
main :: IO ()
main = runIO solve
