...
solve :: Int -> String -> VU.Vector Char
solve w s = VU.create $ do
  !vec <- VU.unsafeThaw $ VU.fromList s

  -- greedy replace
  forM_ [0 .. (w - 2)] $ \i -> do
    b1 <- (== 'T') <$> VUM.read vec (i + 0)
    b2 <- (== 'T') <$> VUM.read vec (i + 1)
    when (b1 && b2) $ do
      VUM.write vec (i + 0) 'P'
      VUM.write vec (i + 1) 'C'
...
main = do
  [h, w] <- getLineIntList
  !inputs <- replicateM h getLine
  let !results = map (solve w) inputs

  forM_ results $ \s -> do
    putStrLn $ VU.toList s
