...
main = do
  ...
  -- let !nSets = shift (1 :: Int) n
  let result = until p f s0
      s0 = (-1 :: Int, 0 :: Int, IS.singleton 0)
      p (!nAcc, !_, !_) = nAcc == k
      f :: (Int, Int, IS.IntSet) -> (Int, Int, IS.IntSet)
      f (!nAcc, !_, !is) =
        let (!x0, !is') = IS.deleteFindMin is
            stepIt !accIs !x = IS.insert (x0 + x) accIs
            !next = VU.foldl' stepIt is' input
         in (succ nAcc, x0, next)
