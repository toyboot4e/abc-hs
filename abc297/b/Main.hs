...
main = do
  ...
  let !ibs = map fst $ filter ((== 'B') . snd) $ zip [0..] s
  let !cond1 = uncurry (/=) $ tuple2 $ map even ibs

  let (!ir1, !ir2) = tuple2 . map fst $ filter ((== 'R') . snd) $ zip [0..] s
  let !ik = head . map fst $ filter ((== 'K') . snd) $ zip [0..] s
  let !cond2 = ir1 < ik && ik < ir2

  putStrLn $ if cond1 && cond2 then "Yes" else "No"
