...
stringToIndex s = foldl' (\acc x -> 100 * acc + x) 0 (map charToIndex s)

-- type G = Array Int Int
type G = IM.IntMap Int

-- True on cycle
solve :: G -> (Bool, IS.IntSet) -> Int -> (Bool, IS.IntSet)
solve graph (b0, visited0) start
  -- end
  | b0 = (True, visited0)
  -- | otherwise = visitNode IS.empty start -- remove visited0
  -- ignore
  | IS.member start visited0 = (False, visited0)
  | otherwise = visitNode visited0 start
  where
    visitNode visited x
      -- REMARK: already checked, ignore
      | IS.member x visited0 = (False, visited)
      -- cycle
      | IS.member x visited = (True, visited)
      -- not a cycle
      | IM.notMember x graph = (False, IS.insert x visited)
      -- check next
      | otherwise = visitNode (IS.insert x visited) (graph IM.! x)

main :: IO ()
main = do
  [n] <- getLineIntList
  input <- replicateM n ((\[a, b] -> (a, b)) . map stringToIndex . words <$> getLine)
  let graph = IM.fromList input
  -- let !_ = traceShow input ()

  let result = fst $ foldl' (solve graph) (False, IS.empty) (map fst input)
  putStrLn $ if result then "No" else "Yes"
