...
solve = do
  !s <- line'
  !t <- S.fromList . BS.unpack <$> line'

  let !s' = S.fromList . catMaybes $ BS.zipWith (\a b -> if isUpper b then Just a else Nothing) s (BS.tail s)
  printYn $ S.size (S.intersection s' t) == S.size s'

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc412/tasks/abc412_b
main :: IO ()
main = runIO solve
