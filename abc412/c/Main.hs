...
solve = do
  ...
  let !h1 = U.head hs
  let !hn = U.last hs
  let !hs' = U.takeWhile (<= hn) . U.dropWhile (< h1) $ U.modify VAI.sort hs

  let !res = inner (1 :: Int, h1, S.delete h1 . S.fromList $ U.toList hs')
        where
          inner :: (Int, Int, S.Set Int) -> Int
          inner (!acc, !cur, !s)
            | cur >= hn = max 2 acc
            | cur >= (hn + 1) `div` 2 = acc + 1
            | otherwise = case S.lookupLE (2 * cur) s of
                Just h -> inner (acc + 1, h, S.delete h s)
                Nothing -> -1

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc412/tasks/abc412_c
main :: IO ()
main = runIO $ do
  n <- int'
  replicateM_ n solve
