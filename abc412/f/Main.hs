...
solve = do
  ...
  let !c = c_ - 1
  let !xs0' = dbgId $ U.accumulate (+) xs0 $ U.singleton (c, 1)

  let !ixs = dbgId $ U.modify (VAI.sortBy (comparing (Down. snd))) $ U.indexed xs0'
  let (!colors, !xs) = U.unzip ixs
  let !ic = fromJust $ U.findIndex (== c) colors

  let !res = U.constructN (G.length ixs) $ \vec -> case U.length vec of
        -- 初めて起こる期待値: 1 / p
        0 -> 1 / ((modInt (xs G.! 0) - 1) * divN)
        iSelf ->
          let !self = 1 / ((modInt (xs G.! iSelf) - 1) * divN)
              !other = U.sum $ U.imap f vec
                where
                  f i e = (1 + e) * modInt (xs G.! i) * divN
              !_ = dbg (iSelf, self, other)
           in self + other

  let !_ = dbg res
  printBSB $ res G.! ic

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc412/tasks/abc412_f
main :: IO ()
main = runIO solve
