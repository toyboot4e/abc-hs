...
main = do
  ...
  -- Stores the maximum length so far

  let !undef = -1 :: Int
  !result <- do
    -- [0, height] -> maximum length
    !maxTree <- newSTree max n (0 :: Int)

    -- height
    dp <- VUM.replicate n undef
    VUM.write dp 0 0

    VU.forM_ xs $ \x -> do
      !c <- succ <$> querySTree maxTree (0, x)
      modifySTree maxTree (max c) x
      VUM.modify dp (max c) x
      -- let !_ = traceShow (x, c) ()
      return ()
