...
count :: [Int] -> Int
count is = sum . snd $ mapAccumL (\n x -> (pred n, pred n * x)) (length is) is
  where
    !_ = dbg (is)
...
main = do
  ...
  let toLen x = min (succ x) (nInput - x)
  let !xn = accumArray @Array (flip (:)) [] (0, nInput) $ zipWith (\i x -> (x, toLen i)) [0 :: Int ..] input
  let !revResult = sum $ map (count . sort) (elems xn)

  let !nLines = sum $ map (\len -> (len `div` 2) * (nInput - len + 1)) [2 .. nInput]
  let !_ = dbg (nLines, revResult)

  print $ nLines - revResult
