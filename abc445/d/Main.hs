...
solve = do
  (!h0, !w0, !n) <- ints3P
  !hws <- U.replicateM n ints2P

  -- First one: fullfil x or y
  -- Second one: fullfil x or y

  let ins :: Int -> Int -> M.Map Int (S.Set Int) -> M.Map Int (S.Set Int)
      ins k v mss = case M.lookup k mss of
        Nothing -> M.insert k (S.singleton v) mss
        Just ms -> M.insert k (S.insert v ms) mss

  let del :: Int -> Int -> M.Map Int (S.Set Int) -> M.Map Int (S.Set Int)
      del k v mss = case M.lookup k mss of
        Nothing -> error $ "cannot delete " ++ show (k, v, mss)
        Just set ->
          let set' = S.delete v set
           in if S.size set' == 0
                then M.delete k mss
                else M.insert k set' mss

  let lkup :: Int -> M.Map Int (S.Set Int) -> Maybe Int
      lkup k mss = do
        set <- M.lookup k mss
        Just $ S.findMin set

  let fromH0 = dbgId $ U.ifoldl' (\mss i (!h, !_) -> ins h i mss) M.empty hws
  let fromW0 = dbgId $ U.ifoldl' (\mss i (!_, !w) -> ins w i mss) M.empty hws

  let (!res, !_, !_, !_, !_) = U.foldl' step s0 (U.generate n id)
        where
          s0 = ([] :: [(Int, (Int, Int))], 0 :: Int, 0 :: Int, fromH0, fromW0)
          step (!acc, !y, !x, !fromH, !fromW) _
            | Just i <- lkup restH fromH =
                let (!_, !delW) = hws G.! i
                    !fromH' = del restH i fromH
                    !fromW' = del delW i fromW
                    !_ = dbg ("delW", (delW, restH))
                 in ((i, (y, x)) : acc, y, x + delW, fromH', fromW')
            | Just i <- lkup restW fromW =
                let (!delH, !_) = hws G.! i
                    !fromH' = del delH i fromH
                    !fromW' = del restW i fromW
                    !_ = dbg ("delH", (delH, restW))
                 in ((i, (y, x)) : acc, y + delH, x, fromH', fromW')
            | otherwise = error "nothing fullfils"
            where
              restH = h0 - y
              restW = w0 - x

  let result = U.accumulate (flip const) (U.replicate n (-1 :: Int, -1 :: Int)) $ U.fromList res
  U.forM_ result $ \(!y, !x) -> do
    printBSB (y + 1, x + 1)

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc445/tasks/abc445_d
main :: IO ()
main = runIO solve
