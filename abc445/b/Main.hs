...
pad :: Int -> BS.ByteString -> BSB.Builder
pad n s = p <> showBSB s <> p
  where
    len = (n - BS.length s) `div` 2
    p = showBSB (BS.replicate len '.')

solve :: StateT BS.ByteString IO ()
solve = do
  !n <- intP
  !ss <- V.replicateM n lineP
  let l = V.maximum $ V.map BS.length ss
  printBSB . unlinesBSB $ V.map (pad l) ss

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc445/tasks/abc445_b
main :: IO ()
main = runIO solve
