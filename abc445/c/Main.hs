...
solve = do
  ...
  -- let p1 = Permutation.new xs
  -- let p10 = stimes (10 :: Int) p1
  -- let p10_100 = stimes (100 :: Int) p10

  -- printVec . U.map succ $ Permutation.unPermutation p10_100

  let res = U.constructrN n $ \sofar -> case G.length sofar of
        0 -> n - 1
        len ->
          let x = xs G.! (n - 1 - len)
              x' = x - (n - len)
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc445/tasks/abc445_c
main :: IO ()
main = runIO solve
