...
solve = do
  (!n, !k) <- ints2P
  !ess <- V.generateM n $ \r -> do
    EV.iconcatMap (\c x -> U.fromListN 2 [(r, c, x), (r, c, x + n)]) <$> intsP

  let dist = (2 * n) (U.concat (V.toList ess)) (-1 :: Int)
  printVec $ U.drop n dist

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc445/tasks/abc445_f
main :: IO ()
main = runIO solve
