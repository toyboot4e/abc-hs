...
lcmMap :: M.Map Int Int -> M.Map Int Int -> M.Map Int Int
lcmMap = M.unionWith max

{-# INLINE modInt #-}
modInt :: Int -> MI.ModInt 998244353
modInt = MI.new

eval :: M.Map Int Int -> MI.ModInt 998244353
eval kvs = foldl' (\acc (!k, !v) -> acc * (modInt k ^ v)) (modInt 1) $ M.assocs kvs

solve :: StateT BS.ByteString IO ()
solve = do
  !n <- intP
  !xs <- V.map (M.fromList . U.toList . EM.primeFactors) . U.convert <$> intsP

  let !_ = dbg (n, "----------", xs)
  let ls = G.scanl' lcmMap M.empty xs
  let rs = G.scanr' lcmMap M.empty xs

  let res = G.zipWith ((eval .) . lcmMap) ls $ G.tail rs
  printVec res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc445/tasks/abc445_e
main :: IO ()
main = runIO $ do
  t <- intP
  replicateM_ t solve
