...
joints :: Int -> Int -> [[Int]]
joints maxSum ni0 = map (map succ) $ inner [] maxSum' ni0
  where
    maxSum' = maxSum - ni0
    inner :: [Int] -> Int -> Int -> [[Int]]
    inner !acc !_ 0 = [acc]
    inner !acc !nRest !i = concatMap (\n -> inner (n : acc) (nRest - n) (pred i)) [0 .. nRest]

applyJoints :: [String] -> [Int] -> String
applyJoints !ss [] = head ss
applyJoints (!s : ss) (!i : ii) =
  s ++ (replicate i '_') ++ applyJoints ss ii
...
main = do
  ...
  !xs <- replicateM nx getLine
  !ys <- S.fromList <$> replicateM ny getLine

  when (nx == 1) $ do
    let !_ = dbg (xs, length (head xs))
    let !_ = dbg (ys, S.member (head xs) ys)
    putStrLn $ if (length (head xs) >= 3) && S.notMember (head xs) ys
      then head xs
      else "-1"
    exitSuccess

  let !s = sum $ map length xs

  -- TODO: check trivial case (n = 1, n = 2)
  -- when s < 3

  let !xss = permutations xs
  let !iss = joints (16 - s) (nx - 1)

  let !strings = concatMap (\ss -> map (\is -> applyJoints ss is) iss) xss
  let !result = fromMaybe ("-1") $ find (`S.notMember` ys) strings
