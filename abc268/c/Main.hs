...
main = do
  ...
  let diffs = zipWith (\a b -> (b - a) `mod` n) [1 :: Int ..] xs
  let arr = accumArray @UArray (+) (0 :: Int) (0, pred n) $ map (,1) diffs
  -- let !_ = traceShow (xs, diffs, arr) ()

  let result = foldl' step 0 [0 .. pred n]
      step acc i = max acc $ arr ! i + arr ! ((i + 1) `mod` n) + arr ! ((i + 2) `mod` n)
