...
main = do
  ...
  let dists = map (\x -> dist n x) ps
  let offset = (sum dists) `div` n
  let f x = pmap n x ps
  print $ maximum [f offset, f ((offset + (n - 1)) `rem` n), f ((offset + 1) `rem` n)]
...
pmap n offset ps =
  let xs = map (\(i, p) -> ((p + offset) `rem` n) - i) (zip [0..] ps)
      dists = map (\d -> dist n d) xs
  in
    length $ filter (\d -> abs d <= 1) dists
