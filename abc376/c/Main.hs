...
solve = do
  !n <- int'
  !xs <- U.modify (VAI.sortBy (comparing Down)) <$> intsU'
  !ys <- U.modify (VAI.sortBy (comparing Down)) <$> intsU'
  let (!ls, !rs) = eat (U.toList xs) (U.toList ys)
  if null rs
    then do
      let [!l] = ls
      printBSB l
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc376/tasks/abc376_c
main :: IO ()
main = runIO solve
