...
solve = do
  ...
  let inner (acc :: Int) lastT [] = acc
      inner acc lastT (t:ts)
        | (t - lastT) < c = inner acc lastT ts
        | otherwise = inner (acc + 1) t ts

  printBSB $ inner 0 (-c) xs

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc376/tasks/abc376_a
main :: IO ()
main = runIO solve
