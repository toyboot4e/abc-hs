...
calc :: Int -> U.Vector Int -> U.Vector Int -> Int
calc k as bs = do
  let abs_ = U.modify (VAI.sortBy (comparing fst)) $ U.zip as bs
  let (!abHead, !abRest) = U.splitAt k abs_
  let (!res, !_, !_) = U.foldl' step s0 abRest
        where
          (!aHead, !bHead) = U.unzip abHead
          !a0 = U.maximum aHead
          !sum0 = U.sum bHead
          !s0 = (a0 * sum0, sum0, fromListMS (U.toList bHead))
          step (!acc, !sum_, !ms) (!a, !b) = (acc', sum', ms')
            where
              (!bMax, !_) = IM.findMax (innerMS ms)
              (!sum', !ms')
                | bMax > b = (sum_ - bMax + b, incMS b . decMS bMax $ ms)
                | otherwise = (sum_, ms)
              -- (a, b) を使っていないが a を使って計算……しても良い. a monotonically increases
              acc' = min acc $ a * sum'
  res
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc376/tasks/abc376_e
main :: IO ()
main = runIO solve
