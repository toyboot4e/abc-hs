...
solve = do
  ...
  let roundStep self other target
        | self > target = roundStep target other self
        -- self <= target
        | lenP <= lenN && not (inRange (self, target) other) = lenP
        | lenN <= lenP && inRange (self, target) other = lenN
        | otherwise = max lenP lenN
        where
          lenP = target - self
          lenN = self + n - target

  -- L, R: 0, 1
  let (!res, !_, !_) = U.foldl' step s0 qs
       where
         s0 = (0 :: Int, 0, 1)
         step (!acc, !l, !r) ('L', !x) = (acc + roundStep l r x, x, r)
         step (!acc, !l, !r) ('R', !x) = (acc + roundStep r l x, l, x)

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc376/tasks/abc376_b
main :: IO ()
main = runIO solve
