...
solve = do
  (!n, !q) <- ints2'
  !qs <- U.replicateM q $ (,) <$> char' <*> int1'
  let !_ = dbg qs

  let stepL :: Int -> Int -> Int -> Int -> [((Int, Int), Int)]
      stepL acc l r x
        | l == x = [((l, r), acc)]
        | otherwise = [onP, onN]
        where
          onP
            -- move R
            | inRange (l, x') r' = ((x, (x + 1) `mod` n), acc + (x' - l) + ((x' + 1) - r'))
            | otherwise = ((x, r), acc + (x' - l))
            where
              -- l < r'
              r'
                | l < r = r
          ...
          onN
            -- move R
            | inRange (x', l) r' = ((x, (x - 1) `mod` n), acc + (l - x') + (r' - (x' - 1)))
            | otherwise = ((x, r), acc + (l - x'))
            where
              -- r' < l
              r'
                | r < l = r
  ...
  let stepR :: Int -> Int -> Int -> Int -> [((Int, Int), Int)]
      stepR acc l r x = map (\((!l, !r), !n) -> ((r, l), n)) $ stepL acc r l x

  -- L, R: 0, 1
  let res = dbgId $ U.foldl' step s0 qs
        where
          s0 :: M.Map (Int, Int) Int
          s0 = M.singleton (0, 1) (0 :: Int)
          step !m (!ch, !x) = M.fromListWith min nexts
            where
              !_ = dbgId m
              nexts =
                prune
                  . concatMap
                    ( \((!l, !r), !acc) ->
                        if ch == 'L'
                          then stepL acc l r x
                          else stepR acc l r x
                    )
                  $ M.assocs m

  printBSB $ minimum $ M.elems res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc376/tasks/abc376_f
main :: IO ()
main = runIO solve
