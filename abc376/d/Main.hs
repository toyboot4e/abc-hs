...
genericDj2 :: forall w. (U.Unbox w, Num w, Ord w) => (Vertex -> U.Vector (Vertex, w)) -> Int -> Int -> w -> U.Vector (Vertex, w) -> U.Vector w
genericDj2 !gr !nVerts !nEdges !undef !vws0 = U.create $ do
...
solve = do
  ...
  let !adjs = gr `adjW` 0
  let !dists = genericDj2 (gr `adjW`) n m (-1 :: Int) adjs
  printBSB $ dists G.! 0

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc376/tasks/abc376_d
main :: IO ()
main = runIO solve
