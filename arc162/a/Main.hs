...
intersects (!f1, !b1) (!f2, !b2) = f1 < f2 && b1 > b2 || f1 > f2 && b1 < b2

-- | Greedy
solve :: IO Int
solve = do
  !l <- int
  !bs <- VU.map pred <$> intsVU

  let !inter = V.map (\i1 -> VU.map (\i2 -> intersects (i1, bs VU.! i1) (i2, bs VU.! i2)) (rangeVG 0 (l - 1))) (rangeVG 0 (l - 1))

  let !unvis = -1 :: Int
  !state <- VUM.replicate l unvis

  let !impossible = 1_000_000 :: Int
  let countInters :: Int -> IO Int
      countInters i1 =
        VU.sum
          <$> VU.mapM
            ( \i2 -> do
                !s <- VUM.read state i2
                let !b = inter V.! i1 VU.! i2
                if not (s == unvis) && b
  ...
  let !_ = dbg ("<<>>")
  forMS_ (rangeMS 0 (l - 1)) $ \_ -> do
    !is <- VU.filterM (fmap (== -1) . VUM.read state) (rangeVG 0 (l - 1))
    !candidates <- VU.mapM (\i -> do !c <- countInters i; return (i, c)) is :: IO (VU.Vector (Int, Int))

    let (!i, !n) = VU.foldl1' (\ni1 ni2 -> if snd ni1 < snd ni2 then ni1 else ni2) candidates :: (Int, Int)
    if n >= impossible
      then do
        -- can't select (TODO: early return)
        VUM.write state i 0
      else do
        -- select
        let !_ = dbg ("select", i)
  ...
  !state' <- VU.unsafeFreeze state
  return $ VU.sum state'

main :: IO ()
main = do
  !n <- int
  replicateM_ n (print =<< solve)
