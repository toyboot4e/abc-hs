...
main = do
  [n, m] <- getLineIntList
  scores <- getLineIntList

  -- bonus <- VU.modify (VAI.sortBy (comparing fst)) <$> VU.replicateM m ((\[a, b] -> (a, b)) <$> getLineIntList)
  input <- replicateM m ((\[a, b] -> (a, b)) <$> getLineIntList)
  let bonus = accumArray @UArray (+) 0 (0 :: Int, n) input

  let dp = snd $ foldl' step s0 scores
      s0 = (0, VU.replicate (n + 1) 0)
      step :: (Int, VU.Vector Int) -> Int -> (Int, VU.Vector Int)
      step (nAcc, xs) x =
        -- let !_= traceShow xs () in
        (succ nAcc, VU.generate (n + 1) f)
        where
          -- TODO: cache last max
          f 0 = VU.maximum xs
          f i
            | i > succ nAcc = 0
            | otherwise = x + xs VU.! pred i + bonus ! i
