...
main = do
  ...
  xs <- replicateM nInput getLine

  -- TODO: replcate with rolling hash and IntMap (multiset)
  let results = snd $ mapAccumL step s0 xs
      s0 = HM.empty @String @Int
      step set x =
        case HM.lookup x set of
          Just n -> (HM.adjust succ x set, x ++ "(" ++ show n ++ ")")
          Nothing -> (HM.insert x 1 set, x)
