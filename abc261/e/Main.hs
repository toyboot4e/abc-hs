...
main = do
  [n, x0] <- getLineIntList
  !ops <- V.replicateM n getTuple2

  -- fns V.! nComposite VU.! nBit
  let !fns = V.scanl' step s0 ops
      !s0 = VU.replicate 64 (True, False)
      step !vec (oper, bits) = VU.imap (\nBit (!b1, !b2) -> (f nBit b1, f nBit b2)) vec
        where
          f !nBit !b
            | oper == 1 = b .&. testBit bits nBit
            | oper == 2 = b .|. testBit bits nBit
            | oper == 3 = b `xor` testBit bits nBit
            | otherwise = error "unreachable"

  -- i >= 1
  -- bit vector -> next bit vector
  let step nComposite =
        VU.imap
          ( \nBit b ->
              let !bitMaps = fns V.! nComposite VU.! nBit
               in if b
                    then fst bitMaps
                    else snd bitMaps
          )

  let bits0 = VU.map (testBit x0) $ vRange 0 63
  let !_ = dbg (bits0)

  let !resultBits = V.scanl' (flip step) bits0 $ vRange 1 n
  let !_ = dbg (resultBits)

  V.forM_ (V.tail resultBits) $ \x -> do
    print $ VU.sum $ VU.imap (\nBit b -> if b then shift 1 nBit else 0 :: Int) x
