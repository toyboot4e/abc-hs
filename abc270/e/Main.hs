...
main = do
  ...
  let !nLoops = fromMaybe 0 . fst $ bsearch (0, 1_000_000_000_000) f
        where
          f nTake = VU.sum (VU.map (min nTake) xs) < k

  let !nTaken = VU.sum $ VU.map (min nLoops) xs

  let !result = snd $ mapAccumL step nTaken $ VU.toList $ VU.map (\x -> max 0 (x - nLoops)) xs
        where
          step !acc 0 = (acc, 0)
          step !acc !d
            | acc == k = (acc, d)
            | otherwise = (succ acc, pred d)
