#!/usr/bin/env stack
-- stack script --resolver lts-16.31 --package bytestring --package vector --package containers --package array --package vector-algorithms
...
main = do
  ...
  let i = halfIndex as n k 0 0
  print i
  -- print ii

  print $ solve as i (k - 1) 0

-- print $ solve xs

halfIndex :: VU.Vector Int -> Int -> Int -> Int -> Int -> Int
halfIndex xs n k acc i =
  if i >= (k - 1)
    then i - 1
    else
      let acc' = acc + xs VU.! i
       in if acc' > n `div` 2
            then i - 1
            else halfIndex xs n k acc' (i + 1)

solve :: VU.Vector Int -> Int -> Int -> Int -> Int
solve xs bounds i acc =
  if i <= bounds
    then acc
    else solve xs bounds (i - 2) (acc + xs VU.! i)
