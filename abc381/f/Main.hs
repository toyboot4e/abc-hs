...
solve = do
  ...
  let !m = 20 :: Int
  let !infty = maxBound @Int `div` 2

  -- dp[S]: minimum `r` where we can achive `S`.
  -- step x l: find next `i` s.t. `i > l && s[i] = x`
  let !maps = V.accumulate (flip IS.insert) (V.replicate m IS.empty) . U.convert $ U.imap (flip (,)) xs
  let step x l = IS.lookupGT l (maps G.! x)

  let !s = U.constructN (bit m) $ \sofar -> case G.length sofar of
        0 -> infty
        s | popCount s == 1 -> fromMaybe infty $ do
          let !i = countTrailingZeros s
          step i =<< step i (-1)
        s ->
          U.minimum
            . U.map
              ( \iBit -> fromMaybe infty $ do
                  let i = sofar G.! clearBit s iBit
                  guard $ i /= infty
                  step iBit =<< step iBit i
              )
            . U.filter (testBit s)
            $ U.generate m id

  printBSB . (2 *) . maximumOr 0 . U.map popCount $ U.findIndices (/= infty) s

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc381/tasks/abc381_f
main :: IO ()
main = runIO solve
