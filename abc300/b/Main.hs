...
main = do
  ...
  gridA <- listArray @UArray ((0, 0), (h - 1, w - 1)) . concat <$> replicateM h getLine
  gridB <- listArray @UArray ((0, 0), (h - 1, w - 1)) . concat <$> replicateM h getLine

  let shifts = [(y, x) | y <- [0 .. h - 1], x <- [0 .. w - 1]]
  let gridBs = map (\(!dy, !dx) -> ixmap bounds_ (\(!y, !x) -> ((y + dy) `mod` h, (x + dx) `mod` w)) gridB) shifts

  putStrLn $ yn $ any (\b -> b == gridA) gridBs
