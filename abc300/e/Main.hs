...
primeFactors235 :: Int -> [(Int, Int)]
primeFactors235 !n_ = map (\ !xs -> (head xs, length xs)) . group $ inner n_ input
  where
    input = [2, 3, 5]
    inner n pps@(p : ps)
      | n == 1 = []
...
main = do
  [n] <- getLineIntList
  let !pns = primeFactors235 n
  let !_ = dbg pns

  if any (\(!p, !_) -> p `notElem` [2, 3, 5]) pns
    then print 0
    else do
      let !j1 = maybe (0 :: Int) snd $ find ((== 2) . fst) pns
      let !j2 = maybe (0 :: Int) snd $ find ((== 3) . fst) pns
      let !j3 = maybe (0 :: Int) snd $ find ((== 5) . fst) pns
      let !end = (j1, j2, j3)
      let maxI = maximum [j1, j2, j3]

      -- NOT UNBOX!!
      let !result = runSTArray $ do
            !dp <- newArray ((0, 0, 0), (maxI + 2, maxI + 2, maxI + 2)) (modInt 0)
            writeArray dp (0, 0, 0) $ modInt 1

            -- TODO: consider recursive probability?
            let !x1 = modInt 1 / modInt 5

            -- CAN REDUCE COMINATIONS
            let !is = [(i1, i2, i3) | i1 <- [0 .. maxI], i2 <- [0 .. maxI], i3 <- [0 .. maxI]]

            forM_ is $ \i@(!i1, !i2, !i3) -> do
              !p <- readArray dp i

              let !plus = p * x1
              modifyArray dp (+ plus) (i1 + 1, i2, i3)
              modifyArray dp (+ plus) (i1 + 2, i2, i3)
              modifyArray dp (+ plus) (i1, i2 + 1, i3)
              modifyArray dp (+ plus) (i1, i2, i3 + 1)
              modifyArray dp (+ plus) (i1 + 1, i2 + 1, i3)

            return dp

      -- if any (\(!p, _) -> p `notElem` [2, 3, 5])
      --   then 
