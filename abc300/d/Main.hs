...
main = do
  [n] <- getLineIntList
  let !pLimit = isqrt (n `div` 4 `div` 3 + 5) + 1
  let !ps = VU.fromList $ takeWhile (<= pLimit) primes
  let !pps = VU.map (\x -> x * x) ps

  -- let !_ = dbg (pLimit)
  -- let !_ = dbg (VU.length ps)
  -- let !_ = dbg (ps)
  -- let !_ = dbg (pps)
  ...
  let !result =
        length
          [ ()
            | i1 <- [0 .. (VU.length ps - 3)],
              i2 <- [i1 + 1 .. (VU.length ps - 2)],
              i3 <- [i2 + 1 .. (VU.length ps - 1)],
              let !x1 = pps VU.! i1,
              let !x2 = ps VU.! i2,
              let !x3 = pps VU.! i3,
              x1 * x2 * x3 <= n
