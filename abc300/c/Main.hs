...
count :: (Int, Int) -> Int -> UArray (Int, Int) Char -> Int
count (!w, !h) !n !arr = case map solve offsets of
  [] -> 0
  xs -> sum xs
  where
    !offsets = [(y, x) | y <- [0 .. (h - 2 * n - 1)], x <- [0 .. (w - 2 * n - 1)]]
    !_ = dbg (n, offsets)
    solve (!y0, !x0) = if all (\yx -> arr ! yx == '#') yxs then 1 else 0
      where
        !o = (y0 + n, x0 + n)
        !yxs = [add2 o (dy, dx) | dy <- [-n .. n], dx <- [-n .. n], abs dy == abs dx]
        !_ = dbg ("test", o, yxs)

main :: IO ()
main = do
  [h, w] <- getLineIntList
  let !bounds_ = ((0, 0), (h - 1, w - 1)) :: ((Int, Int), (Int, Int))
  !grid <- listArray @UArray bounds_ . concat <$> replicateM h getLine
  -- let !_ = dbg (min ((h - 1) `div` 2) ((w - 1) `div` 2))
  let !counts = map (\n -> count (w, h) n grid) [1 .. min h w]
  let !counts' = reverse $ snd $ mapAccumL (\lastX x -> (x, max 0 $ x - lastX)) 0 $ reverse counts
  forM_ counts' print
