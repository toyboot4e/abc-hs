...
foldTree !tree !root !acc0 !mapp = inner (-1) root
  where
    inner !parent !v1 =
      let !v2s = filter (/= parent) $ tree ! v1
       in foldl' (\acc v2 -> acc `mapp` (inner v1 v2)) acc0 v2s
...
bfs3 :: Graph Int -> Int -> VU.Vector Int
bfs3 graph start =
  let !im = inner 0 IM.empty (IS.singleton start)
      !d2v = map swap $ IM.toList im
      !counts =  accumArray @UArray (+) (0 :: Int) (0, 3) d2v
   in VU.fromList . scanl1 (+) $ elems counts
  where
    inner :: Int -> IM.IntMap Int -> IS.IntSet -> IM.IntMap Int
    inner !depth !vis !vs
      | depth == 4 = vis
...
main = do
  [nVerts, nEdges] <- ints
  -- REMARK: Keep using the one-based vertices
  !edges <- concatMap (\a -> [a, swap a]) <$> replicateM nEdges ints2

  -- REMARK: Use zero as a guard
  let !graph = accumArray @Array (flip (:)) [] (0, nVerts) edges
  let !counts = V.map (\v -> bfs3 graph v) (rangeVG 0 nVerts)
  let !_ = dbg (counts)

  !nQueries <- readLn @Int
  !queries <- VU.replicateM nQueries ints2

  VU.forM_ queries $ \(!v, !k) -> do
    let !vec = counts V.! v
    print $ vec VU.! k
