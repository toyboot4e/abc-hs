...
contributions :: Int -> Int -> Int -> Int -> [(Int, Int)]
contributions n m x0 d = inner i0 (x0 + d * i0)
  where
    !_ = dbgAssert (d /= 0)
    !i0 = max 1 $ (\(!a, !b) -> a + min 1 b) $ divMod (0 - x0) d
    inner i x | not $ inRange (1, m) i && (inRange (0, n) x) = []
    inner i x = (i, x) : inner (succ i) (x + d)

findMex :: IS.IntSet -> Int
findMex is = fromJust $ find (`IS.notMember` is) [0..]
...
main = do
  ...
  let !cs = concatMap (\(!d, !x) -> contributions n m x d) $ zip [1..] input
  let !_ = dbg (input)
  -- let !_ = dbg (map (\(!d, !x) -> contributions n m x d) $ zip [1..] input)
  let !sets = accumArray @Array (flip IS.insert) IS.empty (1, m) cs
  let !_ = dbg (sets)

  forM_ (elems sets) (print . findMex)
