...
-- -- | DFS where all the reachable vertices from one vertex are collcetd
-- components2 :: (Int -> [Int]) -> Int -> IS.IntSet
-- components2 getNext start = inner (IS.singleton start) start
--   where
--     inner vis v
--       | null vs = vis'
--       | otherwise = foldl' inner vis' vs
--       where
--         vs = filter (`IS.notMember` vis) $ getNext v
--         vis' = IS.union vis $ IS.fromList vs

-- | BFS template for collecting shortest paths from one vertex to every other.
components2 :: (Int -> [Int]) -> Int -> IM.IntMap Int
components2 getNext start = inner 0 IM.empty (IS.singleton start)
  where
    inner :: Int -> IM.IntMap Int -> IS.IntSet -> IM.IntMap Int
    inner depth vis vs
      | IS.null vs = vis
      ...
      where
        vis' = IM.union vis $ IM.fromSet (const depth) vs
        vs' = IS.fromList $ filter (`IM.notMember` vis') $ concatMap getNext (IS.toList vs)
...
main = do
  ...
  let indexIt (y, x) = x + y * n

  let deindexIt :: Int -> (Int, Int)
      deindexIt i = quotRem i n

  let !dxys = mapMaybe f [0 .. isqrt m]
      f x =
        let xx = x * x
            y = isqrt (m - xx)
         in if y * y + xx == m
  ...
  let !deltas = concatMap (\(!y, !x) -> [(y, x), (- y, x), (y, -x), (- y, -x)]) dxys
  -- let !_ = traceShow ("dxys", dxys) ()
  -- let !_ = traceShow ("deltas", deltas) ()

  -- TODO: count cost
  let !vis = components2 f $ indexIt (0, 0)
      f :: Int -> [Int]
      f =
        map indexIt
          . filter (inRange bounds_)
          . ( \yx ->
                -- let !_ = traceShow (yx, map (add2 yx) deltas) () in
                  map (add2 yx) deltas
            )
          . deindexIt

  forM_ [0 .. pred n] $ \y -> do
    let xs = map (\ !x -> fromMaybe (-1) $ IM.lookup (indexIt (y, x)) vis) [0 .. pred n]
