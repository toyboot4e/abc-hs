...
solve = do
  ...
  -- TODO: mapAccumL, but with filters? hmm
  let res = runST $ do
        que <- Q.new q
        ...
        U.foldM'_
          ( \ms (!q, !t, !w) -> do
              if q == 1
                then do
                  -- insert
                  pure $ S.insert (w - t, t) ms
                else do
                  -- pop
                  case S.lookupMax ms of
                    Just key@(!x, !_t0) -> do
                      Q.pushBack que $ min v (x + t)
                      pure $ S.delete key ms
                    Nothing -> do
                      Q.pushBack que (-1 :: Int)
                      pure ms
          )
          S.empty
          qs
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc471/tasks/abc471_d
main :: IO ()
main = runIO solve
