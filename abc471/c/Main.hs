...
solve = do
  !n <- intP
  !is0 <- S.fromList . U.toList <$> intsP

  let mv :: (Int, Int, S.Set Int) -> Int -> (Int, Int, S.Set Int)
      mv (!acc, !i, !is) x =
        let !l = abs (i - x)
         in (acc + l, x, S.delete x is)

  let f :: (Int, Int, S.Set Int) -> Int
      f (!acc, !i, is)
        | S.null is = acc
        | otherwise = case (S.lookupLT i is, S.lookupGT i is) of
            (Nothing, Nothing) -> error "unreachable"
            (Just l, Nothing) -> f $ mv (acc, i, is) l
            (Nothing, Just r) -> f $ mv (acc, i, is) r
            (Just l, Just r) ->
              let dl = i - l
                  dr = r - i
               in if dl <= dr
                    then f $ mv (acc, i, is) l
                    else f $ mv (acc, i, is) r

  printBSB $ f (0, 0, is0)

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc471/tasks/abc471_c
main :: IO ()
main = runIO solve
