...
solve = do
  ...
  let !xMax = U.maximum xs
  let !res = fromJust $ bisectL 1 xMax $ \i -> U.sum (U.map (min i) xs) <= m

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc365/tasks/abc365_c
main :: IO ()
main = runIO solve
