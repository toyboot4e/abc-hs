...
calc xs iBit =
  let !n = G.length xs
      !bs = dbgId $ U.map (bool (0 :: Int) 1) $ U.postscanl' (\isOn x -> isOn `xor` testBit x iBit) False xs
      !csum = note iBit $ csum1D bs
      (!res, !_) = note iBit $ U.ifoldl' step s0 (U.init xs)
        where
          s0 = (0 :: Int, False)
          step (!acc, !b) i x = (dn * bit iBit + acc, b `xor` testBit x iBit)
            where
              dn
                | not b = csum +! (i + 1, n - 1)
                | b = (n - (i + 1)) - csum +! (i + 1, n - 1)
   in res
...
solve = do
  !n <- int'
  !xs <- intsU'
  -- let !res = U.sum $ U.generate 4 $ calc xs
  let !res = U.sum $ U.generate 28 $ calc xs
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc365/tasks/abc365_e
main :: IO ()
main = runIO solve
