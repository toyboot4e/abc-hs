...
solve = do
  ...
  let (!r1, !r2, !r3) = BS.foldl' step s0 s
        where
          s0 = (0 :: Int, 0 :: Int, 0 :: Int)
          -- R < P < S < R ..
          step (!n1, !n2, !n3) c = dbgId (n1', n2', n3')
            where
              (!n1', !n2', !n3')
                | c == 'R' = step' (n1, n2, n3)
                | c == 'P' =
                  let (!x2, !x3, !x1) = step' (n2, n3, n1)
                   in (x1, x2, x3)
                | c == 'S' =
                  let (!x3, !x1, !x2) = step' (n3, n1, n2)
                   in (x1, x2, x3)
          step' (!n1, !n2, !n3) = (n1', n2', n3')
            where
              n1' = max n2 n3
              n2'
                | n1 == -1 && n3 == -1 = -1
                | otherwise = max (n1 + 1) (n3 + 1)
              n3' = -1

  printBSB $ r1 `max` r2 `max` r3

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc365/tasks/abc365_d
main :: IO ()
main = runIO solve
