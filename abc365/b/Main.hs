...
solve = do
  ...
  let !min1 = U.maximum xs
  let !i1 = dbgId . fromJust $ U.findIndex (== min1) xs
  let !xs' = U.take i1 xs U.++ U.drop (i1 + 1) xs
  let !min2 = dbgId $ U.maximum xs'
  let !i2 = dbgId . fromJust $ U.elemIndex min2 xs'

  printBSB . (+ 1) $ bool i2 (i2 + 1) $ i1 <= i2

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc365/tasks/abc365_b
main :: IO ()
main = runIO solve
