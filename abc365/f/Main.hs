...
solve = do
  ...
  (!maps, !sizes) <- runST $ do
    ...
    let !tpsByPerson = V.map (V.fromList . reverse) . V.accumulate (:) (V.replicate n []) $ V.map swap $ U.modify (VAI.sortBy (comparing fst)) tps
    V.iforM_ tpsByPerson $ \iPerson tps' -> do
      forM_ tps 
