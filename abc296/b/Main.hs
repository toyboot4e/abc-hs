...
main :: IO ()
main = do
  grid <- listArray @UArray ((1, 1), (8, 8)) . concat <$> replicateM 8 (map (== '*') <$> getLine)
  let ys = VU.fromList $ ['a' .. 'h']

  forM_ (range ((1, 1), (8, 8))) $ \(!y, !x) -> do
    when (grid ! (y, x)) $ do
      let !_ = traceShow (y, x, (pred y, x)) ()
      let s = ys VU.! pred x : show (9 - y)
      putStrLn s
