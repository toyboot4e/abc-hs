...
main = do
  ...
  !xs0 <- VU.map pred <$> getLineIntVec
  !ys0 <- VU.map pred <$> getLineIntVec
  let !b0 = xs0 == ys0

  let !xs1 = VU.modify VAI.sort xs0
  let !ys1 = VU.modify VAI.sort ys0
  let !b1 = xs1 == ys1

  let !anyPivot = any ((>= 2) . length) $ group $ VU.toList xs1

  let !sx = invNumVU xs0
  let !sy = invNumVU ys0
  let !_ = traceShow ("nSwaps:", sx, sy) ()

  -- !!!!!!
  let b2 = even $ sx + sy

  let !result = b0 || b1 && (anyPivot || b2)
