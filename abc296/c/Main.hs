...
main = do
  [n, x] <- getLineIntList
  !xs <- VU.modify VAI.sort <$> getLineIntVec
  -- let !_ = traceShow (xs) ()

  let result = x == 0 || foldl' step s0 [0 .. pred $ pred n]
      s0 = False
      -- TODO: shakutori
      step :: Bool -> Int -> Bool
      step True !_ = True
      step False !l = maybe False (\i -> abs x == (xs VU.! i - xs VU.! l)) . fst $ bsearch (succ l, pred n) f
         where
           f !i =
             -- let !_ = traceShow (l, i, abs (xs VU.! i - xs VU.! l) <= abs x) () in
             xs VU.! i - xs VU.! l <= abs x
