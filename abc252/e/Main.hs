...
-- | Create shortest path tree using the Dijkstra algorithm
djTreeEdgesUndirected :: forall a. (Num a, Ord a) => WGraph a -> Int -> [(Int, Int)]
djTreeEdgesUndirected !graph !start =
  let !result = inner (H.singleton $! H.Entry 0 (-1, start)) IM.empty []
   -- delete `(-1, start)` from the result:
   in init result
  where
    merge :: H.Entry a (Int, Int) -> H.Entry a (Int, Int) -> H.Entry a (Int, Int)
    merge (H.Entry !cost1 !_) (H.Entry !cost2 !v2) = H.Entry (cost1 + cost2) v2

    inner :: H.Heap (H.Entry a (Int, Int)) -> IM.IntMap a -> [(Int, Int)] -> [(Int, Int)]
    inner !heap !vis !edges
      | H.null heap = edges
      | IM.member v vis = inner heap' vis edges
      | otherwise = inner heap'' vis' edges'
      where
        -- pop and visit it
        (entry@(H.Entry cost (parent, v)), heap') = fromJust $! H.uncons heap
        vis' = IM.insert v cost vis
        edges' = (min parent v, max parent v) : edges

        -- push neighbors
        vs :: [H.Entry a (Int, Int)]
        vs = map (merge entry . (second (v,))) $ filter ((`IM.notMember` vis') . H.payload) $! graph ! v
        heap'' = foldl' (flip H.insert) heap' $ vs
...
main = do
  ...
  !edges <- replicateM nEdges $ do
    (!v1, !v2, !c) <- ints3
    return $ (pred v1, pred v2, c)

  let !input = concatMap (\(!v1, !v2, !c) -> [(v1, H.Entry c v2), (v2, H.Entry c v1)]) edges
  let !graph = accumArray @Array (flip (:)) [] (0, pred nVerts) input
  let !usedEdges = djTreeEdgesUndirected graph 0

  -- (v1, v2) -> original index
  let !edgeToIndex = M.fromList $ map (\(!i, (!v1, !v2, _)) -> ((min v1 v2, max v1 v2), i)) $ zip [(1 :: Int) ..] edges
  let !indices = map (edgeToIndex M.!) usedEdges
