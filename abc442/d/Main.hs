...
solve = do
  (!n, !q) <- ints2P
  as <- intsP
  !qs <- U.replicateM q $ intP >>= \case
    1 -> (1 :: Int,,-1) <$> int1P
    2 -> (2 :: Int,,) <$> int1P <*> intP
    _ -> error "unreachable"

  seg <- Seg.build $ U.map Sum as
  U.forM_ qs $ \case
    (1, !i, !_) -> do
      x1 <- Seg.read seg i
      x2 <- Seg.read seg $ i + 1
      Seg.write seg i x2
      Seg.write seg (i + 1) x1
    (2, !l, !r) -> do
      Sum s <- Seg.prod seg l r
      printBSB s

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc442/tasks/abc442_d
main :: IO ()
main = runIO solve
