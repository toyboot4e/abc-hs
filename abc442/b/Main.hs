...
solve = do
  !q <- intP
  !qs <- U.replicateM q intP

  let (!_, !res) = EV.mapAccumL ((g .) . f) s0 qs
        where
          s0 = (0 :: Int, False)
          f (!vol, !isPlaying) 1 = (vol + 1, isPlaying)
          f (!vol, !isPlaying) 2 = (max 0 (vol - 1), isPlaying)
          f (!vol, !isPlaying) 3 = (vol, not isPlaying)
          f _ _ = error "unreachable"
          g s@(!vol, !isPlaying) = (s, vol >= 3 && isPlaying)

  U.forM_ res printYn

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc442/tasks/abc442_b
main :: IO ()
main = runIO solve
