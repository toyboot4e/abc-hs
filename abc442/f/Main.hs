...
solve = do
  !n <- intP
  !rows <- V.replicateM n (U.map (bool (0 :: Int) 1 . (== '.')) . U.fromList . BS.unpack <$> lineP)

  -- seg[blacKLen]: [0, blacKLen) is black
  -- seg <- Seg.build $ U.replicate (n + 1) (Max (0 :: Int))
  
  let res = G.foldl' step s0 rows
        where
          s0 = U.replicate (n + 1) (0 :: Int)
          step acc row =
            U.generate (n + 1) $  \lenBlack ->
              let nBlackL = bool 0 (csum +! (0, lenBlack - 1)) $ lenBlack > 0
                  nBlackR = bool 0 (csum +! (lenBlack, n - 1)) $ lenBlack < n
                  nOp = (lenBlack - nBlackL) + nBlackR
                  nMinOp = mins G.! lenBlack
               in nOp + nMinOp
            where
              csum = csum1D row
              mins = U.scanr' min (maxBound @Int) acc
              !_ = dbg ("acc", acc)
              !_ = dbg ("mins", mins)

  printBSB $ G.minimum res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc442/tasks/abc442_f
main :: IO ()
main = runIO solve
