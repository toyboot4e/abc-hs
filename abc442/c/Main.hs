...
solve = do
  (!n, !m) <- ints2P
  !uvs <- U.replicateM m ints11P

  -- dsu <- Dsu.new n
  -- U.forM_ uvs $ uncurry (Dsu.merge dsu)

  -- top3 <- U.take 3 . U.modify (VAI.sortBy (comparing Down)) . U.convert . G.map U.length <$> Dsu.groups dsu
  -- let !_ = dbg top3
  -- let apply ex = U.foldl' (*) (1 :: Int) others
  --       where
  --         others
  --           | Just i <- U.findIndex (== ex) top3 =
  --               -- TODO: update?
  --               U.cons (ex - 1) $ U.ifilter (const . (/= i)) top3
  --           | otherwise = top3

  -- top4 <- U.take 4 . U.modify (VAI.sortBy (comparing Down)) . U.convert . G.map U.length <$> Dsu.groups dsu

  let cnts = U.accumulate (+) (U.replicate n (0 :: Int)) $ U.concatMap (\(!u, !v) -> U.fromListN 2 [(u, 1), (v, 1)]) uvs

  res <- U.generateM n $ \i -> do
    -- s <- Dsu.size dsu i
    let !s = cnts G.! i + 1
    let k = n - s
    let !_ = dbg (i, s)
    pure $ k * (k - 1) * (k - 2) `div` 6

  printBSB $ unwordsBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc442/tasks/abc442_c
main :: IO ()
main = runIO solve
