...
main = do
  [nVerts, nEdges] <- ints
  !edges <- replicateM nEdges ((\[!a, !b, !_] -> (pred a, pred b)) <$> ints)
...
--     forM_ edges $ \(!v1, !_) -> do
--       !root <- rootMUF uf v1
--       VUM.modify vec succ root
--     VU.unsafeFreeze vec
-- 
--   let !_ = dbg (edgeCounts)
-- 
--   !res <-
--     length
--       <$> filterM
--         ( \v -> do
--             !root <- rootMUF uf v
--             !size <- sizeMUF uf root
--             -- TODO:
--             return $ root == v && edgeCounts VU.! root `elem` [pred size, size]
--         )
--         [0 .. pred nVerts]
-- 
--   print res
  ...
  !uf <- newMUF nVerts
  forM_ edges $ uncurry (uniteMUF uf)

  !roots <- IS.fromList <$> mapM (rootMUF uf) [0 .. pred nVerts]
  print $ IS.size roots
