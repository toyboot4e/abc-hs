...
solve = do
  (!n, !q) <- ints2'
  s0 <- line'
  !ixs <- U.replicateM q ((,) <$> int1' <*> char')

  s <- U.unsafeThaw (U.fromList (BS.unpack s0))
  let !b0 = U.generate n $ \i -> fromMaybe False $ do
        a <- BS.indexMaybe s0 i
        b <- BS.indexMaybe s0 $ i + 1
        c <- BS.indexMaybe s0 $ i + 2
        return $ [a, b, c] == "ABC"
  let !n0 = U.length $ U.filter id b0
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc372/tasks/abc372_c
main :: IO ()
main = runIO solve
