...
main :: IO ()
main = do
  [n, goal] <- getLineIntList
  input <- concatMap (\[a, b] -> replicate b a) <$> replicateM n getLineIntList

  let result = foldl' step s0 input
      s0 = VU.generate (goal + 1) (== 0)
      step xs x = VU.generate (goal + 1) f
        where
          f i | i < x = xs VU.! i
              | otherwise = xs VU.! i || xs VU.! (i - x)

  putStrLn $ if result VU.! goal then "Yes" else "No"
