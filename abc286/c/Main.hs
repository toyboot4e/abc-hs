...
main :: IO ()
main = do
  [n, costMove, costReplace] <- getLineIntList
  xs <- BS.getLine

  let result = map solve [0 .. n - 1]
      solve nMove =
        let ans = costMove * nMove + costReplace * nReplace
            -- !_ = traceShow (nMove, nReplace, ans) ()
         in ans
        where
          nReplace = length $ filter f [0 .. (n `div` 2 - 1)]
          f offset = BS.index xs (cycle_ offset) /= BS.index xs (cycle_ $ n - 1 - offset)
          cycle_ i = (nMove + i) `rem` n
