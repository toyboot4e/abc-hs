...
dfsEveryPath (fin, fout) graph start s0 = visitNode (s0, IS.empty) start
  where
    visitNode :: IS.IntSet ->s -> Int -> s
    visitNode visits x x =
        vistNeighbors (visitNeighbors (IS.insert x visits)) x
...
main = do
  [n] <- getLineIntList
  input <- concatMap (map ((\(== 'N') . zip [0..]) <$> replicateM n getLine
  -- grid ! (to, from)
  let grid = listArray @UArray ((1, 1), (n, n)) input
