...
main :: IO ()
main = do
  [n, p, q, r, s] <- map pred <$> getLineIntList
  input <- getLineIntList
  let s1 = take (p) $ drop 0 input
  let s2 = take (q - p + 1) $ drop p input
  let s3 = take (r - q - 1) $ drop (q + 1) input
  let s4 = take (s - r + 1) $ drop r input
  let s5 = take (n - s) $ drop (s + 1) input
  -- print [s1, s2, s3, s4, s5]
  putStrLn . unwords $ concatMap (map show) [s1, s4, s3, s2, s5]
