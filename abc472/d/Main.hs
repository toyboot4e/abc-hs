...
solve = do
  ...
  let nBlocksInRow = U.generate h $ \row -> U.sum $ U.generate w (\col -> bool 0 (1 :: Int) (gr @! (row, col) == '#'))
  let nBlocksInCol = U.generate w $ \col -> U.sum $ U.generate h (\row -> bool 0 (1 :: Int) (gr @! (row, col) == '#'))
  let starts =
        U.concatMap
          ( \row ->
              U.mapMaybe
                ( \col ->
                    bool Nothing (Just (row, col)) $ nBlocksInRow G.! row == 0 && nBlocksInCol G.! col == 0
                )
                (U.generate w id)
          )
          (U.generate h id)
  ...
  let res = Gr.bfs (h, w) f undef starts'
        where
          f (!y, !x) = U.map (, 1 :: Int) . U.filter (\p -> inRange (zero2 h w) p && gr @! p == '.') $ U.map (add2 (y, x)) ortho4
          starts' = U.zip starts (U.replicate (U.length starts) (0 :: Int))

  printBSB $ G.length $ U.filter (<= k) res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc472/tasks/abc472_d
main :: IO ()
main = runIO solve
