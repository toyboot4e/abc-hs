...
solve = do
  (!n, !m, !k) <- ints3P
  !xs <- intsP
  seg <- Seg.build $ U.replicate n (Sum (0 :: Int))
  let readM i = do
        Seg.prod seg (max 0 (i - m + 1)) i
  U.iforM_ xs $ \i x -> do
    Sum r <- readM i
    let b = r + x <= k
    when b $ do
      Seg.write seg i $ Sum x
    printYn b

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc472/tasks/abc472_c
main :: IO ()
main = runIO solve
