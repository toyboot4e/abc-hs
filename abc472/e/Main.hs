...
solve = do
  ...
  let gr = Gr.build' n $ Gr.swapDupe' uvs

  let !_ = dbg (n, m)
  let !_ = dbg uvs

  let undef = -1 :: Int
  ds <- UM.replicate n undef
  GM.write ds 0 0

  ps <- UM.replicate n (-1 :: Int)

  -- res <- V.replicate 0 

  res <- evalContT $ callCC $ \exit -> do
    flip fix (0 :: Int) $ \dfs u -> do
      U.forM_ (Gr.adj gr u) $ \v -> do
        ...
        if dv == undef
          then do
            ...
            when (even (abs (dv - du))) $ do
              ps' <- U.unsafeFreeze ps
              exit . Just . U.dropWhile (/= v) $ Gr.constructPathFromRoot ps' u
    pure Nothing

  case res of
    Nothing -> printBSB "-1"
    Just vs -> do
      printBSB $ G.length vs
      printRow $ U.map succ vs

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc472/tasks/abc472_e
main :: IO ()
main = runIO $ do
  t <- intP
  replicateM_ t  solve
