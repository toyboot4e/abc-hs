...
solve !xs = VU.last $ result
  where
    !result = VU.create $ do
      dp <- VUM.replicate (VU.length xs) (maxBound :: Int)

      -- Always use it
      VUM.write dp 0 (VU.head xs)
      VUM.write dp 1 (VU.head xs)

      VU.forM_ (VU.zip (rangeVG 1 (pred (VU.length xs))) (VU.tail xs)) $ \(!i, !x) -> do
        !s0 <- VUM.read dp i
        !s1 <- VUM.read dp (i - 1)
        let !x0 = min s0 (s1 + x)
        VUM.modify dp (min x0) i

        when (i < (pred $ VU.length xs)) $ do
          VUM.modify dp (min (s1 + x)) (succ i)
...
main = do
  ...
  let !result1 = solve $ xs
  let !result2 = solve $ VU.tail xs VU.++ (VU.singleton $ VU.head xs)
  let !_ = dbg (result1, result2)
