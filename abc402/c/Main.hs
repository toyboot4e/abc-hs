...
solve = do
  ...
  elemToMenu <- VM.replicate nElems S.empty
  sets <- VM.generateM nMenus $ \iMenu -> do
    k <- int'
    vec <- U.map pred <$> intsU'
    U.forM_ vec $ \x -> do
      GM.modify elemToMenu (S.insert iMenu) x
    pure . S.fromList $ U.toList vec

  qs <- U.map pred <$> intsU'

  -- TODO: scanM'
  res <- newBuffer nElems
  U.foldM'_
    ( \acc0 iElem -> do
        !delSet <- GM.read elemToMenu iElem

        acc' <-
          foldM
            ( \acc iMenu -> do
                set' <- S.delete iElem <$> GM.read sets iMenu
                GM.write sets iMenu set'
                if S.size set' == 0
                  then pure $ acc + 1
                  else pure acc
            )
  ...
  printBSB . unlinesBSB =<< unsafeFreezeBuffer res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc402/tasks/abc402_c
main :: IO ()
main = runIO solve
