...
solve = do
  ...
  let slopes = U.map f abs
        where
          f (!a, !b)
            | dx == 0.0 = inf
            | otherwise = dy / dx
            where
              !argA = intToDouble a * 2.0 * pi / intToDouble n
              !argB = intToDouble b * 2.0 * pi / intToDouble n
              !dx = cos argA - cos argB
              dy = sin argA - sin argB

  let slopes' = M.fromListWith (+) $ U.toList $ U.map (,1 :: Int) slopes
  let nSub = sum . map (\ni -> ni * (ni - 1) `div` 2) $ M.elems slopes'

  printBSB $ m * pred m `div` 2 - nSub

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc402/tasks/abc402_d
main :: IO ()
main = runIO solve
