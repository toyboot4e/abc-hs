...
getDiagMat2' :: (HasCallStack, PrimMonad m, MonadState BS.ByteString m) => Int -> m (IxUVector (Int, Int) Int)
getDiagMat2' !n = fmap (IxVector bnd) $ do
  !vec <- UM.replicate (n * n) (0 :: Int)
  U.forM_ (U.generate n id) $ \y -> do
    !ws <- intsU'
    let !_ = dbg ws
    U.iforM_ ws $ \i dw -> do
      let !x = i
...
solve = do
  !n <- int'
  !mat <- mapIV pred <$> getDiagMat2' n
  let !_ = dbg mat

  let !res = U.foldl' f 0 (U.generate n id)
        where
          f i j
            | i >= j = mat @! (i, j)
            | otherwise = mat @! (j, i)
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc370/tasks/abc370_b
main :: IO ()
main = runIO solve
