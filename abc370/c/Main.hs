...
solve = do
  ...
  let diff = U.fromList . findIndices (uncurry (/=)) $ zip (BS.unpack s) (BS.unpack t)
  let (!pros, !cons) = dbgId $ U.partition f diff
        where
          f i = BS.index s i > BS.index t i

  -- TODO: use scan
  res <- (`evalStateT` (0 :: Int)) $ do
    buf <- VM.unsafeNew (G.length diff)
    str <- U.thaw (U.fromList (BS.unpack s))
    U.forM_ pros $ \i -> do
      GM.write str i (BS.index t i)
      str' <- G.freeze str
      j <- get
      GM.write buf j str'
      modify' (+ 1)
    U.forM_ (U.reverse cons) $ \i -> do
      GM.write str i (BS.index t i)
      str' <- G.freeze str
      j <- get
      GM.write buf j str'
  ...
  printBSB . unlinesBSB $ V.map (BS.pack . U.toList) res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc370/tasks/abc370_c
main :: IO ()
main = runIO solve
