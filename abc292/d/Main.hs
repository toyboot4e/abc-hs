...
dfsEveryVertex :: forall s. (s -> Bool, s -> Int -> s, s -> Int -> s) -> Graph -> IS.IntSet -> Int -> s -> (s, IS.IntSet)
dfsEveryVertex (isEnd, fin, fout) graph set0 start s0 = visitNode (s0, set0) start
  where
    ...
    visitNeighbors (s, visits) x
      | isEnd s = (s, visits)
      | otherwise =
        foldl' visitNode (s, visits) (graph ! x)
...
main = do
  ...
  let search start set = dfsEveryVertex (isEnd, fin, fout) graph set start (0, 0)
      isEnd = const False
      fin (nVerts, nEdges) v = (succ nVerts, nEdges + length (graph ! v))
      fout acc _ = acc

  let result = foldl' step s0 [1 .. nVerts]
      s0 = (True, IS.empty)
      step (False, set) _ = (False, set)
      step (True, set) v
        | IS.member v set = (True, set)
        | otherwise =
          let ((nv, ne), set') = search v set
           in (nv == (ne `div` 2), set')

  putStrLn $ if fst result then "Yes" else "No"
