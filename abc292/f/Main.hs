...
main = do
  ...
  let !result = fromJust $ bsearchF64L (0.0, toRadian 30.0) 0.000_000_000_1 f
        where
          f !rad =
            let !d1 = intToDouble l / cos rad
                !_ = dbg ((toDegree rad, 30.0 - toDegree rad), (cos rad, cos (toRadian 30.0 - rad)))
             in d1 * cos (toRadian 30.0 - rad) <= intToDouble r

  let !_ = dbg (result * 180.0 / pi)

  print $ intToDouble l / cos result
