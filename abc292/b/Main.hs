...
main = do
  [n, q] <- getLineIntList
  events <- replicateM q (tuple2 . map pred <$> getLineIntList)

  counts <- VUM.replicate n (0 :: Int)
  forM_ events $ \(e, x) -> case e of
    -- yellow: add 1
    0 -> VUM.modify counts succ x
    1 -> VUM.modify counts (succ . succ) x
    2 -> do
      i <- VUM.read counts x
      putStrLn $ if i >= 2 then "Yes" else "No"
    _ -> error "NOOOOO"
