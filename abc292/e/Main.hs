...
main = do
  ...
  !edges <- replicateM nEdges (tuple2 <$> getLineIntList)
  let !graph = accumArray @Array (flip (:)) [] (1, nVerts) edges

  let result = map step [1 .. nVerts]
      step v = fst $ dfsEveryVertex (isEnd, fin, fout) graph v (-1 :: Int)
        where
          isEnd = const False
          fin s _ = succ s
          fout s _ = s
