...
main = do
  [h, w] <- getLineIntList
  mat <- VU.fromList . map (\c -> c == '#') . concat <$> replicateM h getLine

  forM_ [0..(w-1)] $ \x -> do
    let n = length . filter id $ map (\y -> getMat mat w (x, y)) [0..(h-1)]
    putStr . show $ n
    if x == (w - 1) then return () else putStr " "

getMat :: VU.Vector Bool -> Int -> (Int, Int) -> Bool
getMat mat w (x, y) = mat VU.! idx
  where
    idx = x + y * w
