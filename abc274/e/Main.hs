...
main :: IO ()
main = do
  [nCities_, nTreasures] <- getLineIntList
  let !nCities = nCities_ + 1
  let !nVerts = nCities + nTreasures

  -- Add origin as a city:
  !input <- ((0, 0) :) <$> replicateM (pred nVerts) getTuple2

  let !edges =
        [ len
          | (x1, y1) <- input,
            (x2, y2) <- input,
            let !len = sqrt $ square (intToDouble $ x1 - x2) + square (intToDouble $ y1 - y2) :: Double
        ]

  let !_ = dbg (edges)
  -- graph ! (v1, v2) -> len
  let !graph = listArray @UArray ((0, 0), (pred nVerts, pred nVerts)) edges

  let !nSets = bit nVerts :: Int
  let !cityMask = bit nCities - 1
  let !treasureMask = (nSets - 1) - cityMask

  let getSpeed s = intToDouble . bit . popCount $ s .&. treasureMask

  let undef = 100_000_000_000.0 :: Double

  let !result = runSTUArray $ do
        -- dp ! (s, v)
        !dp <- newArray ((0, 0), (nSets, pred nVerts)) undef
        writeArray dp (1, 0) (0.0)

        forM_ [1 .. pred nSets] $ \s -> do
          let !speed = getSpeed s
          forM_ [0 .. pred nVerts] $ \v1 -> do
            !cost <- readArray dp (s, v1)
            unless (cost == undef) $ do
              forM_ (filter (\v2 -> not $ testBit s v2) [0 .. pred nVerts]) $ \v2 -> do
                let s' = s .|. bit v2
                let !cost' = cost + (graph ! (v1, v2)) / speed
                let !_ = dbg ((s, v1), (s', v2), (cost, cost'))
                modifyArray dp (min cost') (s', v2)

        return dp

  let !_ = dbg (result)

  let !res = minimum $ map f bits
        where
          !bits = filter ((== cityMask) . (cityMask .&.)) [0 .. pred nSets]
          -- FIXME:
          -- !bits = map (\mask -> cityMask .|. (shift mask nCities)) [0 .. treasureMask]
          -- return to the vertex `0`:
          f !s = minimum $ map (\v -> result ! (s, v) + graph ! (v, 0) / getSpeed s) [1 .. pred nVerts]
            where
              !_ = dbg ("res", s, map (\v -> result ! (s, v)) [1 .. pred nVerts])
