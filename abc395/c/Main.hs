...
solve = do
  !n <- int'
  !xs <- intsU'
  let (!res, !_) = U.ifoldl' step s0 xs
        where
          s0 = (maxBound `div` 2 :: Int, IM.empty)
          step :: (Int, IM.IntMap Int) -> Int -> Int -> (Int, IM.IntMap Int)
          step (!acc, !im) i x = case IM.lookup x im of
            Just lastI -> (min acc (i - lastI + 1), IM.insert x i im)
            _ -> (acc, IM.insert x i im)
  printBSB $ if res == maxBound `div` 2 then (-1) else res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc395/tasks/abc395_c
main :: IO ()
main = runIO solve
