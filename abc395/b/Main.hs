...
run :: Int -> Int -> String
run n 0 = replicate n '#'
run n row | row > n `div` 2 = run n (n - row - 1)
run n row = l ++ m ++ r
  where
    !_ = dbg (n, row, l, m, r)
    l = il row
    r = reverse $ il row
    m
      | even row = replicate (n - 2 * row) '#'
      | odd row = replicate (n - 2 * row) '.'
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc395/tasks/abc395_b
main :: IO ()
main = runIO solve
