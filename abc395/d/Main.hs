...
solve = do
  ...
  -- pos -> set
  sets <- VM.generate n S.singleton
  -- x -> representative (set)
  rep <- UM.generate n id
  -- representative -> pos
  repPos <- UM.generate n id
  -- pos -> representative
  posRep <- UM.generate n id

  res <- (`U.mapMaybeM` qs) $ \case
    (1, !x, !bi) -> do
      -- move x to bi
      br <- GM.read posRep bi
      xr <- GM.exchange rep x br
      GM.modify sets (S.delete x) xr
      GM.modify sets (S.insert x) br
      pure Nothing
    (2, !ai, !bi) -> do
      -- swap ai and bi
      ar <- GM.read posRep ai
      br <- GM.read posRep bi
      GM.swap posRep ai bi
      GM.swap repPos ar br

      pure Nothing
    (3, !x, !_) -> do
      -- print position of x
      rx <- GM.read rep x
      ri <- GM.read repPos rx
      pure $ Just ri

  printBSB $ unlinesBSB $ U.map succ res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc395/tasks/abc395_d
main :: IO ()
main = runIO solve
