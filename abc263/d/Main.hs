...
main :: IO ()
main = do
  [n, l, r] <- getLineIntList
  !xs <- getLineIntVec

  !ltree <- newSTree min n (0 :: Int)
  !rtree <- newSTree min n (0 :: Int)

  let !m = foldM step s0 [0 .. pred n]
      s0 = (0, 0, 0)
      step (!accL, !accR, !s) !i = do
        let !accL' = accL + l - xs VU.! i
        let !accR' = accR + r - xs VU.! (pred n - i)
        updateLeaf ltree i accL'
        updateLeaf rtree (pred n - i) accR'
        return (accL', accR', s + xs VU.! i)
  (!_, !_, s) <- m

  let !mm = minimum <$> mapM f [0 .. pred $ pred n]
      f !i = do
        !x1 <- querySTree ltree (0, i)
        !x2 <- querySTree rtree (succ i , pred n)
        return $ x1 + x2

  -- TODO: cover boundary
  !d1 <- mm
  !d2 <- querySTree ltree (0, n)
  !d3 <- querySTree rtree (0, n)
  let !d = minimum [0, d1, d2, d3]
