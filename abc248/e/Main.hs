...
-- Returns the number of the vertices that the line goes throught and the unique set of vertices that composes the line.
foldVerts :: (Int, (Int, Int)) -> (Int, (Int, Int)) -> [(Int, (Int, Int))] -> (Int, (Int, Int))
foldVerts (i01, (!y1, !x1)) (i02, (!y2, x2)) !input =
  let (!nVerts, !v1, !v2) = foldl' step s0 input
   in (nVerts, (v1, v2))
  where
    s0 :: (Int, Int, Int)
    s0 = (2, min i01 i02, max i01 i02)
    isOnLine (!y, !x) = (x2 - x1) * (y - y1) == (y2 - y1) * (x - x1)
    step :: (Int, Int, Int) -> (Int, (Int, Int)) -> (Int, Int, Int)
    step acc@(!nVerts, !i1, !i2) (!i3, !yx3)
      | not $ isOnLine yx3 = acc
      | otherwise = (succ nVerts, min i1 i3, max i2 i3)

main :: IO ()
main = do
  [n, k] <- ints
  !yxs <- VU.replicateM n ints2
  ...
  let !results =
        [ lineRepr
          | !i1 <- [0 .. pred (pred n)],
            !i2 <- [succ i1 .. pred n],
            let !i3s = [i | i <- [0 .. pred n], i /= i1, i /= i2] :: [Int],
            let (!nVerts, lineRepr) = foldVerts (i1, yxs VU.! i1) (i2, yxs VU.! i2) $ map (\i -> (i, yxs VU.! i)) i3s,
            nVerts >= k
        ]

  print $ S.size $ S.fromList results
