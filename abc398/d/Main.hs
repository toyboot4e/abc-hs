...
solve = do
  ...
  U.ifoldM'_
    (\(!s, (!dy, !dx)) i ch -> do
        let (!dy', !dx') = add2 (dy, dx) $ dir ch
        let s' = S.insert (-dy', -dx') s -- (0, 0)
        let (!r', !c') = (r - dy', c - dx')
        GM.write res i . bool 0 1 $ S.member (r', c') s'
        pure (s', (dy', dx'))
        )
    (S.singleton (0, 0), (0, 0))
    . U.fromList
  ...
  printBSB . G.foldMap showBSB =<< U.unsafeFreeze res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc398/tasks/abc398_d
main :: IO ()
main = runIO solve
