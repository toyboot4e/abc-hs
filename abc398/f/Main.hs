...
solve = do
  !s <- line'
  let n = BS.length s
  segA <- buildSTree . U.map (rh1 @100 @998244353 . ord) . U.fromList $ BS.unpack s
  segB <- buildSTree . U.map (Dual . rh1 @100 @998244353 . ord) . U.fromList $ BS.unpack s
  ...
  let test i
        | odd i = do
            ...
            let ir = (i + 1) `div` 2
            let len = n - ir
            let !_ = dbg ("odd", il, ir, len)
            Dual h1 <- foldSTree segB (il - len + 1) il
            h2 <- foldSTree segA ir (ir + len - 1)
            pure $ h1 == h2
        | otherwise = do
            -- on a char
            let iChar = i `div` 2
            let len = n - (iChar + 1)
            Dual h1 <- foldSTree segB (iChar - len) (iChar - 1)
            h2 <- foldSTree segA (iChar + 1) (iChar + len)
            pure $ h1 == h2
  ...
  let restore i
        | odd i =
          let nl = (i + 1) `div` 2
           in showBSB (BS.take nl s) <> showBSB (BS.reverse (BS.take nl s))
        | otherwise =
          let nl = i `div` 2
           in showBSB (BS.take nl s) <> showBSB (BS.index s nl) <> showBSB (BS.reverse (BS.take nl s))

  printBSB . restore =<< findIt i0

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc398/tasks/abc398_f
main :: IO ()
main = runIO solve
