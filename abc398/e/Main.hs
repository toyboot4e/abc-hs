...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc398/tasks/abc398_e
main :: IO ()
main = do
  n <- readLn @Int
  uvs <-
    U.replicateM (n - 1) $
      evalState ints11' <$> BS.getLine

  let gr = buildSG n $ swapDupeU uvs
  let hld = hldOf gr

  let uvSet = S.fromList . U.toList $ U.map (\(!u, !v) -> (min u v, max u v)) uvs
  let !es0 =
        dbgId $
          S.fromList
            [ (u, v)
              | u <- [0 .. n - 1],
                v <- [u + 1 .. n - 1],
                S.notMember (u, v) uvSet,
                odd (lengthHLD hld u v)
            ]

  esState <- newMutVar es0
  let deleteLoop = do
        es <- readMutVar esState
        let ((!u, !v), !es') = S.deleteFindMin es
        writeMutVar esState es'
        ansIO u v
        readLoop

      readLoop = do
        (!a, !b) <- note "read" . evalState ints11' <$> BS.getLine
        if (a, b) == (-2, -2)
          then do
            -- win
            liftIO exitSuccess
            ...
            -- else
            es <- readMutVar esState
            when (S.notMember (min a b, max a b) es) $ error "what??"
            writeMutVar esState . note "del->" $ S.delete (min a b, max a b) es
            deleteLoop
