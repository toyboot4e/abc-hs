...
solve = do
  !n <- int'
  !xs <- intsU'
  let freq = IM.fromListWith (+) $ U.toList $ U.map (, 1:: Int) xs
  let !res = fst $ U.ifoldl' step s0 xs
         where
           s0 = (-1, -1)
           step (!i, !xMax) j x
             | freq IM.! x == 1 && x > xMax = (j + 1, x)
             | otherwise = (i, xMax)
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc398/tasks/abc398_c
main :: IO ()
main = runIO solve
