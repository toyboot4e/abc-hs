...
eat :: Int -> [Int] -> Maybe [Int]
eat _ [] = Nothing
eat n xs
  | x >= n = Just rest
  | otherwise = Nothing
  where
    (x : rest) = sortBy (comparing Down) xs

solve :: StateT BS.ByteString IO ()
solve = do
  !xs <- intsU'
  let input = map U.length . U.group $ U.modify VAI.sort xs
  let res = do
        ys <- eat 3 input
        zs <- eat 2 ys
        pure ()
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc398/tasks/abc398_b
main :: IO ()
main = runIO solve
