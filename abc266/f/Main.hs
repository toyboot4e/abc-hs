...
{-# INLINE querySTree #-}
querySTree :: forall a m. (VU.Unbox a, PrimMonad m) => MSegmentTree (PrimState m) a -> (Int, Int) -> m (Maybe a)
...
findCycle :: Array Int [Int] -> VU.Vector Bool
findCycle !graph = VU.map (> 0) degs
  where
    !_ = dbg degs
    !degs = VU.create $ do
      let !nVerts = rangeSize $ bounds graph
      !vec <- VUM.replicate nVerts (0 :: Int)

      forM_ (assocs graph) $ \(!v1, !v2s) -> do
        forM_ v2s $ \v2 -> do
          VUM.modify vec succ v1
          VUM.modify vec succ v2
      forM_ [0 .. pred nVerts] $ \v -> do
        VUM.modify vec (`div` 2) v

      !initVerts <- filterM (\v -> (== 1) <$> VUM.read vec v) [0 .. pred nVerts]
      flip fix initVerts $ \loop vs -> do
        let m = foldM step [] vs
            step acc0 v = do
              -- the degree of `v` is 1
              VUM.write vec v 0

              !nvs0 <- filterM (\nv -> (>= 2) <$> VUM.read vec nv) $ graph ! v
              flip fix (acc0, nvs0) $ \loop' (acc, nvs) -> do
                case nvs of
                  [] -> return acc
                  (nv : nvs') -> do
                    VUM.modify vec pred nv
                    !n <- VUM.read vec nv
                    if n == 1
                      then loop' (nv : acc, nvs')
                      else loop' (acc, nvs')

        !vs' <- m
        let !_ = dbg ("loop", vs, "->", vs')
        when (not $ null vs') $ loop vs'
...
main = do
  ...
  !input <- concatMap (\(!v1, !v2) -> [(v1, v2), (v2, v1)]) <$> replicateM nVerts (both pred <$> getTuple2)

  let !graph = accumArray @Array (flip (:)) [] (0, pred nVerts) input
  let !isCycle = findCycle graph
  let !cycle = VU.map fst . VU.filter snd $ VU.zip (vRange 0 (pred nVerts)) isCycle
  let !_ = dbg ("cycle:", cycle)

  let !groups = VU.create $ do
        !g <- VU.unsafeThaw $ VU.generate nVerts id
        let m v0 parent v1 = do
              VUM.write g v1 v0
              let !v2s = filter (\v -> v /= parent && not (isCycle VU.! v)) $ graph ! v1
              forM_ v2s $ \v2 -> do
                m v0 v1 v2
        VU.forM_ cycle $ \v -> m v (-1) v
        return g
