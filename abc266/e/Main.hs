...
main = do
  ...
  let !result = VU.foldl' (const . step) (3.5 :: Double) $ vRange 2 nTry
      step !lastX = lastX * intToDouble (length lo) / intToDouble 6 + sum hi / intToDouble 6
        where
          (!lo, !hi) = partition (<= lastX) $ map intToDouble [1 .. 6]
