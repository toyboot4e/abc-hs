...
main = do
  [n] <- getLineIntList
  input <- replicateM n ((\[t, x, a] -> ((t, x), a)) <$> getLineIntList)
  let tMax = 10 ^ 5 :: Int
  -- scores ! (t, x) -> score
  let scores = accumArray @UArray (\_ x -> x) 0 ((0, 0), (tMax, 4)) input

  let dp = snd $ foldl' step s0 [0 .. tMax]
      s0 = (0 :: Int, VU.replicate 5 0)
      step :: (Int, VU.Vector Int) -> Int -> (Int, VU.Vector Int)
      step (i, xs) t =
        -- let !_ = if t < 10 then traceShow (i, xs) () else () in
        (succ i, VU.generate 5 f)
        where
          f x | x > i = 0
              | otherwise = g x
          g 0 = max (xs VU.! 0) (xs VU.! 1) + scores ! (t, 0)
          g 4 = max (xs VU.! 3) (xs VU.! 4) + scores ! (t, 4)
          g x = maximum (map (xs VU.!) [x - 1, x, x + 1]) + scores ! (t, x)
