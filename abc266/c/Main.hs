...
main :: IO ()
main = do
  vs <- replicateM 4 getTuple2
  putStrLn $ if solve vs then "Yes" else "No"

solve :: [(Int, Int)] -> Bool
solve vs = all id checks
  where
    !vecs = map (\i -> sub2 (vs !! (succ i `rem` 4)) (vs !! i)) [0 .. 3]
    !checks = map (\i -> toSign (vecs !! (succ i `rem` 4)) (vecs !! i)) [0 .. 3]
    !_ = dbg (vs)
    !_ = dbg (vecs)
    !_ = dbg (dVecs)
    toSign !dv1 !dv2 = fst dv1 * fst dv2 + snd dv1 + snd dv2 > 0
