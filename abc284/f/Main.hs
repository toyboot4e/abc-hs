...
-- TODO: rolling hash template
main :: IO ()
main = do
  [n] <- getLineIntList
  originalString <- BS.getLine
  let s = VU.fromList $ map ord (BS.unpack originalString)

  let b = 100
  let bMod = 10 ^ (9 :: Int) + 7

  let cache = powerModCache b bMod
  let getPower power = powerByCache power cache

  -- REMARK: Use 1-based index
  let hashes = VU.scanl' step 0 s
      step acc x = (b * acc + x) `rem` bMod

  let hashesRev = VU.scanl' step 0 (VU.reverse s)

  -- AB CBA C (n = 3, i = 2)
  -- 12 345 5
  -- h(3, 5) = h(1, 5) - B^3 h(1, 3)
  let hashSlice l r =
        -- | l > r = 0
        -- | l == 0 = hashes VU.! r
        -- | otherwise =
          let power = getPower (r - l + 1)
              h0r = hashes VU.! r
              h0l_ = hashes VU.! (l - 1)
           in (h0r - (power * h0l_ `rem` bMod)) `mod` bMod

  -- (l, r) in reverse (0 = rightmost)
  let hashSliceRev l r =
          let power = getPower (r - l + 1)
              h0r = hashesRev VU.! r
              h0l_ = hashesRev VU.! (l - 1)
           in (h0r - (power * h0l_ `rem` bMod)) `mod` bMod

  -- i: 0 .. n
  let solve i =
        let -- !_ = traceShow (n, i, (i + 1, i + n), (1, n - i), (2 * n - i + 1, 2 * n)) ()
            !h = hashSlice (i + 1) (i + n)
            !hRev =
              (`rem` bMod) $
                getPower i * hashSliceRev 1 (n - i)
                  + hashSliceRev (2 * n - i + 1) (2 * n)
            !_ = traceShow (n, i, h, hRev) ()
         in if h == hRev
              then Just i
              else Nothing

  -- let !_ = traceShow hashes ()
  -- let !_ = traceShow hashesRev ()

  let result = mapMaybe solve [0 .. n]

  case result of
    [] -> print (-1 :: Int)
    (i : _) -> do
      let ss = BS.take n $ BS.drop i originalString
      putStrLn . reverse $ BS.unpack ss
      print i
