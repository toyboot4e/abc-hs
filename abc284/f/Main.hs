...
main = do
  ...
  let !rhashRev = newRHash @MyModulus revS
  let revIndex !i = 2 * n - 1 - i

  let !resultLen = find f [0 .. n]
      f !len =
        let !s11 = sliceRHash rhash 0 (0 + len - 1)
            !s12 = sliceRHash rhash (len + n) (2 * n - 1)

            !s1 = consHashSlice rhash s11 s12
            -- let !s2 = sliceRHash rhashRev len (len + n - 1)
            !s2 = sliceRHash rhashRev (revIndex $ len + n - 1) (revIndex len)

            -- !_ = dbg (len, (0, 0 + len - 1), (len, len + n - 1), (len + n, 2 * n - 1))
            !_ = dbg (len, s1 == s2, (s11, s12), s1, s2)
         in s1 == s2

  case resultLen of
    Just !len -> do
      putStrLn $ take n $ drop (n - len) revS
      print len
    Nothing -> do
      print (-1 :: Int)
