...
main = do
  ...
  let !leaf = fst . fromJust $ find (\(!_, !v2s) -> case v2s of [_] -> True; _ -> False) $ assocs tree
  let !_ = dbg ("leaf", leaf)

  !counts <- VUM.replicate nVerts (0 :: Int)

  let dfs :: Int -> Vertex -> Vertex -> IO ()
      dfs depth parent v1 = do
        when (depth `rem` 3 == 1) $ do
          -- it's the center of the star:
          let !degree = length (tree ! v1)
          VUM.modify counts succ degree

        forM_ (tree ! v1) $ \v2 -> do
          when (v2 /= parent) $ do
            dfs (succ depth) v1 v2

  dfs 0 (-1) leaf

  !result <- VU.unsafeFreeze counts :: IO (VU.Vector Int)
  let !_ = dbg (result)

  putStrLn $
    unwords . map show . concatMap (\(!deg, !n) -> replicate n deg) $
      VU.toList $
        VU.filter ((> 0) . snd) $
          VU.zip (rangeVU 0 (pred nVerts)) $
            result
