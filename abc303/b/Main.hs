...
main = do
  [n, m] <- ints
  !input <- replicateM m ints

  let !pairs = concatMap f input
      f is = map (\(!i1, !i2) -> (min i1 i2, max i1 i2)) $ zip is (tail is)

  let !dedup = S.fromList pairs
  let !_ = dbg (dedup)

  print $ n * pred n `div` 2 - S.size dedup
