...
main :: IO ()
main = do
  [onA, onShiftA, onCaps] <- ints
  !s <- getLine
  -- let !_ = dbg (s)

  -- (oneIfLarge, lenght)
  let !rle = map (\xs -> (if' (head xs == 'A') 1 0, length xs)) $ group s :: [(Int, Int)]
  -- let !_ = dbg (rle)

  let !result = foldl' step s0 rle
      -- (accNormal, accCaps)
      s0 = (0, 1_000_000_000_000) :: (Int, Int)
      step (!accNormal, !accCaps) (0, len)
        = (minimum [-- from normal
                    accNormal + onA * len, accNormal + onCaps + onShiftA * len + onCaps,
                    -- from caps
                    accCaps + onShiftA * len + onCaps, accCaps + onCaps + onA * len
                   ],
           minimum [ -- from normal
                    accNormal + onA * len + onCaps, accNormal + onCaps + onShiftA * len,
                    -- from caps
                    accCaps + onShiftA * len, accCaps + onCaps + onA * len + onCaps
                   ])
      step (!accNormal, !accCaps) (1, len)
        = (minimum [-- from normal
                    accNormal + onShiftA * len, accNormal + onCaps + onA * len + onCaps,
                    -- from caps
                    accCaps + onA * len + onCaps, accCaps + onCaps + onShiftA * len
                   ],
           minimum [ -- from normal
                    accNormal + onShiftA * len + onCaps, accNormal + onCaps + onA * len,
                    -- from caps
                    accCaps + onA * len, accCaps + onCaps + onShiftA * len + onCaps
                   ])

  -- let !_ = dbg (result)
  print $ uncurry min result
