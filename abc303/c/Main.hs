...
main :: IO ()
main = do
  [n, m, hp0, recoverLimit] <- ints
  !orders <- getLine
  !items0 <- S.fromList <$> replicateM m (swap <$> ints2)

  let !result = foldl' step s0 (map parseDir orders)
      s0 = (items0, (0, 0), hp0)
      step (_, _, 0) _ = (S.empty, (0, 0), -1)
      step (_, _, -1) _ = (S.empty, (0, 0), -1)
      step (items, yx0, hp) dyx
        | S.member (add2 yx0 dyx) items && pred hp < recoverLimit = (S.delete (add2 yx0 dyx) items, add2 yx0 dyx, recoverLimit)
        | otherwise = (items, add2 yx0 dyx, pred hp)

  putStrLn . yn $ ((>= 0) . thd3) result
