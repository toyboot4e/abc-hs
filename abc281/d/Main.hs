...
main = do
  ...
  xs <- VU.map (`divMod` d) <$> getLineIntVec

  let add2 = \(a, b) (a', b') -> (a + a', b + b')
  let list = map (foldl1' add2 . map (xs VU.!)) $ combinations k [0 .. (n -1)]
  let list' = map (\(di, m) -> d * di + m) $ filter (\(_, m) -> m `rem` d == 0) list

  if null list'
    then putStrLn "-1"
    else print $ maximum list'
