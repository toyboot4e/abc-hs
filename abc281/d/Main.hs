...
-- TODO: consider using `unsafeFreeze` with `tabulate`
...
main :: IO ()
main = do
  [nInput, k, d] <- getLineIntList
  !input <- getLineIntVec

  let !result = VU.foldl' step s0 input
      -- index bounds_ (remainder, nUsed)
      !bounds_ = ((0, 0), (pred d, k))
      !s0 = VU.generate (rangeSize bounds_) (\case 0 -> 0 ; _ -> -1 :: Int)
      step !xs !x =
        let -- !_ = traceShow (bounds_, x, xs) ()
          in VU.generate (rangeSize bounds_) (\flat -> f (flat `divMod` succ k))
        where
          r = x `rem` d
          f :: (Int, Int) -> Int
          f (0, 0) = 0
          f (_, 0) = (-1)
          f (r', n) =
            let vKeep = xs VU.! index bounds_ (r', n)
                vInc = case xs VU.! index bounds_ ((r' + d - r) `rem` d, pred n) of
                  (-1) -> (-1)
                  lastX -> lastX + x
                -- !_ = traceShow (x, (r', n), vKeep, vInc) ()
             in max vKeep vInc

  -- let !_ = traceShow (result, index bounds_ (0, k)) ()
  print $ result VU.! index bounds_ (0, k)
