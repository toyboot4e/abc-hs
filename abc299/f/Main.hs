...
main = do
  !s <- getLine
  let !len = length s
  let !rhash = newRHash @MyModulus s

  let count d = modInt . length . filter (> 1) $ IM.elems im
        where
          !spans = filter (\(!l, !r) -> r < len) $ map (\l -> (l, l + d - 1)) [0 .. pred len]
          !_ = dbg (spans)
          !slices = map (\(!l, !r) -> hashValue $ sliceRHash rhash l r) spans
          !im = IM.fromListWith (+) $ map (,1 :: Int) slices
          !_ = dbg (d, im, filter (> 1) $ IM.elems im)

  let !result = sum $ map count [1 .. pred len]
  print result

  -- let !spans = [(l, r) | l <- [0 .. pred len], r <- [succ l .. pred len], (r - l) `rem` 2 == 0]
  -- let !result = foldl' step s0 spans
  --     s0 = IS.empty
  --     step :: IS.IntSet -> (Int, Int) -> IS.IntSet
  --     step !acc (!l, !r)
  --       | not $ inRange (0, pred len) r' = acc
  --       | otherwise =
  --           let !s1 = sliceRHash rhash l r
  --               !s2 = sliceRHash rhash l' r'
  --            in if s1 == s2
  --                 then
  --                   let !_ = traceShow ("same", (l, r), (l', r')) ()
  --                    in IS.insert (hashValue s1) acc
  --                 else acc
  --       where
  --         !d = succ $ r - l
  --         (!l', !r') = (l + d, r + d)
  --
  -- print $ (`rem` 998244353) $ IS.size result
