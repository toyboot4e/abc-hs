...
main = do
  [n] <- getLineIntList
  !rle <- map (\cs@(c:_) -> (c, length cs)) . group <$> getLine :: IO [(Char, Int)]
  let !_ = dbg (rle)

  let (!anyMinus, !maxO) = foldl' step s0 rle
      -- (any `-`, maximum `o` length)
      s0 :: (Bool, Int)
      s0 = (False, 0 :: Int)
      step :: (Bool, Int) -> (Char, Int) -> (Bool, Int)
      step (_, acc) ('-', _) = (True, acc)
      step (b, acc) ('o', len) = (b, max acc len)

  print $ if anyMinus && maxO > 0
    then maxO
    else (-1)
