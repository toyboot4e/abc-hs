...
-- TODO: consider using Vector instead.
...
-- pure solution + byte string builder + Vector-based mutable BFS
main :: IO ()
main = do
  [nVerts, nEdges] <- getLineIntList

  !edges <- concatMap (\[a, b] -> [(a, b), (b, a)]) <$> replicateM nEdges (map pred <$> getLineIntList)

  let !graph = accumArray @Array (flip (:)) [] (0, pred nVerts) edges
  let !lens = V.map (bfsVec graph) $ vRange 0 (pred nVerts)
  let !_ = dbg (lens)

  [!nQueries] <- getLineIntList
  !queries <- VU.replicateM nQueries (first pred <$> getTuple2)

  let (!constraints, !candidates) = VU.foldl' step (IS.empty, IS.empty) queries
      step acc (!v1, !k) = VU.foldl' f acc $ vRange 0 (pred nVerts)
        where
          f (!ws, !bs) v2 = case compare len k of
            LT -> (IS.insert v2 ws, bs)
            EQ -> (ws, IS.insert v2 bs)
            GT -> (ws, bs)
            where
              !len = lens V.! v1 VU.! v2

  let !candidates' = IS.difference candidates constraints

  let !isOk = VU.all check queries
        where
          check (!v1, !k) =
            let !v2s = VU.filter (\v2 -> lens V.! v1 VU.! v2 == k) $ vRange 0 (pred nVerts)
             in VU.any (`IS.member` candidates') v2s

  if not isOk
    then do
      putLnBSB $ BSB.string7 "No"
    else do
      putLnBSB $ BSB.string7 "Yes"
      let !result = V.map (\v -> if IS.member v constraints then 0 else 1 :: Int) $ vRange 0 (pred nVerts)
      let !s = V.foldl' (<>) (BSB.string7 "") $ V.map showBSB result
      putBSB s
