...
main = do
  ...
  let !crs = zip cs rs
  let merge (!r1, !i1) (!r2, !i2)
        | r1 >= r2 = (r1, i1)
        | otherwise = (r2, i2)

  let !input = map (\(!i, (!c, !r)) -> (c, (r, i))) $ zip [0 ..] $ crs :: [(Int, (Int, Int))]
  let !counts = IM.fromListWith merge input :: IM.IntMap (Int, Int)

  print . succ $ case IM.lookup t counts of
    Just (!_, !i) -> i
    Nothing ->
      let c = head cs
       in snd $ fromJust $ IM.lookup c counts
