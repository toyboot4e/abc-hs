...
eval w (!nx, !cx, !ny, !cy)
  | not xIsMoreEfficient = eval w (ny, cy, nx, cx)
  | otherwise =
      minimum
        [ cost
          | let nxMax = (w + (nx - 1)) `div` nx,
            numX <- [max 0 (nxMax - 100) .. nxMax],
            let numY = (max 0 (w - numX * nx) + ny - 1) `div` ny,
            let cost = numX * cx + numY * cy
        ]
  where
    xIsMoreEfficient = nx * cy >= ny * cx

solve :: StateT BS.ByteString IO ()
solve = do
  (!n, !x) <- ints2'
  !input <- U.replicateM n ints4'
  let !res = fromJust . bisectL 0 (10 ^ 10 :: Int) $ \w -> (<= x) . U.sum $ U.map (eval w) input
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc374/tasks/abc374_e
main :: IO ()
main = runIO solve
