...
solve = do
  ...
  let !s = U.sum xs
  let candidates = U.generate (bit n) id
  let eval bits =
        let is = U.findIndices (testBit bits) $ U.generate n id
            sum1 = U.sum $ U.backpermute xs is
            sum2 = s - sum1
         in max sum1 sum2

  printBSB . U.minimum $ U.map eval candidates

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc374/tasks/abc374_c
main :: IO ()
main = runIO solve
