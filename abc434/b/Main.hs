...
solve = do
  (!n, !m) <- ints2'
  !kbs <- U.replicateM n ((,) <$> int1' <*> int')

  let sums = U.accumulate (+) (U.replicate m (0 :: Int)) kbs
  let counts = U.accumulate (+) (U.replicate m (0 :: Int)) $ U.map (\(!k, !_) -> (k, 1)) kbs

  let div2 x d = intToDouble x / intToDouble d
  printBSB . unlinesBSB $ U.zipWith div2 sums counts

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc434/tasks/abc434_b
main :: IO ()
main = runIO solve
