...
solve = do
  ...
  let !imos = imos2d d d $ U.map (\(!a, !b, !c, !d) -> (a, b, c, d, 1 :: Int)) udlrs
  let !csum01 = csum2D $ mapIV (bool (0 :: Int) 1 . (== 1)) imos

  let !res0 = U.length . U.filter (== 0) $ vecIV imos
  let !res = U.map (\(!y1, !y2, !x1, !x2) -> res0 + csum01 @+! ((y1, x1), (y2, x2))) udlrs
  printBSB $ unlinesBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc434/tasks/abc434_d
main :: IO ()
main = runIO solve
