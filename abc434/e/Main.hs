...
filter1 :: U.Vector (Int, Int) -> U.Vector (Int, Int)
filter1 xrs =
  let im :: IM.IntMap (Int, Int)
      !im =
        IM.fromListWith (\(!n1, !i1) (!n2, !_) -> (n1 + n2, i1))
          . U.toList
          . U.concatMap (\(!i :: Int, (!x, !r)) -> U.fromListN 2 [(x - r, (1, i)), (x + r, (1, i))])
          $ U.indexed xrs
      !is = IS.fromList . map snd . filter ((== 1) . fst) $ IM.elems im
      -- TODO: (U.//)
   in U.ifilter (const . (`IS.notMember` is)) xrs

solve :: StateT BS.ByteString IO ()
solve = do
  !n <- int'
  !xrs0 <- U.replicateM n ints2'

  let res :: U.Vector (Int, Int) -> U.Vector (Int, Int)
      res !xrs
        | dx == 0 = xrs'
        | otherwise = res xrs'
        where
          xrs' = filter1 xrs
          dx = U.length xrs - U.length xrs'

  let rest = dbgId $ res xrs0
  let nResolved = U.length xrs0 - U.length rest
  printBSB $ nResolved + U.length rest `div` 2

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc434/tasks/abc434_e
main :: IO ()
main = runIO solve
