...
main = do
  ...
  !queries <- replicateM nQueries getLineIntList

  let !modulus = 998244353 :: Int
  let powerCache = powerModCache 10 modulus

  let (!result, !_, !_, !_) = foldl' step s0 queries
      s0 = ([], Seq.singleton 1, 1, modInt 1)
      step (!res, !queue, !nDigits, !acc) [1, !x] = (res, queue Seq.|> x, succ nDigits, 10 * acc + modInt x)
      step (!res, !q Seq.:<| queue, !nDigits, !acc) [2] = (res, queue, pred nDigits, acc - modInt q * modInt (powerByCache (pred nDigits) powerCache))
      step (!res, !queue, !nDigits, !acc) [3] = (acc : res, queue, nDigits, acc)
      step _ _ = error "unreachable"

  forM_ (reverse result) print
