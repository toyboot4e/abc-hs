...
eqMat matA matB =
  flip all (range $ bounds matA) $ \i -> not (matA ! i) || matB ! i

main :: IO ()
main = do
  [n] <- getLineIntList
  !matA <- listArray @UArray ((0, 0), (pred n, pred n)) . concat <$> replicateM n (map (== 1) <$> getLineIntList)
  !matB <- listArray @UArray ((0, 0), (pred n, pred n)) . concat <$> replicateM n (map (== 1) <$> getLineIntList)

  let rotMat m = ixmap (bounds m) (\(!i, !j) -> (pred n - j, i)) m

  let matAs = [matA, rotMat matA, rotMat (rotMat matA), rotMat (rotMat (rotMat matA))]
  let result = any (`eqMat` matB) matAs
