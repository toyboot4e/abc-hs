...
bounds_ :: ((Int, Int), (Int, Int))
bounds_ = ((0, 0), (1_000_000_000, 1_000_000_000))
...
main = do
  [nVerts] <- ints
  !input <- replicateM nVerts ints3

  -- TODO: consider using index compression instead of sparse data structure

  let !grid = IM.fromListWith (+) $ map (\(!r, !c, !x) -> (ix (r, c), x)) input
  let rowSums :: IM.IntMap Int
      !rowSums = IM.fromListWith (+) $ map (\(!r, !_, !x) -> (r, x)) input

  -- (column, columnSum) with descending order about the sum
  let colSums :: [(Int, Int)]
      !colSums = sortBy (comparing (Down . snd)) . IM.toList $ IM.fromListWith (+) $ map (\(!_, !c, !x) -> (c, x)) input

  let !sums = map (f colSums) $ IM.toList rowSums
        where
          f :: [(Int, Int)] -> (Int, Int) -> Int
          f [] _ = 0
          f ((!col, !colSum) : cs) (!row, !rowSum) = case IM.lookup (ix (row, col)) grid of
            Just s -> (rowSum + colSum - s) `max` f cs (row, rowSum)
            Nothing -> rowSum + colSum
