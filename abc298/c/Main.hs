...
main = do
  ...
  let !nnn = 200001
  !boxToCount <- VM.replicate nnn emptyMS
  !xToBox <- VM.replicate nnn IS.empty

  let m :: [Int] -> IO ()
      m [1, x, iBox] = do
        -- insert to box
        VM.modify boxToCount (incrementMS x) iBox
        -- count x->box map
        let !_ = dbg ("1", iBox, x)
        VM.modify xToBox (IS.insert iBox) x
      m [2, iBox] = do
        -- print box content
        !ms <- VM.read boxToCount iBox
        let !xs = concatMap (\(!x, !n) -> replicate n x) $ IM.toList $ innerMS ms
        let !_ = dbg ("2", iBox, ms)
        putStrLn $ unwords $ map show xs
      m [3, x] = do
        -- print beloging boxes
        !set <- VM.read xToBox x
        putStrLn $ unwords . map show $ IS.toList set
      m _ = error "unreachable"
