...
-- FIXME: HLS crashes with `derivingUnbox` :(
derivingUnbox "ModInt" [t|forall p. ModInt p -> Int|] [|\(ModInt !x) -> x|] [|\ !x -> ModInt x|]
derivingUnbox "MUFNode" [t|MUFNode -> (Bool, Int)|] [|\case (MUFChild !x) -> (True, x); (MUFRoot !x) -> (False, x)|] [|\case (True, !x) -> MUFChild x; (False, !x) -> MUFRoot x|]
derivingUnbox "RNEdge" [t|RNEdge -> (G.Vertex, Int, Int)|] [|\(RNEdge !x1 !x2 !x3) -> (x1, x2, x3)|] [|\(!x1, !x2, !x3) -> RNEdge x1 x2 x3|]

solve :: Int -> Int -> V.Vector (VU.Vector MyModInt)
solve n p = V.scanl' f vec0 $ V.replicate n (False)
  where
    vec0 :: VU.Vector MyModInt
    !vec0 = VU.generate (succ n) $ \case 0 -> modInt 1; _ -> modInt 0
    f :: VU.Vector MyModInt -> a -> VU.Vector MyModInt
    f !vec !_ = VU.generate (succ n) g
      where
        !_ = dbg ("wip", vec)
        csums :: VU.Vector MyModInt
        !csums = VU.scanl' (+) (modInt 0) vec
        g 0 = modInt 0
        g !i | i < n = (csums VU.! i - csums VU.! (max 0 (i - p))) / modInt p
        g !i
          | i == n =
            -- do not derive!:
            -- (+ vec VU.! n) .
            VU.sum $ VU.map (\len -> vec VU.! (i - len) * modInt (succ p - len) / modInt p) (vRange 1 (min n p))
        g _ = error "unreachable"

main :: IO ()
main = do
  [n, a, b, p, q] <- getLineIntList

  -- i times -> probability to reach n
  let !xs = V.map VU.last $ solve (n - a) p
  let !ys = V.map VU.last $ solve (n - b) q

  let !ysSum = V.scanl' (+) (modInt 0) ys

  let !result = V.sum $
        flip V.imap xs $ \i x ->
          case ysSum V.!? i of
            Just subY -> x * (V.last ysSum - subY)
            Nothing -> modInt 0
