...
solve = do
  ...
  seg <- Seg.build $ U.replicate n (Min (0 :: Int))

  -- x -> height
  cur <- UM.replicate n (0 :: Int)
  -- freq -> cnt
  freq <- Seg.build $ U.generate (q + 1) $ \case 0 -> (Sum n) ; _ -> (Sum (0 :: Int))

  U.foldM'_
    (\h (!query, !x) -> case query of
        1 -> do
          curH <- GM.read cur x
          GM.write cur x $ curH + 1
          Seg.modify freq (subtract 1) curH
          Seg.modify freq (+ 1) $ curH + 1

          Seg.modify seg (+ 1) x
          Min minH <- Seg.allProd seg
          pure $ bool h (h + 1) (minH > h)
        2 -> do
          let y = x + h
          printBSB . getSum =<< Seg.prod freq (min (q + 1) y) (q + 1)
          pure h
        _ -> error "unreachable"
    )
    (0 :: Int)
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc459/tasks/abc459_c
main :: IO ()
main = runIO solve
