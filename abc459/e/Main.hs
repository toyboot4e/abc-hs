...
nPr :: Int -> Int -> MI.ModInt 998244353
nPr n r = inner 0 (modInt 1) (modInt 1)
  where
    k = min r (n - r)
    inner i up down
      | i == k = up / down
      | otherwise = inner (i + 1) (up * modInt (n - i)) (down * modInt (i + 1))
...
solve = do
  ...
  let es = U.imap (\i p -> (p - 1, i + 1)) ps
  let dirTree = Gr.build' n es

  let facts = U.map modInt $ factModsN 998244353 (10 ^ 6 + 1)
  ...
  let nCr n r
         ...
         | r < 0 = modInt 0
         | r > n = modInt 0
         | n <= 10 ^ 6 = facts G.! n / (facts G.! r * facts G.! (n - r))
         -- cnt can be really large
  ...
  let verts = U.reverse $ Gr.topSort n (Gr.adj dirTree)
  cnts <- U.thaw ws
  res <-
    U.foldM'
      (\acc v -> do
        cnt0 <- (U.sum <$>) $ U.forM (Gr.adj dirTree v) (GM.read cnts)
        cnt <- (+ cnt0) <$> GM.read cnts v

        let d = qs G.! v
        let dRes = nCr cnt d
        GM.write cnts v $ max 0 (cnt - d)
        pure $ acc * dRes
      )
      (modInt 1)
      verts
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc459/tasks/abc459_e
main :: IO ()
main = runIO solve
