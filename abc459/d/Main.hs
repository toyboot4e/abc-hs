...
solve = do
  !s <- U.fromList . BS.unpack <$> lineP
  let n = U.length s

  let freq = U.accumulate (+) (U.replicate 26 (0 :: Int)) $ U.map ((, 1) . subtract (ord 'a') . ord) s

  -- (i, cnt)
  let byFreq = U.modify (VAI.sortBy (comparing (Down . snd))) $ U.indexed freq

  let indices = dbgId $ evens U.++ odds
        where
          evens = U.generate ((n + 1) `div` 2) (* 2)
          odds = U.generate (n `div` 2) ((+ 1) . (* 2))

  let byFreq' = snd $ EV.mapAccumL (\acc (!i, !cnt) -> (acc + cnt, (i, cnt, acc))) (0 :: Int) byFreq
  let res = U.create $ do
        vec <- UM.replicate n '-'
        U.forM_ byFreq' $ \ (!c, !cnt, !offset) -> do
          for_ [offset .. offset + cnt - 1] $ \i -> do
            GM.write vec (indices G.! i) $ chr (c + ord 'a')
        pure vec

  let !_ = dbg (s, res)
  let ok = (== n) . length $ U.group res
  if ok
    then do
      printBSB "Yes"
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc459/tasks/abc459_d
main :: IO ()
main = runIO $ do
  t <- intP
  replicateM_ t solve
