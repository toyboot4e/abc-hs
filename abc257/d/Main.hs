...
solve !nVerts !minCost !input = all ((< nVerts) . count) [0 .. pred nVerts]
  where
    !edges =
      map (\(!_, !_, !v1, !v2) -> (v1, v2)) $
        filter (\(!p, !d, !_, !_) -> p * minCost >= d) input
    !graph = accumArray (flip (:)) [] (0, pred nVerts) edges
    count !v0 = IM.size $ bfsVerts graph v0

main :: IO ()
main = do
  [n] <- getLineIntList
  !input <- zip [0 ..] <$> replicateM n getTuple3

  let !edges =
        concat $
          [ [(p1, d, i1, i2), (p2, d, i2, i1)]
            | (!i1, (!x1, !y1, !p1)) <- input,
              (!i2, (!x2, !y2, !p2)) <- input,
              i1 /= i2,
              let d = abs (x1 - x2) + abs (y1 - y2)
          ]

  let !result = fromJust . snd $
        bsearch (1, 4_000_000_000) $
          \minCost -> solve n minCost edges
