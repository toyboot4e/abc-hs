...
main = do
  ...
  -- [(maxIndex, cost)]
  -- REMARK: `sort` is stable sort.
  let !cs = VU.fromList . map last . groupBy (\a b -> snd a == snd b) . sortBy (comparing snd) $ zip [(1 :: Int) .. 9] cs_

  let (!minCostIndex, !minCost) = VU.head cs

  let !_ = dbg ("input", cs_)
  let !_ = dbg ("filtered", cs)

  -- [(index, deltaCost)]
  let !cs' = VU.modify (VAI.sortBy (comparing (Down . fst))) $ VU.map (second (subtract minCost)) $ VU.filter ((>= minCostIndex) . fst) cs
  let (!nDigits, !restCost0) = n `divMod` minCost

  let !_ = dbg (nDigits, restCost0)
  let !_ = dbg ("deltas", cs')

  let !result = snd $ mapAccumL (\acc _ -> f acc) restCost0 [1 .. nDigits]
        where
          f restCost =
            let (!i, !cost) = fromJust $ VU.find ((<= restCost) . snd) cs'
             in (restCost - cost, i)

  putBSB . mconcat $ map showBSB result
