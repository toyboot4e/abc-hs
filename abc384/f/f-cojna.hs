...
solve = do
  ...
  -- TODO: 2 * 変域 + 1
  let !m = 2 * (2 * 10 ^ 5) + 1 :: Int
  let !ps = U.accumulate (+) (U.replicate m (0 :: Int)) $ U.map (,1) xs
  ...
  let !res' = U.sum $ U.imap f res
        where
          f i nx
            ...
            | nx == 0 = 0
            | otherwise =
                -- let !_ = dbg (i, j, nx) in div2 (i + j) * nx
                let !_ = dbg (i, nx) in div2 i * nx
  -- where
  --   i = xx `mod` n
  --   j = xx `div` n
  printBSB $ (res' + U.sum (U.map (div2 . (* 2)) xs)) `div` 2

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc384/tasks/abc384_f
main :: IO ()
main = runIO solve
