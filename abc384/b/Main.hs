...
solve = do
  let divs = [(1600, 2800 - 1), (1200, 2400 - 1)]
  (!n, !r) <- ints2'
  !ddxs <- U.replicateM n ints2'

  let !res = U.foldl' step r ddxs
        where
          step rate (!d, !dx)
            | inRange bounds rate = rate + dx
            | otherwise = rate
            where
              bounds = divs !! pred d
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc384/tasks/abc384_b
main :: IO ()
main = runIO solve
