...
solve = do
  !n <- int'
  -- REMARK: The graph is given as gr @! (x, y, z), not gr @! (z, y, x).
  !gr <- IxVector (zero3 n n n) <$> U.replicateM (n * n * n) int'
  !q <- int'
  !qs <- U.replicateM q ints6'

  let !csum = dbgId $ csum3D n gr
  let !res = (`U.map` qs) $ \(pred -> !lx, !rx, pred -> !ly, !ry, pred -> !lz, !rz) ->
        dbgId (csum @! (rx, ry, rz))
          - dbgId
            ( csum @! (rx, ry, lz)
                + csum @! (rx, ly, rz)
          ...
          + dbgId
            ( (csum @! (rx, ly, lz) - csum @! (lx, ly, lz))
                + (csum @! (lx, ry, lz) - csum @! (lx, ly, lz))
                + (csum @! (lx, ly, rz) - csum @! (lx, ly, lz))
            )
          + dbgId (2 * csum @! (lx, ly, lz))

  printBSB $ unlinesBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc366/tasks/abc366_d
main :: IO ()
main = runIO solve
