...
solve = do
  !n <- int'
  !xs <- reverse <$> replicateM n (BS.unpack <$> line')
  let !len = maximum $ map length xs
  let !xs' = map (\line -> line ++ replicate (len - length line) '*') xs
  let !res = map (reverse . dropWhile (== '*') . reverse) $ transpose xs'
  -- printBSB $ length res
  forM_ res printBSB

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc366/tasks/abc366_b
main :: IO ()
main = runIO solve
