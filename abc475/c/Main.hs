...
solve = do
  ...
  let ps = dbgId $ U.scanl' (+) (0 :: Int) ds
  let res = U.maximum $ U.generate n f
        where
          -- s -> i -> opposite side
          f i
            | s == i = 1 :: Int
            | di > l = 1 :: Int
            | otherwise = nMin + turnBack
            where
              xs = ps G.! s
              xi = ps G.! i
              di = abs (xs - xi)
              nMin = abs (s - i) + 1
              rest = l - di * 2
              !_ = dbg (i, di < l, (xs, xi, di), nMin, rest, nMin + turnBack)
              turnBack
                | rest <= 0 = 0
                | i < s =
                  let ds' = U.drop s ds
                      ds'' = U.scanl1' (+) ds'
                   in U.length $ U.takeWhile (<= rest) ds''
                | otherwise =
                  let ds' = U.reverse $ U.take s ds
                      ds'' = U.scanl1' (+) ds'
                   in U.length $ U.takeWhile (<= rest) ds''

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc475/tasks/abc475_c
main :: IO ()
main = runIO solve
