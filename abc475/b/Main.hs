...
solve = do
  ...
  let (!a, !b, !c) = U.foldl' step s0 xs
        where
          s0 = (0, 0, 0) :: (Int, Int, Int)
          step (!n100, !n10, !n1) x = (n100 + dn100, n10 + dn10, n1 + dn1)
            where
              r = (1000 - x `mod` 1000) `mod` 1000
              dn100 = r `div` 100
              dn10 = r `mod` 100 `div` 10
              dn1 = r `mod` 10 `div` 1
              !_ = dbg (r, dn100, dn10, dn1)

  printBSB (c, b, a)

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc475/tasks/abc475_b
main :: IO ()
main = runIO solve
