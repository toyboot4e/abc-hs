...
solve = do
  ...
  let !_ = dbg s
  let ps = U.dropWhile (< (10 ^ (l - 1) :: Int)) $ EM.primes (10 ^ l - 1 :: Int)

  let check :: BS.ByteString -> Int -> Bool
      check s p = runST $ do
        numToChar <- UM.replicate 10 (-1 :: Int)
        charToNum <- UM.replicate 26 (-1 :: Int)
        b <- UM.replicate 1 True
        for_ [0 .. l - 1] $ \i -> do
          let ic = ord (BS.index s i) - ord 'a'
          let pi =
                let unit = 10 ^ (l - 1 - i) :: Int
                 in p `mod` (10 * unit) `div` unit
          let !_ = dbg (i, (s, ic), (p, pi))

          oldC <- GM.exchange numToChar pi ic
          when (oldC /= -1 && oldC /= ic) $ do
            GM.write b 0 False

          oldI <- GM.exchange charToNum ic pi
          when (oldI /= -1 && oldI /= pi) $ do
            GM.write b 0 False
        GM.read b 0

  printBSB . fromMaybe (-1) $ U.find (check s) ps

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc475/tasks/abc475_d
main :: IO ()
main = runIO solve
