...
-- 10000..
tens :: VU.Vector MyModInt
tens = VU.iterateN 200_001 (\x -> modInt 10 * x) $ modInt 1

-- 11111..
ones :: VU.Vector MyModInt
ones = VU.iterateN 200_001 (\x -> x * (modInt 10) + modInt 1) $ modInt 1

--------------------------------------------------------------------------------
-- Acc
--------------------------------------------------------------------------------

-- DigitSum gen acc nDigit
data DigitSum = DigitSum MyModInt Int
  deriving (Show, Eq)

derivingUnbox
  "DigitSum"
  [t|DigitSum -> (MyModInt, Int)|]
  [|\(DigitSum x1 x2) -> (x1, x2)|]
  [|\(!x1, !x2) -> DigitSum x1 x2|]

instance Semigroup DigitSum where
  (DigitSum !x1 !d1) <> (DigitSum !x2 !d2) =
    DigitSum (x1 * (tens VU.! d2) + x2) (d1 + d2)

instance Monoid DigitSum where
  mempty = DigitSum (modInt 0) 0
...
instance Semigroup SetDigits where
  -- REMARK: The propagated operator always comes from the right.
  (<>) = flip const

instance Monoid SetDigits where
  mempty = SetDigits (-1)

instance SemigroupAction SetDigits DigitSum where
  sact (SetDigits (-1)) s = s
  sact (SetDigits !x) s@(DigitSum !_ !nDigits) =
    -- REMARK: use `pred nDigits` because `ones VU.! i` returns `i + 1` digits
    DigitSum (modInt x * (ones VU.! pred nDigits)) nDigits

instance MonoidAction SetDigits DigitSum

main :: IO ()
main = do
  [nDigits, nQueries] <- ints
  !queries <- VU.replicateM nQueries ints3

  !stree <- generateLazySTree nDigits (\_ -> DigitSum 1 1)

  VU.forM_ queries $ \(!l_, !r_, !x) -> do
    let (!l, !r) = (pred l_, pred r_)
    updateLazySTree stree l r $ SetDigits x
    !xSum <- (\(DigitSum xx _) -> xx) <$> queryLazySTree stree 0 (pred nDigits)

    -- -- --------------------------------------------------------------------------------
    -- -- !xSum0 <- (\(DigitSum xx _) -> xx) <$> queryLazySTree stree 0 (pred nDigits)
    -- -- let !_ = dbg (nDigits, nQueries, xSum0)

    let LazySegmentTree as ops _ = stree
    !as_ <- VU.unsafeFreeze as
    !ops_ <- VU.unsafeFreeze ops
    let !_ = dbg (as_)
    let !_ = dbg (ops_)
