...
- [ ] Graph
  ...
  - [ ] better BFS
  - [x] topological sort
  - [x] minimum spanning tree template
  - [x] maximum flow / biparate matching
  - [ ] minimum cut
...
main = do
  ...
  !input <- concatMap (\[a, b] -> [(a, b), (b, a)]) <$> replicateM nEdges getLineIntList

  let !graph = accumArray @Array (flip (:)) [] (1, nVerts) input
  let (_, !infos, anyFailure) = foldl' step s0 [1 .. nVerts]
      s0 = (IM.empty, [], False)
      step (_, _, True) _ = (IM.empty, [], True)
      step (colors, acc, False) v
        | v `IM.member` colors = (colors, acc, False)
        | otherwise = case colorize graph colors v of
          (colors', Just a) -> (colors', a : acc, False)
          (colors', Nothing) -> (colors', acc, True)

  -- let !_ = traceShow (infos) ()
  -- let !_ = traceShow (anyFailure) ()

  print
    if anyFailure
      then (0 :: Int)
      else countEdges nVerts graph infos

countEdges :: Int -> Graph -> [ColorInfo] -> Int
countEdges nVerts graph infos =
  -- Returns (nVerts, nPossibleEdgeAddition)
  let countSelf :: ColorInfo -> (Int, Int)
      countSelf (red, green) =
        let n1 = length red
            n2 = length green
            count = sum . map (length . (graph !))
            nEdges_ = (`div` 2) $ count red + count green
         in (n1 + n2, n1 * n2 - nEdges_)

      (accNVerts, accNEdges) = unzip $ map countSelf infos
      -- Number of edges between self vertex and self vertex
      nSelf = sum accNEdges
      -- Number of edges between self vertex and other vertex
      nOther = case accNVerts of
        [] -> 0
        [_] -> 0
        _ -> (`div` 2) . sum $ map (\n -> n * (nVerts - n)) accNVerts
      -- !_ = traceShow ((accNVerts, accNEdges), nSelf, nOther) ()
   in nSelf + nOther
