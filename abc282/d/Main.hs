...
main = do
  ...
  input <- concat <$> replicateM nEdges ((\[a, b] -> [(a, b), (b, a)]) <$> getLineIntList)
  let graph = accumArray (flip (:)) [] (1, nVerts) input :: Array Int [Int]

  let step1 :: (Bool, IM.IntMap Int, (Int, Int)) -> Int -> (Bool, IM.IntMap Int, (Int, Int))
      step1 (isFail, visits, n@(nOdd, nEven)) x =
        case IM.lookup x visits of
          Just n' ->
            let isFail' = n' == (nOdd + nEven) `rem` 2
             in (isFail', visits, n)
          Nothing ->
            let visits' = IM.insert x (x `rem` 2) visits
                n' = if even x then (nOdd, nEven + 1) else (nOdd + 1, nEven)
             in (isFail, visits', n')

  let dfs :: (Bool, IM.IntMap Int, (Int, Int)) -> Int -> (Bool, IM.IntMap Int, (Int, Int))
      dfs s@(_, visits, _) x =
        let s'@(_, visits', _) = step1 s x
         in if IM.member x visits
              then s'
              else foldl' dfs s' $ filter (not . flip IM.member visits') $ graph ! x

  let collectGraph :: (IM.IntMap Int, [(Int, Int)]) -> Int -> (IM.IntMap Int, [(Int, Int)])
      collectGraph (visits, ns) x =
        case IM.lookup x visits of
          Just _ -> (visits, ns)
          Nothing ->
            let (isFail, visits', n) = dfs (False, visits, (0, 0)) x
             in if isFail
                  then (visits', ns)
                  else (visits', n : ns)

  let input' = map fst input
  let (_, ns') = foldl' collectGraph (IM.empty, []) input'
  let ns = VU.fromList ns'

  let countSingle :: Int -> Int
      countSingle 0 = 0
      countSingle 1 = 0
      countSingle n = n * (n - 1) `div` 2

  let result = VU.sum $ VU.map (uncurry (+) . bimap countSingle countSingle) ns
  let result' =
        if VU.length ns < 2
          then 0
          else
            sum
              [ uncurry (+) (ns VU.! i) * uncurry (+) (ns VU.! j)
                | [i, j] <- combinations 2 [0 .. (VU.length ns - 1)]
              ]
