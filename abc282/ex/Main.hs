...
main :: IO ()
main = do
  [n, bounds] <- getLineIntList
  as <- getLineIntVec
  bs <- getLineIntVec

  minTree <- newTree min n maxBound
  forM_ [0 .. (n - 1)] $ \i -> do
    updateLeaf minTree i (as VU.! i)

  sumTree <- newTree (+) n 0
  forM_ [0 .. (n - 1)] $ \i -> do
    updateLeaf sumTree i (bs VU.! i)

  xs <- forM [0 .. (n - 1)] $ \i -> do
    forM [i .. (n - 1)] $ \j -> do
      m <- queryByRange minTree (i, j)
      s <- queryByRange sumTree (i, j)
      return $ if m + s <= bounds then 1 else 0
