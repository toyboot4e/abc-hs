...
-- maxBounds :: ((Int, Int), (Int, Int))
-- maxBounds = ((-10_000, 10_000), (10_000, 10_000))
--
-- feat :: (Int, Int) -> Int
-- feat = index maxBounds

-- Kruscal method + maximum P
main :: IO ()
main = do
  [nVerts, nEdges, nPeople] <- ints
  !verts <- VU.fromList . ((0, 0) :) <$> replicateM nVerts (swap <$> ints2)
  !edges <- VU.modify (VAI.sortBy (comparing (thd3 . snd))) . VU.indexed <$> VU.replicateM nEdges ints3
  ...
  -- print powers
  putStrLn . unwords . map show $ replicate nVerts (5000 :: Int)

  -- print if edges are used
