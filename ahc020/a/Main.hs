...
-- feat (!v1, !v2) = (min v1 v2, max v1 v2)

inCircle :: (Int, Int) -> (Int, Int) -> Int -> Bool
inCircle (!x0, !y0) (!x, !y) !p =
  let !dx = x - x0
      !dy = y - y0
   in dx * dx + dy * dy <= p * p
...
main :: IO ()
main = do
  [nVerts, nEdges, nPeople] <- ints
  !verts <- VU.replicateM nVerts ints2
  !edges <- VU.modify (VAI.sortBy (comparing (thd3 . snd))) . VU.indexed <$> VU.replicateM nEdges ((\(!v1, !v2, !w) -> (pred v1, pred v2, w)) <$> ints3)
  !people <- VU.replicateM nPeople ints2

  -- 1. Find minimum P where every people is powered:
  let !eqP = 5000
  -- let !eqP = fromMaybe 5000 $ bsearchR (1, 5000) \p ->
  --       let !res = flip VU.all people $ \xy ->
  --             flip VU.any verts $ \xy0 -> inCircle xy0 xy p
  --           !_ = dbg ("bin", p, res)
  --        in not res
  ...
  -- 2. Decide powers
  !powers <- VUM.replicate nVerts eqP

  -- Turn off P where NO people is captured:
  !nRefs <- VUM.replicate nPeople (0 :: Int)
  VU.forM_ (VU.indexed verts) $ \(!i, xy0) -> do
    let !captured = VU.filter (\(!_, !xy) -> inCircle xy0 xy eqP) (VU.indexed people)
    if (VU.null captured)
      then do
        -- turn off
        VUM.write powers i 0
      else do
        -- increment reference counts
        VU.forM_ captured $ \(!iPerson, !_) -> do
          VUM.modify nRefs succ iPerson

  -- -- Turn off P where every captured person has multiple references:
  -- VU.forM_ (VU.indexed verts) $ \(!i, !xy0) -> do
  --   let !captured = VU.filter (\(!_, !xy) -> inCircle xy0 xy eqP) (VU.indexed people)
  --   !canRemove <- VU.all id <$> VU.mapM (\(!iPerson, !_) -> (1 <) <$> VUM.read nRefs iPerson) captured
  --   when canRemove $ do
  --     -- turn off the power and decrement reference counts
  --     VUM.write powers i 0
  --     VU.forM_ captured $ \(!iPerson, !_) -> do
  --       VUM.modify nRefs pred iPerson

  -- Reduce P using bsearch where all ref counts are more than one
  VU.forM_ (VU.indexed verts) $ \(!i, !xy0) -> do
    !p0 <- VUM.read powers i

    unless (p0 == 0) $ do
      !p' <- fmap (fromMaybe p0) . bsearchMR (0, p0) $ \p -> do
        let !deltaCapture = VU.filter (\(!_, !xy) -> inCircle xy0 xy p0 && (not (inCircle xy0 xy p))) (VU.indexed people)
        !refCounts <- VU.mapM (\(!iPerson, !_) -> VUM.read nRefs iPerson) deltaCapture
        return $ VU.any (<= 1) refCounts

      -- update ref counts
      when (p' < p0) $ do
        let !_ = dbg ("reducing", i, p0, "->", p')
        VUM.write powers i p'
        VU.forM_ (VU.indexed people) $ \(!iPerson, !xy) -> do
          when (inCircle xy0 xy p0 && not (inCircle xy0 xy p')) $ do
            VUM.modify nRefs pred iPerson

  -- 3. Create minimum spanning tree
  !powers' <- VU.unsafeFreeze powers

  !edgesUsed <- VUM.replicate nEdges False

  -- Create minimum spanning tree
  !uf <- newMUF nVerts
  !degs <- VUM.replicate nVerts (0 :: Int)
  VU.forM_ edges $ \(!iEdge, (!v1, !v2, !_w)) -> do
    !r1 <- rootMUF uf v1
    !r2 <- rootMUF uf v2
    when (r1 /= r2) $ do
      VUM.modify degs succ v1
      VUM.modify degs succ v2
      VUM.write edgesUsed iEdge True
      uniteMUF uf r1 r2

  -- Remove needless tail edges
  forMS_ (rangeMS 1 10) $ \_ -> do
    VU.forM_ edges $ \(!iEdge, (!v1, !v2, !_)) -> do
      !used <- VUM.read edgesUsed iEdge
      when used $ do
        !d1 <- VUM.read degs v1
        !d2 <- VUM.read degs v2

        let !unused1 = powers' VU.! v1 == 0
        let !unused2 = powers' VU.! v2 == 0

        let !p1 = unused1 && d1 == 1 && d2 > 1
        let !p2 = unused2 && d2 == 1 && d1 > 1

        when (p1 && (v1 /= 0) || p2 && (v2 /= 0)) $ do
          let !_ = dbg ("removing edge", iEdge, (v1, v2))
          VUM.write edgesUsed iEdge False

          VUM.modify degs pred v1
          VUM.modify degs pred v2

  -- print powers
  putStrLn . unwords . map show $ VU.toList powers'

  -- print edge uses
  !edgesUsed' <- VU.unsafeFreeze edgesUsed
  putStrLn . unwords . map show . VU.toList $ VU.map (\case True -> 1; False -> 0 :: Int) edgesUsed'
