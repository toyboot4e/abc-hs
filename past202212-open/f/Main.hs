...
main = do
  ...
  -- !aver' <- round . (1000.0 *) <$> readLn @Double :: IO Int
  !aver' <- (\s -> (digitToInt $ s !! 0) * 1000 + (digitToInt $ s !! 2) * 100 + (digitToInt $ s !! 3) * 10 + (digitToInt $ s !! 4) *  1) <$> getLine

  let !s = sum $ zipWith (*) [1, 2, 3, 4] ns :: Int
  let !up = 1000 * s - n * aver'
  let !down = aver' - 1000

  print $ if up <= down
    then 0
    else (up + down - 1) `div` down
