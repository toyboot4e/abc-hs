...
main :: IO ()
main = do
  [nPlayers, nCards] <- ints
  !input <- getLine

  let !result = VU.create $ do
        !vec <- VUM.replicate nPlayers (0 :: Int)

        foldForM (0 :: Int) (zip [(0 :: Int) ..] input) $ \acc (!iPlayer, query) -> do
          VUM.modify vec succ (iPlayer `mod` nPlayers)
          case query of
            '0' -> return acc
            '+' -> do
              VUM.modify vec (+ acc) (iPlayer `mod` nPlayers)
              return 0
            '-' -> do
              !x <- VUM.read vec (iPlayer `mod` nPlayers)
              VUM.write vec (iPlayer `mod` nPlayers) 0
              return $ acc + x
