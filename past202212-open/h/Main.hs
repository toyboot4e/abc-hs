...
main = do
  [nDigits] <- ints
  !input <- map ((, 1) . digitToInt) <$> getLine

  let !arr = accumArray @UArray (+) (0 :: Int) (0, 9) input
  let !_ = dbg (arr)

  let !result = (`div` 2) $ sum $ map f [0 .. 9]
        where
          f !i1 = safeSum $ map g [0 .. 9]
            where
              g i2 = abs (i2 - i1) * (arr ! i2) * (arr ! i1)
          safeSum [] = 0
          safeSum xs = sum xs
