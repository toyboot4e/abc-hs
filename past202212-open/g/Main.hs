...
main = do
  ...
  let !csums = VU.scanl' (+) 0 xs
  let !_ = dbg (xs)
  let !_ = dbg (csums)

  !stree <- newSTreeVU (<>) (succ n) (mempty :: Max Int)
  flip VU.imapM_ csums $ \i x -> do
    insertSTree stree i (Max x)

  let !results = VU.mapM f (rangeVU 0 (n - 1))
        where
          f i = do
            let !xl = csums VU.! i
            Max !xr <- fromJust <$> querySTree stree (succ i, n)
            let !_ = dbg (i, xl, xr)
            return $ xr - xl

  !res <- VU.maximum <$> results
  print res
