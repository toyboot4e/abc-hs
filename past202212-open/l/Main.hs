...
chunk2 :: [a] -> [(a, a)]
chunk2 [] = []
chunk2 (l:r:rest) = (l, r) : chunk2 rest

main :: IO ()
main = do
  [n] <- ints
  !input <- VG.unfoldr (BS.readInt . BS.dropWhile isSpace) <$> BS.getContents :: IO (VU.Vector Int)

  -- sort by 1. ascending r, 2. descending l
  let !input' = VU.modify (VAI.sortBy $ comparing (\(!l, !r) -> (r, Down l))) . VU.fromList . chunk2 . VU.toList . snd $ compressVU input
  let !_ = dbg (input')

  let size = 2 *n
  !stree <- newSTreeVU max size (0 :: Int)

  VU.forM_ input' $ \(!l, !r) -> do
    !xLast <- fromJust <$> querySTree stree (l, pred size)
    modifySTree stree (max (succ xLast)) l

  !res <- fromJust <$> querySTree stree (0, pred size)
  print res
