...
data Xor = Xor (UArray Int Int)
  deriving (Show, Eq, Ord)

instance Semigroup Xor where
  -- TODO: How to keep `nDigits` value??
  (Xor !arr1) <> (Xor !arr2) =
    let !n1 = rangeSize $ bounds arr1
        !n2 = rangeSize $ bounds arr2
     in case (n1, n2) of
      (1, _) -> Xor arr2
      (_, 1) -> Xor arr1
      _ -> Xor . listArray (bounds arr1) $ map (\i -> (arr1 ! i) `xor` (arr2 ! i)) [0 .. pred (rangeSize $ bounds arr1)]

instance Monoid Xor where
  -- Make sure they don't make change:
  mempty = Xor $ listArray (0, 0) [0]

--------------------------------------------------------------------------------
-- operator
--------------------------------------------------------------------------------

data Rotate = Rotate Int
  deriving (Show, Eq, Ord)
...
instance Semigroup Rotate where
  (Rotate !r1) <> (Rotate !r2) = Rotate $ r1 + r2

instance Monoid Rotate where
  -- `mempty` as operator:
  mempty = Rotate 0

instance SemigroupAction Rotate Xor where
  sact (Rotate !n) org@(Xor !arr) =
    let !size = rangeSize $ bounds arr
        !n' = n `mod` size
     in Xor $ ixmap (bounds arr) (\i -> (i + n') `mod` size) arr

instance MonoidAction Rotate Xor

spans :: Int -> Int -> Int -> Int -> [(Int, Int)]
spans n offset l r
  | offset + r < n = [(l, r)]
  | offset + l >= n = [((l + offset) `rem` n, (r + offset) `rem` n)]
  | otherwise = [(offset + l, pred n), (0, (r + offset) - n)]

main :: IO ()
main = do
  [!nInput, !nDigits] <- ints
  !xs0 <- V.fromList . map (listArray (0, pred nDigits) . map digitToInt) . words <$> getLine :: IO (V.Vector (UArray Int Int))

  !nQueries <- readLn @Int
  !queries <- replicateM nQueries ints

  !stree@(LazySegmentTree as ops _) <- generateLazySTreeV nInput (\i -> case xs0 V.!? i of Just x -> Xor x ; Nothing -> mempty)

  -- !xxx <- VU.unsafeFreeze as
  -- let !_ = dbg (xxx)

  let m = foldM step s0 queries
        where
          s0 = 0 :: Int

          step !offset [1, !x] =
            let !_ = dbg ("shift", offset, (offset + x) `rem` nInput)
             in return $ (offset + x) `rem` nInput

          step !offset [2, !l_, !r_, !y] = do
            let (!l, !r) = (pred l_, pred r_)
            let !lrs = spans nInput offset l r

            forM_ lrs $ \(!l', !r') -> do
              updateLazySTree stree l' r' (Rotate y)
            let !_ = dbg ("update", offset, (l, r), "->", lrs, y)

            return offset

          step !offset [3, !l_, !r_] = do
            let (!l, !r) = (pred l_, pred r_)
            let !lrs = spans nInput offset l r

            !xs <- forM lrs $ \(!l, !r) -> do
              queryLazySTree stree l r
            let !_ = dbg ("answer", offset, (l, r), "->", lrs, mconcat xs)

            print $ (\(Xor x) -> x) $ mconcat xs
            return offset
