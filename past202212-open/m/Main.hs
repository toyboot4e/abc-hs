...
solve :: Array Vertex [(Int, Vertex)] -> Int
solve !tree = fromZero + fromAfterZero
  where
    !nVerts = rangeSize $ bounds tree
    !nEdges = pred nVerts

    !res = VU.create $ do
      !vec <- VUM.replicate nVerts (maxBound, minBound)

      let m (!l, !r) !parent !v1 = do
            forM_ (tree ! v1) $ \(!iEdge, v2) -> do
              when (v2 /= parent) $ do
                let l' = min l iEdge
                let r' = max r iEdge
                VUM.write vec v2 (l', r')
                m (l', r') v1 v2

            return ()

      m (maxBound, minBound) (-1) 0
      return vec

    !res' = VU.map (\(!l, !r) -> succ l * (nEdges - r)) $ VU.tail res
    !_ = dbg (nVerts)
    !_ = dbg (res)
    !_ = dbg (res')

    !fromZero = nVerts * pred nVerts `div` 2
    !fromAfterZero = VU.sum res'

main :: IO ()
main = do
  [n] <- ints
  !edges <- concatMap (\(!i, (!v1, !v2)) -> [(v1, (i, v2)), (v2, (i, v1))]) . zip [0 :: Int ..] <$> replicateM (pred n) (both pred <$> ints2)

  let !tree = accumArray @Array (flip (:)) [] (0, pred n) edges
  print $ solve tree
