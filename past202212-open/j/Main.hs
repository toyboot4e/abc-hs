...
solve (!sx, !sy, !tx, !ty) (!p, !q, !r, !w) = if' intersects w 0
  where
    !intersects = cmp sx sy * cmp tx ty <= 0
    cmp x y = case compare (p * x + q * y) r of
      EQ -> 0
      LT -> -1
      GT -> 1
...
main = do
  [n, k] <- ints
  !st <- ints4
  !input <- VU.replicateM n ints4

  let !_ = dbg (st)
  let !_ = dbg (input)
  let !_ = dbg $ VU.map (solve st) input

  let !results = VU.map (solve st) input

  let !result = VU.sum . VU.take k $ VU.modify VAI.sort results
  print result
