...
main = do
  ...
  -- 0-based maximum digit
  -- TODO: fix 10^n with faster calculation?
  let !maxDigit = pred $ until (\n -> a * (10 ^ n) + b > x0) succ (0 :: Int)
  let !_ = dbg ("decide max digit", a, b, x0, maxDigit)

  -- OMG. We're given upper limit:
  when (maxDigit >= 9) $ do
    print 1_000_000_000
    exitSuccess

  let !result = snd $ mapAccumL step x0 [maxDigit, maxDigit - 1 .. 0]
        where
          step !rest !nthDigit =
            let !x = rest `div` (a * (10 ^ nthDigit) + b)
                !_ = dbg (nthDigit, rest, x, a * x * (10 ^ nthDigit) + b * x)
             in (rest - (a * x * (10 ^ nthDigit) + b * x), x)
