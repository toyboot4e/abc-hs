...
main :: IO ()
main = do
  [nRow, nCol] <- getLineIntList
  let bounds_ = ((0, 0), (pred nRow, pred nCol))
  mat <- listArray @UArray bounds_ . concat <$> replicateM nRow getLine

  let result = runSTUArray $ do
        !arr <- newArray bounds_ False

        forM_ (range bounds_) $ \(y, x) -> do
          let c = mat ! (y, x)
          -- let !_ = traceShow (y, x, c) ()
          when (c /= '#') do
            writeArray arr (y, x) True
          when (c /= '#' && c /= '.') $ do
            let maxR = read [c] :: Int
            forM_ [1 .. maxR] $ \r -> do
              forM_ [0 .. r] $ \dx -> do
                let dy = r - dx
                let yxs = filter (inRange bounds_) $ map (add2 (y, x)) [(dx, dy), (-dx, dy), (dx, -dy), (-dx, -dy)]
                forM_ yxs $ \ (!y1, !x1) -> do
                  writeArray arr (y1, x1) True

        return arr

  forM_ [0 .. pred nRow] $ \y -> do
    let xs = map ((\case True -> '.' ; False -> '#') . (\x -> result ! (y, x))) [0 .. pred nCol]
    putStrLn xs
