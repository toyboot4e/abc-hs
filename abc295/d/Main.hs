...
main :: IO ()
main = do
  !xs <- V.fromList . map (\c -> read @Int [c]) <$> getLine

  let csums = V.scanl' step IS.empty xs
      step is x
        | IS.member x is = IS.delete x is
        | otherwise = IS.insert x is

  let myHash is = sum $ map (10 ^) $ IS.toList is

  let acc = V.foldl' step2 IM.empty (V.map myHash csums)
      step2 im x = IM.insertWith (+) x 1 im

  -- let !_ = traceShow (csums) ()
  -- let !_ = traceShow (V.map myHash csums) ()
  -- let !_ = traceShow (acc) ()

  let result = sum $ map (\x -> (x * pred x) `div` 2) $ IM.elems acc
  print result
