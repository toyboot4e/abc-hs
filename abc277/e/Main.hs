...
add2 a b = (fst a + fst b, snd a + snd b)

sub2 :: (Int, Int) -> (Int, Int) -> (Int, Int)
sub2 a b = (fst a - fst b, snd a - snd b)
...
-- Super cool answer by @gotoki_no_joe, but edited for my style
solve :: Graph -> Graph -> IS.IntSet -> Int -> Int -> Maybe Int
solve graph0 graph1 switchVerts start goal =
  bfs 0 IS.empty IS.empty IS.empty (IS.singleton start)
  where
    bfs depth vis0 vis1 vs0 vs1
      | IS.null vs1 && IS.null vs0 = Nothing
      | IS.member goal vs1 = Just depth
      | IS.member goal vs0 = Just depth
      | otherwise = bfs (succ depth) vis0' vis1' next0 next1
      where
        vs0' = vs0 `IS.union` (vs1 `IS.intersection` switchVerts)
        vs1' = vs1 `IS.union` (vs0 `IS.intersection` switchVerts)

        vis0' = vis0 `IS.union` vs0'
        vis1' = vis1 `IS.union` vs1'

        next0 = IS.fromList $ filter (`IS.notMember` vis0') $ concatMap (graph0 !) (IS.elems vs0')
        next1 = IS.fromList $ filter (`IS.notMember` vis1') $ concatMap (graph1 !) (IS.elems vs1')

main :: IO ()
main = do
  [!nVerts, !nEdges, !nSwitchVerts] <- getLineIntList
  !input <- map tuple3 <$> replicateM nEdges getLineIntList
  !switchVerts <- IS.fromList <$> getLineIntList

  let !graph0 = accumArray @Array (flip (:)) [] (1, nVerts) $ do (a, b, 0) <- input; [(a, b), (b, a)]
  let !graph1 = accumArray @Array (flip (:)) [] (1, nVerts) $ do (a, b, 1) <- input; [(a, b), (b, a)]
  let result = fromMaybe (-1) $ solve graph0 graph1 switchVerts 1 nVerts
