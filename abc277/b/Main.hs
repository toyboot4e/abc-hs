...
main = do
  [n] <- getLineIntList
  xys <- sort <$> replicateM n getLine
  let (xs, ys) = unzip . map (\[a, b] -> (a, b)) $ xys
  putStrLn $ if solve n xys (compress xs) (compress ys) then "Yes" else "No"

solve :: Int -> [[Char]] -> [Char] -> [Char] -> Bool
solve n xys xs ys = q1 && q2 && q3
  where
    q1 = all (`elem` ['H', 'D', 'C', 'S']) xs
    q2 = all (`elem` ['A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K']) ys
    q3 = length (compress xys) == n
