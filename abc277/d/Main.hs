...
main :: IO ()
main = do
  [n, modulus] <- getLineIntList
  input <- getLineIntVec

  let sorted = VU.modify (VAI.sortBy (comparing fst)) $ VU.map (\x -> (x `rem` modulus, x)) input
  let grouped = HT.groupBy (\a b -> fst b - fst a <= 1) $ VU.toList sorted
  let sums = VU.fromList $ map (sum . map snd) grouped

  -- let !_ = traceShow (sorted) ()
  -- let !_ = traceShow (grouped) ()
  let s = VU.sum input
  let m = VU.maximum sums

  -- handle loop
  let min_ =
        if VU.length sums >= 2 && fst (VU.head sorted) == 0 && fst (VU.last sorted) == (modulus - 1)
           then max m (VU.head sums + VU.last sums)
           else m
