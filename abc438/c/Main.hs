...
greedy :: [Int] -> [Int] -> Int
greedy s [] = length s
greedy [] (t : ts) = greedy [t] ts
greedy s (t : ts)
  | p = greedy (drop 3 s) ts
  | otherwise = greedy (t : s) ts
  where
    s1 = head s
    p = t == s1 && take 3 s == [s1, s1, s1]

solve :: StateT BS.ByteString IO ()
solve = do
  !n <- intP
  !xs <- intsP
  printBSB $ greedy [] $ U.toList xs

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc438/tasks/abc438_c
main :: IO ()
main = runIO solve
