...
match :: BS.ByteString -> BS.ByteString -> Int
match s t
  | BS.length s /= BS.length t = maxBound
match s t = sum $ BS.zipWith eval s t
...
solve = do
  ...
  let res = minimum $ map (\i -> match (BS.take m (BS.drop i s)) t) [0 .. n - 1]
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc438/tasks/abc438_b
main :: IO ()
main = runIO solve
