...
-- sum of three consecutive parts
...
solve = do
  ...
  -- dp[i][l1][l2], but we're only intersted in (l1 + l2 = i) part
  -- dp[i][l1] is enough. Still, it's n^2 space

  -- Use bs as a benchmark. Then, come from either the left or the right.

  -- 0 1 2 3 4

  -- \* * *

  let diffA = U.zipWith subtract bs as
  let csumA = U.scanl1' (+) diffA
  let maxA = U.scanl1' max csumA

  let diffC = U.zipWith subtract bs cs
  let csumC = U.scanr1' (+) diffC
  let maxC = U.scanr1' max csumC

  let !_ = dbg $ U.sum bs
  let !res = maximum [maxA G.! (i - 1) + maxC G.! (i + 1) | i <- [1 .. n - 2]]

  printBSB $ U.sum bs + res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc438/tasks/abc438_d
main :: IO ()
main = runIO solve
