...
main = do
  ...
  let ss = zip s (tail s)
  let !ns = M.fromListWith (+) $ map (, 1:: Int) ss
  let !_ = dbg $ M.assocs ns
  let !result = head . last . groupBy (\a b -> (snd a) == (snd b)) . sortBy (comparing snd) $ M.assocs ns

  putStrLn $ (\(!c1, !c2) -> [c1] ++ [c2]) $ fst result
