...
main = do
  ...
  let !iOver =
        if base == 0
          then len + 1
          else fst $ until ((> 10^9) . snd) (bimap succ (* base)) (0, 1)

  let !_ = dbg (base, len, iOver, base ^ iOver)

  let !io = min (pred iOver) len
  let !ix = max 0 (len - pred iOver)

  putStr $ replicate io 'o'
  putStr $ replicate ix 'x'
