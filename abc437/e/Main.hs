...
solve = do
  ...
  -- Transition array: [(val, to)]
  let transitions :: V.Vector (U.Vector (Int, Int))
      transitions =
        V.map (U.modify VAI.sort . U.fromList)
          . V.accumulate (flip (:)) (V.replicate (n + 1) [])
          . U.convert
          $ U.imap (\((+ 1) -> to) (!from, !v) -> (from, (v, to))) fromVals

  let !_ = dbg transitions
  res <- Q.new n

  let visit nexts = do
        -- TODO: should perform BFS to same value, then children
        for_ (G.groupBy ((==) `on` fst) nexts) $ \vtos -> do
          let !_ = dbg (nexts, vtos)
          G.forM_ vtos $ \(!_, !to) -> do
            Q.pushBack res to
          let nexts' = U.modify VAI.sort $ U.concatMap ((transitions G.!) . snd) vtos
          visit nexts'
  visit (transitions G.! 0)

  printVec =<< Q.unsafeFreeze res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc437/tasks/abc437_e
main :: IO ()
main = runIO solve
