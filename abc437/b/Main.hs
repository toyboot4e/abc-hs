...
solve = do
  (!h, !w, !n) <- ints3P
  !as <- V.replicateM h intsP
  bs <- S.fromList . U.toList <$> U.replicateM n intP
  printBSB . G.maximum $ G.map (U.length . U.filter (`S.member` bs)) as

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc437/tasks/abc437_b
main :: IO ()
main = runIO solve
