...
solve = do
  (!n, !q) <- ints2P
  orgXys <- U.replicateM n ints2P
  !qs <-
    U.replicateM q $
      intP >>= \case
        1 -> (1 :: Word8,,,,-1) <$> intP <*> intP <*> intP
        2 -> (2 :: Word8,,,,) <$> intP <*> intP <*> intP <*> intP
        _ -> error "unreachable"

  let xys = U.map rot45 orgXys
  mins <- Seg.build $ U.map (both Min) xys
  maxes <- Seg.build $ U.map (both Max) xys

  res <- (`U.mapMaybeM` qs) $ \case
    (1, pred -> i, !x_, !y_, -1) -> do
      let (!x, !y) = rot45 (x_, y_)
      Seg.write mins i (Min x, Min y)
      Seg.write maxes i (Max x, Max y)
      pure Nothing
    (2, pred -> l, pred -> r, !x_, !y_) -> do
      let (!x, !y) = rot45 (x_, y_)
      (Min minX, Min minY) <- Seg.prod mins l (r + 1)
      (Max maxX, Max maxY) <- Seg.prod maxes l (r + 1)
      let !a = abs $ x - minX
      let !b = abs $ x - maxX
      let !c = abs $ y - minY
      let !d = abs $ y - maxY
      pure . Just $ a `max` b `max` c `max` d
    _ -> error "unreachable"

  printBSB $ unlinesBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc437/tasks/abc437_f
main :: IO ()
main = runIO solve
