...
solve = do
  ...
  -- 到達可能か判定して、後は貪欲か何かで解けるんじゃないか
  --
  -- 各頂点の最後の色は一意に定まる (初期値 + 次数)
  --
  -- 到達不能な場合を不変の値から導けないか
  -- - 初期状態で全ての頂点が同じ色であれば、明らかに切断不能
  -- - 辺を切った後の部分問題において、全ての辺が同じ色であれば、やはり切断不能
  --
  -- 同じ色の頂点を 1 つの連結成分とみなして考察すべきか

  printBSB "TODO"

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc437/tasks/abc437_g
main :: IO ()
main = runIO $ do
  t <- intP
  replicateM_ t solve
