...
solve = do
  !n <- intP
  !wps <- U.replicateM n ints2P

  -- Under sum(W) <= sum(P), retrieve maximum n_W
  --
  -- dp[n] := P - W; maximize P - W
  --
  -- Sum up (P_i + W_i), retrieve

  let s = dbgId . U.sum $ U.map snd wps
  let ds = dbgId . U.modify VAI.sort $ U.map (uncurry (+)) wps
  printBSB . U.length . dbgId . U.takeWhile (<= s) $ U.scanl1' (+) ds

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc437/tasks/abc437_c
main :: IO ()
main = runIO $ do
  t <- intP
  replicateM_ t solve
