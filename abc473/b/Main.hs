...
solve = do
  !n <- intP
  !xs <- intsP
  let freq = U.map (`mod`2 ) . U.accumulate (+) (U.replicate 101 (0 :: Int)) $ U.map (,1) xs
  printBSB $ U.sum $ U.imap (*) freq

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc473/tasks/abc473_b
main :: IO ()
main = runIO solve
