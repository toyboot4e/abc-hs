...
main = do
  ...
  !grid <- listArray @UArray bounds_ . concat <$> replicateM h ints

  !vis <- newArray @IOUArray ((0, 0), (pred h, pred w)) False

  -- FIXME: easier result.. maybe just do foldM
  !res <- VUM.replicate (3 * h * w) (maxBound :: Int, maxBound :: Int)
  !resCount <- newIORef (0 :: Int)

  let !dyxs = [(0, 1), (1, 0)]
  let dfs v1 = do
        writeArray vis v1 True
        let !v2s = map (add2 v1) dyxs
        forM_ v2s $ \v2 -> do
          when (inRange bounds_ v2) $ do
            let !x = grid ! v1
            let !y = grid ! v2
            !cnt <- readIORef resCount
            VUM.write res cnt (min x y, max x y)
            modifyIORef resCount succ
  ...
  !cnt <- readIORef resCount
  !res' <- VU.take (cnt) . VU.modify VAI.sort <$> VU.unsafeFreeze res
  VU.forM_ (res') $ \(!x, !y) -> putStrLn $ show x ++ " " ++ show y
