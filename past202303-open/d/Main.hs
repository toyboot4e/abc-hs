...
main = do
  ...
  -- greedy
  let candidates = map (\i -> (d * i, times i (apply2 c) h)) [0 .. 32]
  let results = map (until ((<= 0) . snd) (bimap (+ b) (apply1 a))) candidates

  print $ minimum $ map fst results
