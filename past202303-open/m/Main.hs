...
main = do
  ...
  !stree <- newSTreeVU max nBoxes (0 :: Int)
  VU.imapM_ (insertSTree stree) boxes

  -- TODO: better code..
  !result <- foldForMVG (True, -1) (VU.zip (rangeVU 0 (pred nInput)) input) $ \res (!ix, !x) -> do
    if not (fst res)
      then return res
      else do
        maybeI <- bsearchMR (0, pred nBoxes) $ \r -> do
          (< x) . fromJust <$> querySTree stree (0, r)

        case maybeI of
          Nothing -> return (False, ix)
          Just i -> do
            modifySTree stree (subtract x) i
            return (True, -1)

  if fst result
    then do
      putStrLn "Yes"
      !ws <- forM [0 .. pred nBoxes] $ \i -> do
        !rest <- fromJust <$> querySTree stree (i, i)
        return $ boxes VU.! i - rest
      putStrLn . unwords $ map show ws
    else do
      putStrLn "No"
      print $ succ $ snd result
