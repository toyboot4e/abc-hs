...
main = do
  [n, t, p_] <- ints
  !as <- VU.reverse <$> intsVU

  let t' a = a - t * a `div` 100

  let p = intToDouble p_ / 100.0
  let p' = 1.0 - p

  let !result = VU.create $ do
        -- Start from the end. Consider penalty as a transition to the before last.
        !dp <- VUM.replicate n (0.0 :: Double)

        let !a0 = as VU.! 0
        VUM.write dp 0 (p * intToDouble (t' a0) + p' * intToDouble a0)

        forMS_ (rangeMS 1 (pred n)) $ \i -> do
          let !ai = intToDouble $ as VU.! i
          let !ai' = intToDouble $ t' (as VU.! i)
          !x1 <- do
            !dp1 <- VUM.read dp (i - 1)
            !dp2 <- if i == 1 then return 0.0 else VUM.read dp (i - 2)
            return $ p * (ai' + dp2) + p' * (ai + dp1)
          !x2 <- do
            !dp1 <- VUM.read dp (i - 1)
            return $ ai' + dp1
          let !_ = dbg (i, x1, x2)
          VUM.write dp i (max x1 x2)

        return dp

  let !_ = dbg (result)
  print $ VU.last result
