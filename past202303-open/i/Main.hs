...
contribs :: Int -> [Char] -> [(Char, Int)]
contribs !constraint !xs = map (\cs -> (head cs, min constraint . length $ cs)) . group $ sort xs

main :: IO ()
main = do
  [n] <- ints
  !field <- contribs 3 <$> getLine
  !players <- replicateM n (contribs 2 <$> getLine)

  let !_ = dbg ("field", field)
  let !_ = dbg ("players", players)

  let !charCounts = map (accumArray @UArray (+) (0 :: Int) ('A', 'Z')) $ map (field ++) players
  -- let !_ = dbg (charCounts)

  let !results = map (head . sortBy (comparing (\(!ch, !n) -> (Down n, ch))) . assocs) charCounts
  let !_ = dbg (results)

  let !best = fst . head . sortBy (comparing (\(!_, (!ch, !n)) -> (Down n, ch))) $ zip [1 :: Int ..] results
  print best
