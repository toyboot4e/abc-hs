...
solve = do
  (!n, !h0, !c) <- ints3P
  !hps <- U.replicateM n ints2P

  let (!_, !res) = U.foldl' step s0 hps
        where
          s0 = (h0, 0 :: Int)
          step (!acc, !nSkip) (!h, !p)
            | acc >= h = (acc - h + p, nSkip)
            | otherwise = (acc, nSkip + 1)

  printBSB $ res * c

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0009/tasks/awc0009_b
main :: IO ()
main = runIO solve
