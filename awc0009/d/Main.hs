...
solve = do
  ...
  !lrs0 <- U.modify VAI.sort . U.map (first pred) <$> U.replicateM m ints2P

  -- Assume that the spans are merged.
  -- Create input events: [(offset, length)].

  let lrsMerged = dbgId $ U.unfoldr f lrs0
        where
          f :: U.Vector (Int, Int) -> Maybe ((Int, Int), U.Vector (Int, Int))
          f lrs = uncurry g <$> G.uncons lrs
          g :: (Int, Int) -> U.Vector (Int, Int) -> ((Int, Int), U.Vector (Int, Int))
          g (!l, !r) lrs = case G.uncons lrs of
            Just ((!l', !r'), !lrs')
              -- merge if intersectig or adjacent
              | l' <= r -> g (l, r') lrs'
            _ -> ((l, r), lrs)

  let rmost = U.maximum $ U.map snd lrs0
  let inputs = dbgId . (`U.snoc` (rmost, rmost + n)) . snd $ EV.mapAccumL f s0 lrsMerged
        where
          s0 = 0 :: Int
          f l0 (!l, !r) = (r, (l0, l))

  let accs = dbgId . snd $ EV.mapAccumL f (0 :: Int) inputs
        where
          f !acc (!l, !r) = (acc + (r - l), (acc, l, r))

  let res =
        U.head $
          U.mapMaybe
            ( \(!acc, !l, !r) ->
                if inRange (acc, acc + r - l) n
                  then Just $ l + n - acc
                  else Nothing
            )
            accs
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0009/tasks/awc0009_d
main :: IO ()
main = runIO solve
