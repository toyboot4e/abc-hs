...
solve = do
  (!n, !q) <- ints2P
  xs <- intsP
  lrs <- U.replicateM q (first pred <$> ints2P)

  -- TODO: RMQ?
  mins <- Seg.build $ U.map Min xs
  maxes <- Seg.build $ U.map Max xs

  let f (!l, !r) = do
        Min l' <- Seg.prod mins l r
        Max r' <- Seg.prod maxes l r
        pure $ r' - l'

  printBSB . unlinesBSB =<< G.mapM f lrs

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0009/tasks/awc0009_e
main :: IO ()
main = runIO solve
