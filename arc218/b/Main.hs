...
solve = do
  !n <- intP
  !rle <- U.fromList . map ((,) <$> U.head <*> U.length) . U.group . U.modify VAI.sort <$> intsP

  let deltas = U.imap f rle
        where
          f 0 x = x
          f i (!x, !len) =
            let !xPrev = fst $ rle G.! (i - 1)
             in (x - xPrev, len)

  let !_ = dbg deltas
  let res = U.foldr' f False deltas
        where
          -- fst will be snd
          f (0, 1) fstWins = not fstWins
          -- fst can select whichever result
          f (0, _) fstWins = True
          -- fst will be fst
          f (1, 1) fstWins = fstWins
          -- snd can select whichever result
          f (1, !_) fstWins = False
          -- snd can select whichever result
          f (!_, !1) fstWins = True
          -- fst can select whichever
          f (!_, !_) fstWins = True

  printBSB $ bool "Bob" "Alice" res

-- verification-helper: PROBLEM https://atcoder.jp/contests/arc218/tasks/arc218_b
main :: IO ()
main = runIO $ do
  t <- intP
  replicateM_ t solve
