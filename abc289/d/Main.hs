...
main = do
  ...
  xs' <- getLineIntList
  [_] <- getLineIntList
  stopInput <- getLineIntList
  [x] <- getLineIntList

  let xs = sort $ filter (<= x) xs'
  let stops = accumArray @UArray (||) False (0, x) (map (,True) stopInput)

  let yn = (x `IS.member`) . fst $ until (null . snd) step s0
      s0 = (IS.singleton 0, [0 :: Int])
      step :: (IS.IntSet, [Int]) -> (IS.IntSet, [Int])
      step (vs, []) = (vs, [])
      step (vs, is) =
        let -- !_ = traceShow (vs, is) ()
            nexts :: [Int]
            nexts = filter (\v -> v `IS.notMember` vs && v <= x) $ nubSort $ concatMap (\i -> map (+ i) xs) is
         in (IS.union vs (IS.fromList nexts), filter (not . (stops !)) nexts)

  putStrLn $ if yn then "Yes" else "No"
