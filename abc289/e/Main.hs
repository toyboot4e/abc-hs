...
main = do
  ...
  replicateM_ nTests solveOne

bfs :: Array Int [Int] -> Array Int [Int] -> (Int, Int) -> (Int, Int) -> Int
bfs !graph0 graph1 !start !end = runST $ do
  let !undef = -1 :: Int
  -- TODO: replace with array
  !vis <- VUM.replicate (nVerts * nVerts) False
  -- let !_ = dbg ("start", start, "end", end)

  let m !_ [] = return undef
      m !len !vs = do
        let inner = foldM step [] vs
            step !acc !v = do
              !visited <- VUM.read vis (ix v)
              if visited
                then return acc
                else do
                  VUM.write vis (ix v) True

                  -- collect neighbors
                  foldM
                    ( \ !acc' !v -> do
                        !vv <- VUM.read vis $ ix v
                        return $
                          if vv
                            then acc'
                            else v : acc'
        ...
        !vs' <- inner
        !isEnd <- VUM.read vis (ix end)

        if isEnd
          then return len
          else m (succ len) vs'

  VUM.write vis (ix start) True
  !len <- m 1 (get start)
  return len
  where
    !nVerts = succ . snd $ bounds graph0

    ix :: (Int, Int) -> Int
    ix = index ((0, 0), (pred nVerts, pred nVerts))
...
solveOne = do
  ...
  !colors <- getLineIntVec

  !input <- concatMap (\[a, b] -> [(a, b), (b, a)]) <$> replicateM nEdges (map pred <$> getLineIntList)

  -- Having two separate graph is much cheaper
  let !graph0 = accumArray @Array (flip (:)) [] (0, pred nVerts) $ filter (\(!_, !b) -> colors VU.! b == 0) input
  let !graph1 = accumArray @Array (flip (:)) [] (0, pred nVerts) $ filter (\(!_, !b) -> colors VU.! b == 1) input

  -- let !_ = dbg stateGraph
  print $ bfs graph0 graph1 (0, pred nVerts) (pred nVerts, 0)
