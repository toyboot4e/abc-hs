...
main = do
  -- [n, m] <- getLineIntList
  -- reverses <- concatMap (\i -> [(i - 1, True), (i , True)]) <$> getLineIntList
  -- let xs = accumArray @UArray (||) False (0, n) reverses

  -- let iss = groupBy (\i1 i2 -> xs ! i1 == xs ! i2) [1 .. n]
  -- let result = map reverse iss
  -- let !_ = traceShow (iss, result) ()
  -- putStrLn $ unwords $ map show $ concat result
  ...
  let g = foldM step s0 [2 .. n]
      s0 = ([1], [[]])
      step (is, acc) i = do
        b <- sameMUF uf (i - 1) i :: IO Bool
        return $ if b
          then (i : is, acc)
          else ([i], is : acc)

  (k, result_) <- g
  let result = k : result_
  let result' = tail $ reverse result
