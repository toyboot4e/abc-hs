...
main = do
  ...
  -- no need of self loop edges
  -- !edges0 <- S.fromList . filter (uncurry (/=)) <$> replicateM nEdges (feat <$> ints2)
  !edges0 <- replicateM nEdges (feat <$> ints2)
  forM_ edges0 $ uncurry (uniteMUF uf)

  [nConstraints] <- ints
  !constraints <-
    S.fromList
      <$> ( replicateM nConstraints $ do
              (!v1, !v2) <- ints2
              (\v1 v2 -> feat (v1, v2)) <$> rootMUF uf v1 <*> rootMUF uf v2
          )

  let !_ = dbg (constraints)
  !nq <- readLn @Int

  when (any (uncurry (==)) (S.elems constraints)) $ do
    replicateM_ nq $ putStrLn "No"
    exitSuccess
  ...
  replicateM_ nq $ do
    ...
    -- let !_ = dbg (edge, edge `S.member` constraints)
    printYn . not $ (feat (r1, r2)) `S.member` constraints
