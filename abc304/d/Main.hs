...
main = do
  ...
  let areaOf (!x, !y) =
        let !ix = maybe 0 succ $ bsearchL (0, pred nvs) $ \i -> vs VU.! i < x
            !iy = maybe 0 succ $ bsearchL (0, pred nhs) $ \i -> hs VU.! i < y
         in (ix, iy)

  let !contribs = map ((,1 :: Int) . areaOf) xys :: [((Int, Int), Int)]
  let !result = M.fromListWith (+) contribs

  let !resMax = maximum $ M.elems result
  let !resMin =
        if M.size result < ((nvs + 1) * (nhs + 1))
          then 0
          else minimum $ M.elems result

  putStrLn $ unwords . map show $ [resMin, resMax]
