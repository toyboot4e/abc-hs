...
main = do
  [n, d] <- ints
  !xys <- VU.replicateM n ints2

  !vis <- VUM.replicate n False
  VUM.write vis 0 True
        ...
        where
          inner !vs = do
            ...
            when (not (null vs')) $ do
              inner vs'
          s0 = [0]
          step !v1s = foldM f [] v1s
          f !acc0 !v1 = foldM h acc0 [0 .. pred n]
            where
              (!x1, !y1) = xys VU.! v1
              h !acc !v2 = do
                !b <- VUM.read vis v2
                let (!x2, !y2) = xys VU.! v2
                if b || ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1) > d * d)
                  then return acc
                  else do
  ...
  !res <- VU.unsafeFreeze vis
  VU.forM_ res printYn
