...
count :: VU.Vector Bool -> Int -> MyModInt
count !xs d = pow2 nFreeDays
  where
    !n = VU.length xs
    !nd = n `div` d

    -- requirements
    !mustAttend = VU.generate d f
      where
        f i = VU.any (\j -> not $ xs VU.! (i + j * d)) (rangeVU 0 (pred nd))

    !nFreeDays = d - (vLength $ VU.filter id mustAttend)
    pow2 i = VU.foldl' (\acc _ -> acc * modInt 2) (modInt 1) (rangeVU 1 i)

main :: IO ()
main = do
  [n] <- ints
  !xs <- VU.fromList . map (== '#') <$> getLine

  -- Solution using Exclusion Princile. Too difficult:
  -- <https://qiita.com/hossie/items/1f9fd893928cf7327119#%E6%96%B9%E9%87%9D2-%E5%91%A8%E6%9C%9F%E3%81%AE%E9%95%B7%E3%81%84%E6%96%B9%E3%81%8B%E3%82%89%E7%B5%9E%E3%82%8A%E8%BE%BC%E3%82%80>
  let ps = map fst $ primeFactors n
  let !_ = dbg (n, ps)

  let !results = VU.map f (rangeVU (1 :: Int) (length ps))
        where
          f k =
            let prods = map (foldl' (*) (1 :: Int)) $ combinations k ps
                intersection = sum $ map (\prod -> count xs (n `div` prod)) prods
                sign = modInt $ if even k then -1 else 1
             in sign * intersection

  print $ VU.sum results
