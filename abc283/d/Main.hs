...
solve s =
  let (b, _, _) = foldl' step (True, IS.empty, []) [0 .. BS.length s - 1]
   in b
  where
    step :: (Bool, IS.IntSet, [Int]) -> Int -> (Bool, IS.IntSet, [Int])
    step end@(False, _, _) _ = end
    step (True, set, open) i =
      case BS.index s i of
        '(' -> (True, set, i : open)
        ')' ->
          let set' =
                fst $
                  foldl'
                    ( \state_@(set_, iNest) ch ->
                        case ch of
                          '(' -> (set_, iNest + 1)
                          ')' -> (set_, iNest - 1)
                          _ ->
                            if iNest == 0
                              then (IS.delete (ord ch) set_, iNest)
                              else state_
                    )
                    (set, 0 :: Int)
                    (map (BS.index s) [(head open + 1) .. (i - 1)])
           in (True, set', tail open)
        ch ->
          let ch' = ord ch
           in (not $ IS.member ch' set, IS.insert ch' set, open)
