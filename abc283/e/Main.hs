...
main = do
  [h, w] <- getLineIntList
  mat <- V.replicateM h getLineIntVec

  let testRows = all (\i -> all (testRow (mat V.! i)) [1 .. w - 2]) mat
      testRow row iCol = row VU.! (iCol - 1) == row VU.! (iCol + 1)

  let result = any step [0 .. h -3]
      step i =
        let row1 = mat V.! i
            row2 = mat V.! (i + 1)
            row3 = mat V.! (i + 2)
  ...
  -- any (\i -> row2 VU.! (i - 1) == row VU.! (i + 1)

  putStrLn if h >= 2 then "Yes" else "No"
