...
import AtCoder.Extra.SqrtDecomposition qualified as Sq
...
solve = do
  ...
  !qs <-
    U.replicateM q $
      int' >>= \case
        1 -> (1 :: Word8,,,) <$> int' <*> int' <*> int'
        2 -> (2 :: Word8,,,) <$> int' <*> int' <*> int'
        3 -> (2 :: Word8,,,-1 :: Int) <$> int' <*> int'
        _ -> error "unreachable"

  let blockLen :: Int = fromIntegral ceiling $ sqrt (intToDouble n)
  let nBlocks = (n + (blockLen - 1)) `div` blockLen
  blocks <- V.replicate nBlocks (0 :: Int, IM.empty)
  ...
  res <-
    U.mapMaybe
      ( \case
          (1, l, !r, !x) -> do
            -- range add
            Sq.forM_ blockLen actFull actPart l (r + 1)
            pure ()
          (2, l, !r, !x) -> do
            -- range remove
            Sq.forM_ blockLen actFull actPart l (r + 1)
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc430/tasks/abc430_g
main :: IO ()
main = runIO solve
