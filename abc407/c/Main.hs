...
roll lastX dx
  | dx == 0 && lastX == 0 = 0
  | dx == 0 = 10 - lastX
  | otherwise = inner lastX
  where
    -- TODO: faster
    inner x
      | 10 - x == dx {- stop -} = 0
      | otherwise = 1 + inner (succ x `mod` 10)

try :: Int -> Int -> Int -> U.Vector Int -> Int
try cnt0 offset0 desiredOffset dxs = resCnt + diffMod resHeadOffset desiredOffset
  where
    !_ = dbg ("try", offset0)
    (!resCnt, !resHeadOffset, !_) =
      U.foldl'
        ( \(!cnt, !headOffset, !lastX) dx ->
            let !_ = dbg (cnt, headOffset, lastX, dx)
                !deltaOffset = roll lastX dx
                !headOffset' = (headOffset + deltaOffset) `mod` 10
                !cnt' = cnt + 1 + deltaOffset
                -- !_ = dbg ((dx, lastX), deltaOffset)
             in (cnt', headOffset', 0)
        )
        (cnt0, offset0, offset0)
        dxs
...
solve = do
  !xs <- U.fromList . map (subtract (ord '0') . ord) . BS.unpack <$> line'

  when (U.length xs == 1) $ do
    printBSB $ U.head xs + 1
    liftIO exitSuccess

  -- try all the first numbers
  -- add numbers (greedy)
  -- match the offset
  let !dxs = U.zipWith diffMod xs (U.tail xs)
  let !res = minimum [try (1 + offset) offset (U.head xs) dxs | offset <- [0 .. 9]]
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc407/tasks/abc407_c
main :: IO ()
main = runIO solve
