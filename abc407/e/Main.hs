...
solve = do
  !n <- int'
  !xs <- U.replicateM (2 * n) int'
  let !_ = dbg (n, xs)

  heap <- buildMaxBinaryHeap . U.init . U.tail $ U.imap (flip (,)) xs
  seg <- newSTree @(Sum Int) (2 * n)
  writeSTree seg 0 $ Sum 1

  flip fix (n - 1) $ \loop rest -> do
    when (rest > 0) $ do
      (!x, !i) <- deleteBH heap
      let !lenL = i
      let !lenR = 2 * n - (i + 1)
      Sum nBlackL <- foldSTree seg 0 (i - 1)
      Sum nBlackR <- foldSTree seg (i + 1) (2 * n - 1)
      -- * 白を使い尽くせること
      -- 左の黒が過半数以上
      let nUseL = max 0 $ ((lenL + 1) `div` 2) - nBlackL
      -- 右の白を消費し尽くせる
      let nFromL = max 0 $ nBlackL - lenL `div` 2
      let nUseR = max 0 $ ((lenR + 2) `div` 2) - (nFromL + 1 + nBlackR)
      let canConsumeWhite = nUseL + 1 <= rest && nUseL + 1 + nUseR <= rest
      -- * 余った黒を使い尽くせること
      let canConsumeBlack = nFromL + 1 + nBlackR <= (nFromL + 1 + lenR) `div` 2
      if canConsumeWhite && canConsumeBlack
        then do
          let !_ = dbg ("use", (i, x))
          writeSTree seg i $ Sum 1
          loop (rest - 1)
        else do
          loop rest

  is <- U.findIndices (== Sum 1) <$> unsafeFreezeLeavesSTree seg
  printBSB $ U.sum $ U.backpermute xs is

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc407/tasks/abc407_e
main :: IO ()
main = runIO $ do
  t <- int'
  replicateM_ t solve
