...
dfs :: (Int, Int) -> UArray (Int, Int) Int -> (Int, Int) -> Int
dfs (h, w) graph start = inner (IS.empty) start
  where
    bounds_ = bounds graph

    inner vis (y, x)
      | (y, x) == (pred h, pred w) =
        if IS.notMember (graph ! (y, x)) vis
          then 1
          else 0
      | IS.member (graph ! (y, x)) vis = 0
      | otherwise = sum $ map (inner vis') vs
      where
        vs = filter (inRange bounds_) $ map (add2 (y, x)) [(1, 0), (0, 1)]
        vis' = IS.insert (graph ! (y, x)) vis

-- search all of 20C10: 2*10^5. not too much
main :: IO ()
main = do
  [h, w] <- getLineIntList
  input <- concat <$> replicateM h getLineIntList
  let mat = listArray @UArray ((0, 0), (pred h, pred w)) input
  let result = dfs (h, w) mat (0, 0)
