...
dfs :: Graph -> IS.IntSet -> Int -> (IS.IntSet, Bool)
dfs !graph !vis0 !start = inner start vis0 start
  where
    inner :: Int -> IS.IntSet -> Int -> (IS.IntSet, Bool)
    inner !lastV !vis !v
      -- looping
      | lastV /= start && v == start = (vis, True)
      | otherwise =
        let !vis' = IS.insert v vis
            !vs = filter (/= lastV) $ graph ! v
         in foldl' f (vis', False) vs
      where
        f (!vis'', True) _ = (vis'', True)
        f (!vis'', False) v2 = inner v vis'' v2
...
-- graph version. why WA? give me test cases please!
main :: IO ()
main = do
  [!nVerts, !nOpers] <- getLineIntList

  !input0 <- replicateM nOpers $ do
    [!v1, !c1, !v2, !c2] <- words <$> getLine
    let !v1' = pred (read @Int v1) + nVerts * colorMod c1
    let !v2' = pred (read @Int v2) + nVerts * colorMod c2
    return [(v1', v2'), (v2', v1')]

  let !input = foldl' (\ !acc !x -> (x, x + nVerts) : (x + nVerts, x) : acc) (concat input0) [0 .. pred nVerts]
  let !graph = accumArray @Array (flip (:)) [] (0, pred $ 2 * nVerts) input

  let (!_, !nCycles, !nLinears) = foldl' step s0 [0 .. pred $ 2 * nVerts]
      s0 = (IS.empty, 0 :: Int, 0 :: Int)
      step acc@(!vis, !n1, !n2) v
        | IS.member v vis = acc
        | otherwise =
          let (vis', b) = dfs graph vis v
           in if b
                then (vis', succ n1, n2)
                else (vis', n1, succ n2)

  putStrLn $ unwords $ map show [nCycles, nLinears]
