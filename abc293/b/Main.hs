...
main = do
  ...
  let result = VU.create $ do
        vec <- VUM.replicate n False

        flip VU.imapM_ input $ \i x -> do
          isCalled <- VUM.read vec i
          unless isCalled $ do
            VUM.write vec (pred x) True

        return vec

  let is = VU.map (succ . fst) $ VU.filter (not . snd) $ VU.zip (VU.fromList [0 .. pred n]) result
  print $ VU.length is
  putStrLn $ unwords $ map show $ VU.toList is
