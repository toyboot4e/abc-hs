...
main :: IO ()
main = do
  [a, x, m] <- getLineIntList

  --     result = a^0 + a^1 + .. + a^(x-1)
  -- a * result =       a^1 + .. + a^(x-1) + a^x
  -- (a - 1) result = a^x - a^0
  --
  -- Get mod with (a - 1) * m, but with `Integer` type
  let ax = powerModConstant a x m
  let ax' = fromIntegral $ ((m - 1) + ax) `rem` m :: Integer
  let m' = fromIntegral m * fromIntegral (a - 1) :: Integer
  let !_ = traceShow ((a, x, m), ax, ax', m', ax' `rem` m') ()

  print $ if' (a == 1) ax' $ ax' `rem` m'
