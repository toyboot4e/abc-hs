...
mulMat :: Int -> (Int, Int) -> (Int, Int) -> (Int, Int)
mulMat m (!x1, !x2) (!y1, !y2) = both (`rem` m) (x1 * y1, x1 * y2 + x2)

main :: IO ()
main = do
  [a, x, m] <- ints

  let !doubled = newDoubling @VU.Vector (a, 1) (\z -> mulMat m z z)
  let !result = applyDoubling doubled (1, 0) (mulMat m) x
