...
walkPath :: Graph -> Int -> (IS.IntSet, (Int, Int))
walkPath graph x0 = visitX IS.empty (0, 0) x0 (-1)
  where
    -- stop on failure
    visitX _ (-1, -1) _ _ = (IS.empty, (-1, -1))
    visitX vs (n1, n2) x parent
      -- ignore parent
      | x == parent = (vs, (n1, n2))
      -- failure
      | IS.member x vs = (vs, (-1, -1))
      | otherwise =
        let vs' = IS.insert x vs
            nbs = filter (\i -> i /= parent) $ graph ! x
            !_ = traceShow () ()
         in if parent == -1
              then case nbs of
                [] -> (IS.empty, (-1, -1))
                [nb0] -> visitX vs' (n1 + 1, n2) nb0 x
                [nb0, nb1] ->
                  let (vs'', n') = visitX vs' (n1, n2 + 1) nb0 x
                   in visitX vs'' n' nb1 x
                _ -> (vs', (-1, -1))
              else case nbs of
                [] -> (vs', (n1 + 1, n2))
                [nb0] -> visitX vs' (n1, n2 + 1) nb0 x
                _ -> (vs', (-1, -1))
...
main = do
  ...
  -- two neighbors: (n - 2) vertices
  -- one neighbor: two vertices
  let (_, (n1, n2)) = walkPath graph 1
  let !_ = traceShow (n1, n2) ()
