...
main = do
  [n] <- getLineIntList
  xs <- V.replicateM n (VU.fromList . map (subtract 97 . ord) <$> getLine)
  results <- VUM.replicate n (0 :: Int)

  let solveM :: [Int] -> Int -> IO ()
      solveM [] _ = return ()
      solveM [_] _ = return ()
      solveM search i = do
        -- let !_ = traceShow (search, i) ()
        let arr = accumArray @Array (flip (:)) [] (0, pred 26) $ mapMaybe (\x -> (, x) <$> xs V.! x VU.!? i) search
        let nextSearches = filter (\case [] -> False; [_] -> False; _ -> True) $ elems arr
        forM_ nextSearches $ \search' -> do
          let i' = succ i
          forM_ search' $ \x -> do
            VUM.modify results (max i') x
          solveM search' i'

  _ <- solveM [0 .. pred n] 0

  results' <- VU.unsafeFreeze results
  VU.forM_ results' print
