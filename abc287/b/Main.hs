...
giveId :: String -> Int
giveId = sum . map (\(i, x) -> 1000 * i * ord x) . zip [1..]

main :: IO ()
main = do
  [n, m] <- getLineIntList
  xs <- replicateM n (giveId . drop 3 <$> getLine)
  keys <- IS.fromList <$> replicateM m (giveId <$> getLine)

  print $ length $ filter (`IS.member` keys) xs
