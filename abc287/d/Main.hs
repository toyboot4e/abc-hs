...
main :: IO ()
main = do
  -- s <- map (\c -> ord c - 65) <$> getLine
  -- t <- map (\c -> ord c - 65) <$> getLine
  s <- getLine
  t <- getLine

  let lt = length t

  let matchL = VU.fromList . scanl1 (&&) . (True :) $ map (uncurry f) $ zip t s
      matchR = VU.fromList . scanl1 (&&) . (True :) $ map (uncurry f) $ zip (reverse t) (reverse s)
      f '?' _ = True
      f _ '?' = True
      f c1 c2 = c1 == c2

  let results = map g [0 .. lt]
      g i = matchL VU.! i && matchR VU.! (lt - i)

  -- let !_ = traceShow (matchL) ()
  -- let !_ = traceShow (results) ()
