...
solveNaive limit xs = runST $ do
  heap <- newMinBinaryHeap (n + m)
  U.iforM_ xs $ \i x -> do
    insertBH heap (x, i + 1, x)

solve :: StateT BS.ByteString IO ()
solve = do
  (!n, !m) <- ints2'
  !xs <- U.filter ((<= m) . snd) . U.imap ((,) . (+ 1)) <$> intsU'

  -- すべての商品の値段を max の倍以内に収める
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc389/tasks/abc389_e
main :: IO ()
main = runIO solve
