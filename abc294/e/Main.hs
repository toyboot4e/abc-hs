...
main :: IO ()
main = do
  [l, n, m] <- getLineIntList
  !xs <- replicateM n (tuple2 <$> getLineIntList)
  !ys <- replicateM m (tuple2 <$> getLineIntList)
  print $ solve 0 0 xs ys

solve :: Int -> Int -> [(Int, Int)] -> [(Int, Int)] -> Int
solve _ _ [] _ = 0
solve _ _ _ [] = 0
solve !x0 !y0 xss@((!x, !lx) : xs) yss@((!y, !ly) : ys)
  | x /= y || nIntersection == 0 = case compare x1 y1 of
    LT -> solve (x0 + lx) y0 xs yss
    GT -> solve x0 (y0 + ly) xss ys
    EQ -> solve (x0 + lx) (y0 + ly) xs ys
  | x == y = case compare x1 y1 of
    LT -> nIntersection + solve (x0 + lx) y0 xs yss
    GT -> nIntersection + solve x0 (y0 + ly) xss ys
    EQ -> nIntersection + solve (x0 + lx) (y0 + ly) xs ys
  where
    -- !_ = traceShow (x0, y0, (x, lx), (y, ly), x == y, nIntersection) ()
    !x1 = x0 + lx - 1
    !y1 = y0 + ly - 1
    nIntersection =
      if
          | x1 < y0 || y1 < x0 -> 0
          | True -> succ $ min x1 y1 - max x0 y0
