...
main = do
  [n, m] <- getLineIntList
  !as <- map (0,) <$> getLineIntList
  !bs <- map (1,) <$> getLineIntList
  let !sorted = zip [1 .. ] $ HT.mergeBy (\(!_, !a) (!_, !b) -> a <= b) as bs

  resA <- VUM.replicate n (0 :: Int)
  resB <- VUM.replicate m (0 :: Int)

  let !m = foldlM step (0 :: Int, 0 :: Int) sorted
      step (na, nb) (!i, (0, _)) = do
        VUM.write resA na i
        return (succ na, nb)
      step (na, nb) (!i, (1, _)) = do
  ...
  !resA' <- VU.unsafeFreeze resA
  putStrLn $ unwords $ map show $ VU.toList $ resA'

  !resB' <- VU.unsafeFreeze resB
  putStrLn $ unwords $ map show $ VU.toList $ resB'
