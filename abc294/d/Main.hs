...
main :: IO ()
main = do
  [n, nQueries] <- getLineIntList

  let !noCall0 = [1 .. n]
  let !called0 = IS.empty

  !queries <- replicateM nQueries getLineIntList
  let (!_, !_, !result) = foldl' step s0 queries
      s0 = (noCall0, called0, [])
      step (x : noCall, !called, !acc) [1] = (noCall, IS.insert x called, acc)
      step (!noCall, !called, !acc) [2, x] = (noCall, IS.delete x called, acc)
      step (!noCall, !called, !acc) [3] =
        let x = IS.findMin called
         in (noCall, called, x: acc)
      step _ _ = error "A"
