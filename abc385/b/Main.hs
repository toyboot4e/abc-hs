...
solve = do
  (!h, !w, pred -> !y0, pred -> !x0) <- ints4'
  !gr <- getGrid' h w
  moves <- U.map dir . U.fromList . BS.unpack <$> line'
  let points = U.scanl' step (y0, x0) moves
        where
          step yx@(!y, !x) dir
            | gr @! yx' == '#' = yx
            | otherwise = yx'
            where
              !yx' = add2 yx dir
  let res = U.length . U.filter ((== '@') . (gr @!)) . U.uniq $ U.modify VAI.sort points
  let (!resY, !resX) = U.last points
  printBSB (resY + 1, resX + 1, res)

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc385/tasks/abc385_b
main :: IO ()
main = runIO solve
