...
main = do
  ...
  if k == 1
    then do
      ...
      let !input' = VU.zip (VU.map pred input) (VU.fromList [1 .. n])
      let !result = VU.accumulate (curry snd) (VU.replicate n 0) input'
      VU.forM_ result print
    else solve n k input

solve :: Int -> Int -> VU.Vector Int -> IO ()
solve n k input = do
  !result <- VUM.replicate n (-1 :: Int)
  let !m = VU.ifoldM step s0 input

      s0 :: IM.IntMap (Int, [Int])
      s0 = IM.empty

      step :: IM.IntMap (Int, [Int]) -> Int -> Int -> IO (IM.IntMap (Int, [Int]))
      step !acc turn0 !x = case IM.lookupGE x acc of
        Nothing -> return $ IM.insert x (1, [x]) acc
        Just (!x', (!n, !xs)) -> do
          -- let !_ = traceShow (turn0, x, x', (n, xs)) ()
          let !acc' = IM.delete x' acc
          if succ n == k
            then do
              -- eat
              -- let !_ = traceShow ("eat", turn0, x : xs, acc') ()
              forM_ (x : xs) $ \ !x1 -> do
                VUM.write result (pred x1) (succ turn0)
              return acc'
            else -- re-insert
              return $ IM.insert x (succ n, x : xs) acc'
