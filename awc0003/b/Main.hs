...
eat :: String -> String -> Int
eat _ [] = 0
eat [] _ = 0
eat (a : as) (b : bs) = bool 0 1 (a == b) + eat as bs

solve :: StateT BS.ByteString IO ()
solve = do
  !n <- intP
  (!ls, !rs) <- U.unzip <$> U.replicateM n ((,) <$> charP <*> charP)
  printBSB $ eat (tail (U.toList ls)) (U.toList rs)

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0003/tasks/awc0003_b
main :: IO ()
main = runIO solve
