...
match :: U.Vector Int -> [Int] -> Bool
match cs sums = U.and $ U.zipWith (>=) cs sums'
  where
    -- REMARK: |sums| <= |cs|. Some can be zero.
    sums' = U.modify (VAI.sortBy (comparing Down)) $ U.fromList sums
...
solve = do
  ...
  -- length: m (targets)
  !cs <- U.modify (VAI.sortBy (comparing Down)) <$> intsP

  -- NOTE: some can be empty
  let results = map (map (U.sum . U.backpermute ws . unBitSet n)) . filter ((<= m) . length) $ partitionsOf (bit n - 1)
  printYn $ any (match cs) results

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0003/tasks/awc0003_e
main :: IO ()
main = runIO solve
