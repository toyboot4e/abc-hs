...
solve = do
  (!n, !k) <- ints2P
  (!as, !bs) <- U.unzip <$> U.replicateM n ints2P

  let !m = U.sum as
  let !o = U.sum . U.take k . U.modify VAI.sort $ U.zipWith subtract as bs

  printBSB $ m + o

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0003/tasks/awc0003_c
main :: IO ()
main = runIO solve
