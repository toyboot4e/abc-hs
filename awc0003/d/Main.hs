...
solve = do
  ...
  -- Oh, we're filtering empty ranges with twoPointersU..
  let res = U.imap f . dbgId $ U.accumulate max (U.generate n pred) (twoPointersU n p)
        where
          f l r =
            let len = r + 1 - l
                iStart = min n $ r + max 1 (k - len)
             in n - iStart
          p l r = csum +! (l, r) < m

  printBSB $ U.sum res

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0003/tasks/awc0003_d
main :: IO ()
main = runIO solve
