...
resolve :: U.Vector Int -> IM.IntMap Int
resolve = IM.insert 0 1 . U.foldl' step IM.empty
  where
    step !m x = IM.unionWith (+) m . IM.insertWith (+) x 1 $ IM.mapKeys (+ x) m

solve :: StateT BS.ByteString IO ()
solve = do
  (!n, !s) <- ints2P
  (!xs, !ys) <- U.splitAt (n `div` 2) <$> intsP

  let res1 = dbgId $ resolve $ dbgId xs
  let res2 = dbgId $ resolve $ dbgId ys

  let res = sum $ mapMaybe (\(!x, !c) -> (c *) <$> res2 IM.!? (s - x)) $ IM.assocs res1
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0002/tasks/awc0002_e
main :: IO ()
main = runIO solve
