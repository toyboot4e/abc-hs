...
solve = do
  ...
  let bruises = U.accumulate (||) (U.replicate n False) $ U.map ((,True) . pred) ys
  let res = U.mapMaybe (\(!x, !b) -> bool Nothing (Just x) (b && x < k)) $ U.zip xs bruises
  printBSB $ showBSB (U.length res) <> showBSB ' ' <> showBSB (U.sum res)

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0002/tasks/awc0002_b
main :: IO ()
main = runIO solve
