...
eat (x : xs) (y : ys)
  | x <= y = 1 + eat xs ys
  | otherwise = eat (x : xs) ys

solve :: StateT BS.ByteString IO ()
solve = do
  (!n, !m) <- ints2P
  !cs <- U.modify VAI.sort <$> intsP
  !rs <- U.modify VAI.sort <$> intsP

  let !_ = dbg cs
  let !_ = dbg rs
  printBSB $ eat (U.toList cs) (U.toList rs)

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0002/tasks/awc0002_d
main :: IO ()
main = runIO solve
