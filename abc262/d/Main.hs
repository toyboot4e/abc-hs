...
solve :: VU.Vector Int -> Int -> MyModInt
solve xs nTake = arr ! (n, nTake, 0)
  where
    !n = VU.length xs
    !_ = dbg (arr)
    !arr = runSTArray $ do
      -- dp ! (nSeen, nTook, remTake)
      !dp <- newArray ((0, 0, 0), (n, nTake, pred nTake)) (modInt 0)
      writeArray dp (0, 0, 0) (modInt 1)

      flip VU.imapM xs $ \nSeen x -> do
        forM_ [0 .. nTake] $ \nTook -> do
          forM_ [0 .. pred nTake] $ \r -> do
            !c <- readArray dp (nSeen, nTook, r)

            -- take
            when (nTook < nTake) $ do
              modifyArray dp (+ c) (succ nSeen, succ nTook, (r + x) `rem` nTake)

            -- ignore
            modifyArray dp (+ c) (succ nSeen, nTook, r)
