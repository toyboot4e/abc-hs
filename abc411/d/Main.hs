...
{-# INLINE vfoldM #-}
vfoldM :: (Monad m, G.Vector v a, G.Vector v b) => (a -> b -> m a) -> a -> v b -> m a
vfoldM f x = FB.foldM f x . G.stream

{-# INLINE vreplicateM #-}
vreplicateM :: (PrimMonad m, G.Vector v a) => Int -> m a -> m (v a)
vreplicateM n m = unstreamPrimM (MB.replicateM n m)
...
solve = do
  (!n, !q) <- ints2'
  !qs <- vreplicateM @_ @V.Vector q $ do
    int' >>= \case
      1 -> (1 :: Int,,mempty) <$> int'
      2 -> (2 :: Int,,) <$> int' <*> word'
      3 -> (3 :: Int,,mempty) <$> int'

  vec <- VM.replicate n (mempty :: BSB.Builder)
  res <-
    vfoldM
      ( \server q -> case q of
          (1, pred -> i, !_) -> do
            GM.write vec i server
            pure server
          (2, pred -> i, !s) -> do
            GM.modify vec (<> showBSB s) i
            pure server
          (3, pred -> i, !_) -> do
            GM.read vec i
      )
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc411/tasks/abc411_d
main :: IO ()
main = runIO solve
