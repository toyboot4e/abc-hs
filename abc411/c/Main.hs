...
{-# INLINE postscanlM #-}
postscanlM :: (HasCallStack, Monad m, G.Vector v a, G.Vector v b) => (a -> b -> m a) -> a -> v b -> m (v a)
postscanlM f x0 =
  G.unstreamM
    . postscanlMB f x0
    . G.stream
...
{-# INLINE postscanlMB #-}
postscanlMB :: (Monad m) => (a -> b -> m a) -> a -> FB.Bundle v b -> MB.Bundle m v a
postscanlMB f x0 = MB.postscanlM f x0 . FB.lift

{-# INLINE primPostscanlM #-}
primPostscanlM :: (HasCallStack, PrimMonad m, G.Vector v a, G.Vector v b) => (a -> b -> m a) -> a -> v b -> m (v a)
primPostscanlM f x0 =
  unstreamPrimM
    . postscanlMB f x0
    . G.stream
...
unstreamPrimM :: (PrimMonad m, G.Vector v a) => MB.Bundle m u a -> m (v a)
...
solve = do
  (!n, !q) <- ints2'
  !xs <- intsU'
  vec <- UM.replicate (n + 2) $ Bit False

  res <-
    primPostscanlM
      ( \cnt i -> do
          Bit old <- GM.read vec i
          let !new = not old
          GM.write vec i $ Bit new

          Bit l <- GM.read vec (i - 1)
          Bit r <- GM.read vec (i + 1)

          pure $ case (l, new, r) of
            (False, True, False) -> cnt + 1
            (True, True, True) -> cnt - 1
            (True, False, True) -> cnt + 1
            (False, False, False) -> cnt - 1
            _ -> cnt
      )
      (0 :: Int)
      xs
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc411/tasks/abc411_c
main :: IO ()
main = runIO solve
