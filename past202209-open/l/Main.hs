...
solve :: VU.Vector Int -> Array Int [(Int, Int, Int)] -> Int -> Int
solve !xs !pqlrs !sumRem = res VU.! sumRem
  where
    !res = foldl' step s0 (assocs pqlrs)
    !s0 = VU.generate 3 (\case 0 -> 0; _ -> minBound)
    step !vec (!q, !plrs)
      -- REMARK: q is in range [1, n + 1]
      | q == (n + 1) =
          VU.imap (\i acc -> (acc +) . sum $ map (\(!p, !l, !r) -> if r == 0 && i == l then p else 0) plrs) vec
      where
        !n = VU.length xs
    step !vec (!q, !plrs) = VU.generate 3 f
      where
        -- take `x` or not take `x`, and contributions for it
        f i =
          let !vKeep = vec VU.! i + contribs i plrs
              !i' = (i - 1) `mod` 3
              !vTake = vec VU.! i' + contribs i' plrs + xs VU.! pred q
           in max vKeep vTake
        contribs :: Int -> [(Int, Int, Int)] -> Int
        contribs !i !plrs = sum $ map (\(!p, !l, !r) -> if i == l && ((sumRem - i) `mod` 3) == r then p else 0) plrs
...
main = do
  ...
  -- REMARK: q is in range [1, n + 1]
  !pqlrs <- accumArray @Array (flip (:)) [] (1, n + 1) . map (\(!p, !q, !l, !r) -> (q, (p, l, r))) <$> replicateM m ints4

  let !results = map (solve xs pqlrs) [0 .. 2]
