...
findLoop :: Int -> Int -> VU.Vector Int
findLoop a m = VU.fromList $ inner IS.empty (a `mod` m)
  where
    inner !is !x
      | IS.member x is = []
      | otherwise = cost x : inner (IS.insert x is) ((x + a) `mod` m)
    cost 0 = 0
    cost x = m - x

solve :: Int -> Int -> Int -> Int
solve n a m =
  let !loopVec = findLoop a m
      (!nLoops, !remLoop) = n `divMod` VU.length loopVec
      !_ = dbg ((n, a, m), loopVec, (nLoops, remLoop), nLoops * VU.sum loopVec + VU.sum (VU.take remLoop loopVec))
   in nLoops * VU.sum loopVec + VU.sum (VU.take remLoop loopVec)

main :: IO ()
main = do
  [nTests] <- ints
  replicateM_ nTests $ ints3 >>= print . uncurry3 solve
