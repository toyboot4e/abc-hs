...
main = do
  (!nVerts, !nEdges) <- ints2
  !edges <- nubSort . map (\(!v1, !v2) -> (min v1 v2, max v1 v2)) <$> replicateM nEdges ints2
  let !noDouble = length edges == nEdges
  let !isInBounds = all (uncurry (&&) . both (inRange (1, nVerts))) edges
  let !noSelfLoop = all (uncurry (/=)) edges
  putStrLn . yn $ noDouble && isInBounds && noSelfLoop
