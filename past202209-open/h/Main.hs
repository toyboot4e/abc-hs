...
main :: IO ()
main = do
  [nInput, maxWeight] <- ints
  !input <- VU.replicateM nInput ints3

  let !undef2 = (-1, -1) :: (Int, Int)

  let !result = VU.foldl' step s0 input
        where
          s0 = VU.generate (succ maxWeight) (\case 0 -> (0 :: Int, 0 :: Int); _ -> undef2)
          step !vec (!a, !w, !c) = VU.generate (succ maxWeight) f
            where
              -- REMARK: `w` can be zero.
              f i | i - w < 0 = vec VU.! i
              f i =
                let !xKeep = vec VU.! i
                    !xLast = vec VU.! (i - w)
                 ...
                 in merge xKeep xTake
              merge :: (Int, Int) -> (Int, Int) -> (Int, Int)
              merge (!a1, !c1) (!a2, !c2) = case compare c1 c2 of
                EQ -> (min a1 a2, c1)
                GT -> (a1, c1)
                LT -> (a2, c2)

  let !_ = dbg (result)
  let (!resW, (!resA, !resC)) = VU.maximumBy (comparing (\(!w, (!a, !c)) -> (c, Down a, Down w))) $ VU.zip (rangeVU 0 (succ maxWeight)) result

  putStrLn $ unwords $ map show [resC, 1_000_000_000 - resA, maxWeight - resW]
