...
main = do
  [nVerts, nEdges] <- ints
  -- REMARK: They contain both mono and dynamic edges:
  !initialEdges <- S.fromList <$> replicateM nEdges ints2
  [nQueries] <- ints
  !queries <- replicateM nQueries ints3

  let !dynEdges = S.fromList $ map (\(1, !v1, !v2) -> feat (v1, v2)) $ filter ((== 1) . fst3) queries

  -- simulate to the end:
  let (!lastMonoEdges, !lastDynEdges) = foldl' step s0 queries
        where
          s0 = (S.difference initialEdges dynEdges, S.intersection initialEdges dynEdges)
          -- add dyn
          step (!monos, !dyns) (1, !v1, !v2) = (monos, S.insert (feat (v1, v2)) dyns)
          -- delete mono or dynamic
          step (!monos, !dyns) (2, !v1, !v2)
            -- delete dyn
            | S.member (feat (v1, v2)) dynEdges = (monos, S.delete (feat (v1, v2)) dyns)
            -- delete mono
            | otherwise = (S.delete (feat (v1, v2)) monos, dyns)
          -- TODO: debug assert it is contained
          -- ask
          step acc (3, !_, !_) = acc

  let !_ = dbg ("dyn", dynEdges)
  let !_ = dbg ("mono", lastMonoEdges)
  let !_ = dbg ("last dyn", lastDynEdges)

  -- start from the end:
  !uf <- newMUF (nVerts + 1)

  forM_ lastMonoEdges $ \(!v1, !v2) -> do
    uniteMUF uf v1 v2

  -- answer from the end:
  let m = foldM step s0 (reverse queries)
        where
          s0 = ([], lastDynEdges)

          -- delete dyn (do the inverse of add dyn)
          step (!acc, !dyns) (1, !v1, !v2) =
            return $ (acc, S.delete (feat (v1, v2)) dyns)

          -- add (do the inverse of deletion)
          step (!acc, !dyns) (2, !v1, !v2)
            -- ddd dyn
            | S.member (feat (v1, v2)) dynEdges = do
              return (acc, S.insert (feat (v1, v2)) dyns)
            -- ddd mono
            | otherwise = do
              uniteMUF uf v1 v2
              return (acc, dyns)
          ...
          -- answer
          step (!acc, !dyns) (3, !v1, !v2) = do
            !r1 <- rootMUF uf v1
            !r2 <- rootMUF uf v2
            if r1 == r2
              then return (True : acc, dyns)
              else do
                !dynRoots <- mapM (\(!v1, !v2) -> (,) <$> (rootMUF uf v1) <*> (rootMUF uf v2)) $ S.elems dyns
                let suf = fromListSUF dynRoots
                let !_ = dbg ("challenge", "roots", (r1, r2), "dynRoots", dynRoots, "same", sameSUF suf r1 r2)
                return (sameSUF suf r1 r2 : acc, dyns)

  -- REMARK: already reversed
  !acc <- fst <$> m
  forM_ acc $ printYn
