...
genMasks :: Int -> Int -> [Int]
genMasks n l = filter ((== l) . popCount) [1 .. shiftL 1 n - 1]

applyMask :: UArray Int Char -> Int -> Int -> Int
applyMask !arr !n !mask = foldString . map (arr !) $ filter (not . testBit mask) [0 .. pred n]

foldString :: [Char] -> Int
foldString = foldl' (\acc c -> 100 * acc + (ord c - ord 'a')) (0 :: Int)

main :: IO ()
main = do
  [nInput, l, maskLen] <- ints

  !arrs <- replicateM nInput $ do
    listArray @UArray (0, pred l) <$> getLine :: IO (UArray Int Char)

  when (maskLen == 0) $ do
    let !res = maximum . map length . group . sort $ map (foldString . elems) arrs
    print res
    exitSuccess

  let !masks = genMasks l maskLen
  -- let !_ = dbg (masks)

  let !res = maximum $ map f masks
        where
          f :: Int -> Int
          f mask = maximum . IM.elems . IM.fromListWith (+) $ map ((, 1) . g) arrs
            where
              g arr = applyMask arr l mask
