...
rotateH :: VM.MVector (PrimState IO) (Seq.Seq Char) -> Int -> Char -> IO Char
rotateH !grid !row !ins = do
  !dropped <- (\(_ Seq.:|> last) -> last) <$> VM.unsafeRead grid row
  VM.unsafeModify grid (\(seq Seq.:|> _) -> ins Seq.<| seq) row
  return dropped

rotateV :: VM.MVector (PrimState IO) (Seq.Seq Char) -> Int -> Char -> IO Char
rotateV !grid !col !ins = do
  let !n = VM.length grid
  !dropped <- (\seq -> Seq.index seq col) <$> VM.unsafeRead grid (pred n)

  forM_ [pred n, pred (pred n) .. 1] $ \i -> do
    !lastCh <- (\seq -> Seq.index seq col) <$> VM.unsafeRead grid (pred i)
    VM.unsafeModify grid (Seq.adjust' (const lastCh) col) i
  --

  VM.unsafeModify grid (Seq.adjust' (const ins) col) 0
...
main = do
  ...
  !grid <-
    if easy
      then VM.replicateM h (Seq.fromList <$> getLine)
      else do
        !grid' <- transpose <$> replicateM h getLine
        V.unsafeThaw . V.fromList $ map Seq.fromList grid'

  let step :: [String] -> IO ()
      step ["1", !row_, !ch_] = do
        let row = pred (read row_) :: Int
        !dropped <- rotateH grid row (head ch_)
        putChar dropped
      step ["2", !col_, !ch_] = do
        let col = pred (read col_) :: Int
        !dropped <- rotateV grid col (head ch_)
        putChar dropped
  ...
  replicateM_ q $ do
    !query <-
      if easy
        then words <$> getLine
        else (\case ["1", a, b] -> ["2", a, b]; ["2", a, b] -> ["1", a, b]; x -> x) . words <$> getLine
