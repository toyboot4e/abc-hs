...
main = do
  ...
  !pills <- V.replicateM nPills do
    !_nAllergen <- readLn @Int
    IS.fromList <$> ints

  !nQueries <- readLn @Int
  !queries <- replicateM nQueries $ do
    !_nAllergens <- readLn @Int
    IS.fromList <$> ints

  let !_ = dbg (pills)
  let !_ = dbg (queries)

  forM_ queries $ \is1 -> do
    let !iv = V.maximumBy (comparing snd) $ V.imap (\i is2 -> if IS.disjoint is1 is2 then (succ i, values VU.! i) else (-1, -1)) pills
    print $ fst iv
