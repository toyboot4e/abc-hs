...
main = do
  ...
  -- REMARK: zero-based
  !xs <- intsVU
  !rvls <- accumArray @Array (flip (:)) [] (1, n) . map (\(!v, !l, !r) -> (r, (v, l))) <$> replicateM m ints3

  let !_ = dbg (rvls)

  !stree <- newSTreeVU (min) (succ n) (0 :: Int)

  forM_ (assocs rvls) $ \(!r, !vls) -> do
    -- single
    !lastX <- fromJust <$> querySTree stree (pred r, pred r)
    insertSTree stree r (lastX + xs VU.! pred r)

    -- set
    forM_ vls $ \(!v, !l) -> do
      !v' <- (v +) . fromJust <$> querySTree stree (max 0 (pred l), r)
      modifySTree stree (min v') r

  !res <- fromJust <$> querySTree stree (n, n)
  print res
