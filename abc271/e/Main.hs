...
myDj :: forall a. (Num a, Ord a) => (Int -> [H.Entry a Int]) -> Int -> IM.IntMap a
myDj !getVerts !start = inner (H.singleton $! H.Entry 0 start) IM.empty
  where
    merge :: H.Entry a Int -> H.Entry a Int -> H.Entry a Int
    merge (H.Entry !cost1 !_v1) (H.Entry !cost2 !v2) = H.Entry (cost1 + cost2) v2
    ...
    inner !heap !vis
      ...
      where
        ...
        -- push neighbors
        vs = map (merge entry) $! filter ((`IM.notMember` vis') . H.payload) $! getVerts v
        heap'' = foldl' (flip H.insert) heap' vs

main :: IO ()
main = do
  [nVerts, nEdges, nConstraints] <- getLineIntList
  !edges <- VU.replicateM nEdges ((\[!v1, !v2, !c] -> (v1 - 1, v2 - 1, c)) <$> getLineIntList)
  !input <- VU.map pred <$> getLineIntVec

  let !result = VU.foldl' step s0 input
      s0 = IM.singleton 0 (0 :: Int)
      step !im iEdge = case (cKeep, cNew) of
        (Just c1, Just c2) -> IM.insert v2 (min c1 c2) im
        (Just c1, _) -> IM.insert v2 c1 im
        (_, Just c2) -> IM.insert v2 c2 im
        (_, _) -> im
        where
          (!v1, !v2, !c) = edges VU.! iEdge
          !cKeep = IM.lookup v2 im
          !cNew = (+ c) <$> IM.lookup v1 im

  print $ fromMaybe (-1) $ IM.lookup (pred nVerts) result
