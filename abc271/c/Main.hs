...
solve n xs = f (0, 0) 0
  where
    f (nRead, nDrop) i =
      -- let !_ = traceShow (nRead, nDrop, i) () in
      if
          | i == VU.length xs -> (n - nConsumed) `div` 2
          | n - nConsumed == 0 -> 0
          | n - nConsumed == 1 ->
            -- let !_ = traceShow (xs VU.! i, nRead, nDrop, i) () in
            if xs VU.! i == succ nRead then 1 else 0
          | otherwise ->
            if xs VU.! i == succ nRead
              then 1 + f (nRead + 1, nDrop) (succ i)
              else 1 + f (nRead + 1, nDrop + 2) i
      where
        nConsumed = nRead + nDrop `div` 2

main :: IO ()
main = do
  [n] <- getLineIntList
  -- REMARK: We have to consider duplciate items
  xs <- VU.fromList . compress . sort <$> getLineIntList
