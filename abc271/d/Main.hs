...
main = do
  ...
  let dp = foldl' step s0 input
      ...
      step xs [fa, ba] =
        merge
          (filter ((<= s) . (\(f, _, _) -> f)) $ map (\(f, b, log_) -> (f + fa, b + ba, True : log_)) xs)
          (filter ((<= s) . (\(f, _, _) -> f)) $ map (\(f, b, log_) -> (f + ba, b + fa, False : log_)) xs)
      step _ _ = error "unreachable"
      merge xs [] = xs
      merge [] ys = ys
      merge xs@(x@(xf, xb, xlog) : xrest) ys@(y@(yf, yb, ylog) : yrest) = case compare xf yf of
        LT -> x : merge xrest ys
        GT -> y : merge xs yrest
        EQ -> x : merge xrest yrest

  let (fa, fb, log_) = last dp

  if fa == s
    then do
      putStrLn "Yes"
      putStrLn $ concatMap (\x -> if x then "H" else "T") (reverse log_)
    else putStrLn "No"
