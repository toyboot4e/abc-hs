...
solve = do
  ...
  -- Each character should be used just once
  let warps :: V.Vector (U.Vector (Int, Int))
      warps =
        V.map U.fromList
  ...
  used <- UM.replicate 26 False

  let res = Gr.bfs (h, w) grF (-1 :: Int) (U.singleton ((0, 0), 0 :: Int))
        where
          grF :: (Int, Int) -> U.Vector ((Int, Int), Int)
          grF (!y, !x) = U.map (,1) $ warp U.++ adj4
            where
              adj4 = U.filter ((&&) <$> inRange (zero2 h w) <*> ((/= '#') . (gr @!))) $ U.map (add2 (y, x)) ortho4
              warp = unsafePerformIO $ do
                case gr @! (y, x) of
                  '#' -> pure U.empty
                  '.' -> pure U.empty
                  ch -> do
                    let !ch' = ord ch - ord 'a'
                    b <- GM.exchange used ch' True
                    pure $ if b
                      then U.empty
                      else U.filter (/= (y, x)) $ warps G.! ch'

  printBSB $ U.last res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc436/tasks/abc436_d
main :: IO ()
main = runIO solve
