...
-- 本来あるべき場所に既にある: swap の対象外
-- 本来あるべき場所が入れ替わっている: 互いに入れ替わっている
...
solve = do
  ...
  dsu <- Dsu.new n
  U.iforM_ ps $ Dsu.merge_ dsu

  -- let gr = Gr.build' n $ U.imap (,) ps
  let res = U.sum <$> U.imapM f ps
        where
          f i desired
            | i == desired = pure 0
            | ps G.! desired == i = pure 1
            | otherwise = pred <$> Dsu.size dsu i

  printBSB . (`div` 2) =<< res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc436/tasks/abc436_e
main :: IO ()
main = runIO solve
