...
solve = do
  ...
  let !res = snd $ EV.mapAccumL step s0 (U.generate (n * n - 1) (+ 2))
        where
          s0 = (S.singleton (0, n `div` 2), (0, n `div` 2))
          step (!yxs, (!r, !c)) d = ((yxs', yx), (yx, d))
            where
              (!r1, !c1) = ((r - 1) `mod` n, (c + 1) `mod` n)
              (!r2, !c2) = ((r + 1) `mod` n, c)
              yx
...
(r,c)、書き込んだ整数を k としたとき、マス ((r−1)modN,(c+1)modN) が空白ならばそのマスに、そうでなければマス ((r+1)modN,c) に k+1 を書き込む。 ここで、xmodN は x を N で割ったあまりを表す。
-}

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc436/tasks/abc436_b
main :: IO ()
main = runIO solve
