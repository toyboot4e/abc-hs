...
solve = do
  ...
  let toIndex = U.accumulate (\_ x -> x) (U.replicate n (-1 :: Int)) $ U.imap (flip (,)) xs
  let !_ = xs
  let !_ = toIndex

  let takeMex set (!l, !r)
        | iMex < l =
            let targets = U.take (l - iMex) $ U.drop iMex xs
                set' = U.foldl' (flip S.delete) set targets
             in (set', (iMex, r))
        | iMex > r =
            let targets = U.take (iMex - r) $ U.drop (r + 1) xs
                set' = U.foldl' (flip S.delete) set targets
             in (set', (l, iMex))
        | otherwise = error "unreachable"
        where
          mex = fromJust $ S.lookupMin set
          iMex = toIndex G.! mex

  let i0 = fromJust $ U.elemIndex 0 xs
  let lrs = fst3 $ until p f s0
        where
          -- s0 :: (Int, S.Set Int, (Int, Int))
          s0 = ([(i0, i0 - 1)], S.fromList [1 .. n - 1], (i0, i0))
          -- p :: (Int, S.Set Int, (Int, Int)) -> Bool
          p (!_, !_, (!l, !r)) = (l, r) == (0, n - 1)
          -- f :: (Int, S.Set Int, (Int, Int)) -> (Int, S.Set Int, (Int, Int))
          f (!acc, !set, (!l, !r)) =
            let (!s', !lr') = takeMex set (l, r)
             in (lr' : acc, s', lr')

  let !_ = dbg lrs
  let !res = sum $ zipWith f ((0, n - 1) : lrs) lrs
        where
          f (!l', !r') (!l, !r) = (abs (l' - l) + 1) * (abs (r' - r) + 1)
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc436/tasks/abc436_f
main :: IO ()
main = runIO solve
