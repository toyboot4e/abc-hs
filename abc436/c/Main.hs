...
solve = do
  (!n, !m) <- ints2P
  !rcs <- U.replicateM m ints2P

  let !res = U.foldl' step S.empty rcs
        where
          -- . . .
          -- . x .
          -- . . .
          step yxs (!y, !x)
            | conflicts = yxs
            | otherwise = S.insert (y, x) yxs
            where
              conflicts = or $ do
                y' <- [y - 1 .. y + 1]
                x' <- [x - 1 .. x + 1]
                pure $ S.member (y', x') yxs

  printBSB $ S.size res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc436/tasks/abc436_c
main :: IO ()
main = runIO solve
