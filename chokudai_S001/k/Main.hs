...
-- | Creates a new segment tree for `n` leaves with given zero values.
...
newSTree !f !n !zero = MSegmentTree f <$!> VUM.replicate n' zero
  where
...
dictOrderModuloVU :: VU.Vector Int -> Int -> Int
dictOrderModuloVU xs modulus = runST $ do
  !stree <- newSTree (+) (VU.length xs + 1) (0 :: Int)

  -- Pre-calculate factorial numbers:
  let !facts = factMods (VU.length xs) modulus
  ...
  !counts <- flip VU.imapM xs $ \i x -> do
    !nUsed <- querySTree stree (0, x)
    let !nUnused = x - nUsed
    let !factMod = facts VU.! (VU.length xs - (i + 1))
  ...
  return $ succ $ VU.foldl1' (\ !acc x -> (acc + x) `rem` modulus) counts

main :: IO ()
main = do
  [n] <- getLineIntList
  !xs <- VU.map pred <$> getLineIntVec
  print $ dictOrderModuloVU xs 1_000_000_007
