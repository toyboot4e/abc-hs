...
-- Returns compressed indices.
compressVU :: VU.Vector Int -> VU.Vector Int
compressVU xs =
  let !ixs = VU.zip (VU.fromList [0 .. pred (VU.length xs)]) xs
      !ixs' = VU.map fst $ VU.modify (VAI.sortBy (comparing snd)) ixs
   in ixs'
...
main = do
  [n] <- getLineIntList
  !xs <- getLineIntVec
  let !xs' = compressVU xs

  let !nSwap = invNumVU xs'
  let !result = even (n - nSwap)
