...
solve = do
  (!n, !m, !k) <- ints3P
  !xs <- intsP
  pvs <- U.replicateM m ((,) <$> int1P <*> intP)
  printBSB . U.length . U.filter (< k) $ U.accumulate (\ _ x -> x) xs pvs

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0005/tasks/awc0005_b
main :: IO ()
main = runIO solve
