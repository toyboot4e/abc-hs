...
solve = do
  !n <- int'
  !dangerouses <- line'
  let mask = bit n - 1 :: Int
  let isBlock =
        U.accumulate
          (||)
          (U.replicate (bit n) False)
          . U.map ((,True) . succ)
          . U.elemIndices '1'
          . U.fromList
          $ BS.unpack dangerouses

  -- bfs from 0 to (bit n - 1)
  let res =  genericBfs grF (bit n) 0 (-1 :: Int)
        where
          grF i = U.map ((,1) . setBit i) bits
            where
              bits = U.filter (\iBit -> not (testBit i iBit) && not (isBlock G.! setBit i iBit)) $ U.generate n id

  printYn . (/= -1) . U.last $ res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc415/tasks/abc415_c
main :: IO ()
main = runIO $ do
  t <- int'
  replicateM_ t solve
