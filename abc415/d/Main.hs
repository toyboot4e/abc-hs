...
solve = do
  ...
  let !xs' = dbgId $ U.modify (VAI.sortBy (comparing swap)) xs
  let !xs'' = note "strategy" . U.fromList . map U.minimum $ U.groupBy ((==) `on` snd) xs'

  -- apply xs'' in a greedy approach
  let !res = snd $ U.foldl' step s0 xs''
        where
          s0 = dbgId (n, 0 :: Int)
          step (!rest, !score) (!req, !cost)
            | req > rest = (rest, score)
            | otherwise = (rest - dn * cost, score + dn)
            where
              -- rest - dn * cost >= req
              -- rest - req >= dn * cost
              -- (rest - req) / cost >= dn
              dn = dn1 + dn2
              dn1 = (rest - req) `div` cost
              dn2
                | rest - dn1 * cost >= req && rest - (dn1 + 1) * cost >= 0 = 1
                | otherwise = 0
              !_ = dbg (dn, (rest, score), (req, cost))

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc415/tasks/abc415_d
main :: IO ()
main = runIO solve
