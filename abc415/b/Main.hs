...
solve = do
  !s <- line'
  let !is = U.map succ . U.elemIndices '#' $ U.fromList $ BS.unpack s
  printBSB
    . unlinesBSB
    . V.unfoldr
      ( \xs -> do
          guard . not $ U.null xs
          pure . first (\x -> showBSB (U.head x) <> showBSB "," <> showBSB (U.last x)) $ U.splitAt 2 xs
      )
    $ U.convert is

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc415/tasks/abc415_b
main :: IO ()
main = runIO solve
