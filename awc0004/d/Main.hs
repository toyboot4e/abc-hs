...
test :: Int -> U.Vector (Int, Int) -> Bool
test n lrs = runST $ do
  seg <- Seg.build $ U.generate n (Min @Int)
  res <- newMutVar True
  U.forM_ lrs $ \(!l, !r) -> do
    m <- fromMaybe mempty <$> Seg.prodMaybe seg l (r + 1)
    if m == mempty
      then writeMutVar res False
      else Seg.write seg (getMin m) mempty
  readMutVar res

solve :: StateT BS.ByteString IO ()
solve = do
  (!n, !m) <- ints2P
  !lrs <- U.replicateM m ints11P

  let lrs1 = dbgId $ U.modify (VAI.sortBy (comparing (\(!l, !r) -> (r, l)))) lrs
  let lrs2 = dbgId $ U.modify VAI.sort lrs
  printYn $ n >= m && any (test n) [lrs1, lrs2]

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0004/tasks/awc0004_d
main :: IO ()
main = runIO solve
