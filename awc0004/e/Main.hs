...
solve = do
  ...
  let s = dbgId $ U.sum xs
  let cntL = dbgId $ U.init $ U.scanl' (+) (0 :: Int) xs
  -- TODO: better way to create this
  let cntR = dbgId . V.map snd . V.scanr' (\x (!acc, !ms) -> (x + acc, incMS (x + acc) ms)) (0 :: Int, singletonMS 0) $ U.convert xs

  -- 0  1  2  3  4  5
  let res =
        U.imap
          ( \i l ->
              let rs = innerMS $ cntR G.! (i + 1)
                  !_ = dbg (i, (l, s - k - l), rs)
               in fromMaybe 0 $ rs IM.!? (s - k - l)
          )
          cntL

  printBSB $ U.sum res

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0004/tasks/awc0004_e
main :: IO ()
main = runIO solve
