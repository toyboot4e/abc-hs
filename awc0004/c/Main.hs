...
solve = do
  ...
  let minH = U.minimum hs
  let maxH = U.maximum hs

  if signum minH == signum maxH
    then printBSB $ 2 * max (abs maxH) (abs minH)
    else printBSB $ 2 * abs (maxH - minH)

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0004/tasks/awc0004_c
main :: IO ()
main = runIO solve
