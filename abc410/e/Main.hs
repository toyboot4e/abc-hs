...
solve = do
  (!n, !w, !h) <- ints3'
  !dxys <- U.replicateM n ints2'

  let !undef = maxBound `div` 2 :: Int
  let !result = fst $ U.ifoldl' step s0 dxys
        where
          s0 = (0 :: Int, U.generate (w + 1) (\case 0 -> 0; _ -> undef))
          step (!acc, !sofar) iStep (!dx, !dy) = (acc', sofar')
            where
              acc'
                | not (U.null fromX) || not (U.null fromY) = iStep + 1
                | otherwise = acc
              sofar' = U.accumulate min (U.replicate (w + 1) undef) $ fromX U.++ fromY
              fromX = U.imapMaybe f $ U.take (w + 1 - dx) sofar
                where
                  f lastX lastY
                    | lastY == undef = Nothing
                    | otherwise = Just (lastX + dx, lastY)
              fromY = U.imapMaybe f sofar
                where
                  f x lastY
                    | lastY == undef = Nothing
                    | lastY + dy > h = Nothing
                    | otherwise = Just (x, lastY + dy)
