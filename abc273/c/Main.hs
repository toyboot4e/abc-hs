...
-- TODO: look into the code and try running it
-- [lower, upper]
...
-- `compress` duduplicates sorted list, while `nub` deduplicates non-sorted list
-- TODO: std?
compress []     = []
...
main = do
  [n] <- getLineIntList
  asList <- getLineIntList

  let asVec = VU.fromList asList
  let asSortedVec = VU.fromList $ compress $ sort asList

  forM_ [0..(n - 1)] $ \i -> do
    let a = asVec VU.! i

    let nAccIndex = bsearch (\index -> (asSortedVec VU.! index) >= a) 0 (n - 1)
    let nRest = n - (nAccIndex + 1)
    print nRest
