...
main = do
  ...
  let !kinds = VU.fromList $ nubSort input

  let countKind !kind =
        let Just !l = fst $ bsearch (0, pred $ vLength kinds) p
            !count = vLength kinds - 1 - l
            !_ = dbg ("kind", kind, "len", l, "count", count, input, kinds)
         in count
        where
          p !i = kinds VU.! i <= kind

  -- nKind -> kind
  let !counts = IM.fromList $ VU.toList $ VU.map (\kind -> (countKind kind, kind)) kinds
  let !_ = dbg (counts)

  -- kind -> number of input
  let !nInput = IM.fromListWith (+) $ map (,1 :: Int) input

  forM_ [0 .. pred n] $ \nKind -> do
    let !result = fromMaybe 0 $ do
          !kind <- IM.lookup nKind counts
          return $ nInput IM.! kind
    print result
