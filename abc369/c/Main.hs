...
solve = do
  ...
  let goRight i0
        | i0 == n - 1 = i0
        | otherwise = inner (i0 + 1)
        where
          dx = xs G.! (i0 + 1) - xs G.! i0
          inner i
            | i == n || x2 - x1 /= dx = i
            | otherwise = inner (i + 1)
            where
  ...
  let resolve i
        | i >= n - 1 = 0
        | i == i' = i + 1
        | otherwise = cnt + resolve (max (i + 1) (i' - 1))
        where
          i' = goRight i
          cnt = ((i' - i) * (i' - i - 1)) `div` 2
          !_ = dbg (i, i', cnt)

  printBSB . (+ n) $ resolve 0

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc369/tasks/abc369_c
main :: IO ()
main = runIO solve
