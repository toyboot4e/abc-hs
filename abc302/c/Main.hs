...
isCompatible s1 s2 = (== 1) . length . filter (uncurry (/=)) $ zip s1 s2

isCompatiblePerm :: [String] -> Bool
isCompatiblePerm (s1:ss) = fst $ foldl' step (True, s1) ss
  where
    step (False, _) _ = (False, [])
    step (True, s0) sx = (isCompatible sx s0, sx)

main :: IO ()
main = do
  [nStrings, len] <- ints
  !ss <- V.replicateM nStrings getLine
  !is <- V.thaw . V.fromList $ [0 .. pred nStrings]

  let !nTry = 8 * 7* 6 * 5 * 4 * 3 * 2 -- omg.

  forM_ [1 .. nTry] $ \_ -> do
    !is' <- V.unsafeFreeze is
    let !ss' = V.map (ss V.!) is'
    let !_ = dbg (is')
    let !_ = dbg (ss')

    when (isCompatiblePerm $ V.toList ss') $ do
      putStrLn "Yes"
      exitSuccess

    VM.nextPermutation is
    return ()
