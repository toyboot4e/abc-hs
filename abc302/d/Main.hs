...
-- | Two pointers over the same array.
...
main = do
  [n1, n2, d] <- ints
  !xs1 <- VU.modify VAI.sort <$> intsVU
  !xs2 <- VU.modify VAI.sort <$> intsVU

  let !results = VU.map f xs1
        where
          f x1 = case bsearchL (0, pred n2) $ \i2 -> xs2 VU.! i2 - x1 <= d of
            Just i2 ->
              let !x2 = xs2 VU.! i2
               in if abs (x1 - x2) <= d
                     then x1 + xs2 VU.! i2
