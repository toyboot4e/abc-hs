...
insertS :: Int -> Int -> (Int, IM.IntMap IS.IntSet) -> (Int, IM.IntMap IS.IntSet)
insertS v1 v2 (!n, !im) =
  if IM.notMember v1 im
    then (succ n, IM.insert v1 (IS.singleton v2) im)
    else (n, IM.adjust (IS.insert v2) v1 im)

deleteS :: Int -> Int -> (Int, IM.IntMap IS.IntSet) -> (Int, IM.IntMap IS.IntSet)
deleteS k v (!n, !im) =
  let !is = IS.delete v $ im IM.! k
   in if IS.null is
        then (pred n, IM.delete k im)
        else (n, IM.insert k is im)

main :: IO ()
main = do
  [!nVerts, !nQueries] <- ints
  !queries <- V.replicateM nQueries ints

  let !res = V.foldl' step ([], (0 :: Int, IM.empty)) queries
        where
          step (!acc, graph) [1, !v1, !v2] =
            let graph' = insertS v2 v1 $ insertS v1 v2 graph
             in (fst graph' : acc, graph')
          step (!acc, (!n, !im)) [2, !v] =
            if IM.member v im
              then
                let n' = pred n
                    v2s = im IM.! v
                    im' = IM.delete v im
                    graph' = foldl' (\gr v2 -> deleteS v2 v gr) (n', im') $ IS.toList v2s
                 in (fst graph' : acc, graph')
              else (n : acc, (n, im))
  ...
  let !results = map (nVerts -) . reverse $ fst res
  forM_ results print
