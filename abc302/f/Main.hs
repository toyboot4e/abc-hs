...
main = do
  [nSets, maxX] <- ints
  !sets <- replicateM nSets $ do
    !_ <- getLine
    -- REMARK: 0-based
    map pred <$> ints

  -- 1. [x] BFS / 2
  -- 2. [x] Nonsense Dijkstra (IntMap)
  -- 3. [x] Nonsense Dijkstra (Vector)
  -- 4. [ ] 01-BFS
  ...
  -- slower
  -- let !edges = concatMap edges
  --       where
  --         f (!iSet, !xs) = concatMap (\x -> [(set0 + iSet, x0 + x), (x0 + x, set0 + iSet)]) xs

  -- TODO: easier fold template?
  let !edges = foldl' f [] $ zip [0 ..] sets
        where
          f !acc0 (!iSet, !xs) =
            foldl' (\acc x -> (set0 + iSet, H.Entry (0 :: Int) (x0 + x)) : (x0 + x, H.Entry 1 (set0 + iSet)) : acc) acc0 xs

  let !graph = accumArray @Array (flip (:)) [] (0, nSets + maxX - 1) edges
  let !result = djVec graph x0 (-1 :: Int)
  let !_ = dbg (result)

  print $ case result VU.! xLast of
    (-1) -> -1
    x -> pred x
