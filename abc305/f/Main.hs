...
getVerts v = do
  ...
  getLine >>= \case
    "OK" -> exitSuccess
    "-1" -> exitSuccess
    line -> return $ map read . tail . words $ line
...
main = do
  ...
  !vis <- VUM.replicate (succ nVerts) False
  VUM.write vis 1 True

  flip fix (1, vs0) $ \loop (!v, !vs) -> do
    -- on end
    !isEnd <- VUM.read vis nVerts
    when isEnd $ do
      void $ getVerts nVerts
    ...
    forM_ vs $ \v2 -> do
      ...
      unless visited $ do
        -- go into
        VUM.write vis v2 True
        !vs' <- filterM (fmap not . VUM.read vis) =<< getVerts v2
