...
main = do
  (h, w) <- ints2
  !grid <- listArray @UArray ((0, 0), (pred h, pred w)) . concat <$> replicateM h getLine :: IO (UArray (Int, Int) Char)

  let !is = [(y, x) | y <- [0 .. pred h], x <- [0 .. pred w]]
  let !blocks = filter ((== '#') . (grid !)) is

  let !blockMax = foldl' (\acc yx -> (max (fst acc) (fst yx), max (snd acc) (snd yx))) (-1, -1) blocks :: (Int, Int)
  let !blockMin = foldl' (\acc yx -> (min (fst acc) (fst yx), min (snd acc) (snd yx))) (h, w) blocks :: (Int, Int)

  let !candidates = [(y, x) | y <- [fst blockMin .. fst blockMax], x <- [snd blockMin .. snd blockMax]]
  let !result = fromJust $ find ((== '.') . (grid !)) candidates

  let (!y, !x) = both succ $ result
