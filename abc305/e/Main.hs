...
solve :: Array Vertex [Vertex] -> (H.Heap (H.Entry (Down Int) Vertex)) -> VU.Vector Bool
solve !graph !heap0 = VU.create $ do
  !vis <- VUM.replicate nVerts False

  flip fix heap0 $ \loop heap -> case H.uncons heap of
    Nothing -> return ()
    Just (H.Entry (Down hp) v, heap') -> do
      let !_ = dbg ("go", v, heap')
      !visited <- VUM.read vis v
      if visited
        then loop heap'
        else do
          let !_ = dbg (" >> mark", v, hp)
          ...
          if hp > 0
            then do
              !entries <- map (\v2 -> H.Entry (Down $ pred hp) v2) <$> (filterM (fmap not . VUM.read vis) $ graph ! v)
              let !_ = dbg ("next", v, "->", entries)
              loop $ foldl' (flip H.insert) heap' entries
            else do
              loop heap'
...
main :: IO ()
main = do
  [nVerts, nEdges, k] <- ints
  !edges <- replicateM nEdges (both pred <$> ints2)
  let !_ = dbg ("edges", edges)

  let !graph = accumArray @Array (flip (:)) [] (0, pred nVerts) $ concatMap2 (second swap . dupe) edges
  let !_ = dbg ("graph", graph)

  !entries <- fmap H.fromList . replicateM k $ do
    (!p, !h) <- ints2
    return $ H.Entry (Down h) (pred p)

  let !_ = dbg (entries)
  let !res = solve graph entries
  let !_ = dbg (res)

  let !verts = VU.map (succ . fst) . VU.filter snd $ (VU.zip (rangeVU 0 (pred nVerts)) res)

  print $ VU.length verts
  putStrLn . unwords . map show $ VU.toList verts
