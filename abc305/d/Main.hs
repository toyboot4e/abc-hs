...
intersectionOf :: (Int, Int) -> (Int, Int) -> Int
intersectionOf (!l1, !r1) (!l2, !r2)
  | r1 <= l2 = 0
  | r2 <= l1 = 0
  | otherwise = (min r1 r2) - (max l1 l2)

getMid :: VU.Vector Int -> (Int, Int) -> Int
getMid csums (!l, !r)
  | l > r = 0
  | otherwise = csums VU.! (r + 1) - csums VU.! l

solve :: VU.Vector (Int, Int) -> VU.Vector Int -> (Int, Int) -> Int
solve !spans !csum (!l, !r) = fromL + fromR + mid
  where
    !n = VU.length spans
    -- intersection
    !l' = bsearchL (0, pred n) $ \i -> let (!sl, !_) = spans VU.! i in sl <= l
    !r' = bsearchR (0, pred n) $ \i -> let (!_, !sr) = spans VU.! i in sr < r
    !fromL = maybe 0 (intersectionOf (l, r) . (spans VU.!)) l'
    !fromR
      | l' == r' = 0 -- do not duplicate the count!!!
      | otherwise = maybe 0 (intersectionOf (l, r) . (spans VU.!)) r'
    -- mid
    !mid =
      let !lm = maybe 0 succ l'
          !rm = maybe (pred n) pred r'
          !_ = dbg ("query", (l, r), (l', r'), (lm, rm), (fromL, getMid csum (lm, rm), fromR))
       in getMid csum (lm, rm)
...
main = do
  ...
  let !spans = VU.fromList $ map tuple2 $ filter ((== 2) . length) $ chunks 2 $ tail xs
  let !csum = VU.scanl' (\acc (!l, !r) -> acc + r - l) (0 :: Int) spans
  let !_ = dbg (spans)
  let !_ = dbg (csum)

  [q] <- ints
  queries <- replicateM q ints2

  forM_ queries $ \(!l, !r) -> do
    print $ solve spans csum (l, r)
