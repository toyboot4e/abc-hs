...
csumRing :: Int -> U.Vector Int -> (Int, Int) -> Int
csumRing n csum (!l, !r)
  | l <= r = csum +! (l, r)
  | otherwise = csum +! (l, n - 1) + csum +! (0, r)
...
solve = do
  ...
  U.foldM'_
    (\offset query -> case query of
        (1, !c, !_) -> do
          pure $ (offset + c) `mod` n
        (2, !l, !r) -> do
          printBSB $ csumRing n csum ((l + offset) `mod` n, (r + offset) `mod` n)
          pure offset
        _ -> error "unreachable"
    )
    (0 :: Int)
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc425/tasks/abc425_c
main :: IO ()
main = runIO solve
