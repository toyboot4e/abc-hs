...
solve = do
  ...
  -- no duplicates other than `-1`, then fill in
  let use = filter (/= -1) xs

  if length (nubSort use) /= length use
    then do
      -- duplicates
      printBSB "No"
    else do
      ...
      let rest = [1 .. n] \\ xs
      let !res = U.create $ do
            vec <- U.thaw $ U.fromList xs
            mix <- newMutVar (0 :: Int)
            U.iforM_ (U.fromList xs) $ \i x -> do
              when (x == -1) $ do
                ix <- readMutVar mix
                GM.write vec i $ rest !! ix
                writeMutVar mix $ ix + 1
            pure vec
      printVec res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc425/tasks/abc425_b
main :: IO ()
main = runIO solve
