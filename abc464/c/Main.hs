...
solve = do
  (!n, !m) <- ints2P
  !adbs <- U.replicateM n ((\(!a, !b, !c) -> (a, b - 1, c)) <$> ints3P)

  let evs0 =
        U.modify VAI.sort $
          U.concatMap
            ( \(!a, !d, !b) ->
                U.fromListN 3 [(min 0 (d - 1), INSERT, a), (d, DELETE, a), (d, INSERT, b)]
            )
            adbs

  let res = snd $ EV.mapAccumL step s0 $ U.generate m id
        where
          s0 = (emptyMS2, evs0)
          step (dbgId -> !ms, !evs) i = ((ms', evs'), M.size (snd ms'))
            where
              (!ms', !evs') = popRec i (ms, evs)
          popRec i (!ms, !evs) = case U.uncons evs of
            Just ((!j, INSERT, !x), !evs')
              | j <= i -> popRec i (incMS2 x ms, evs')
            Just ((!j, DELETE, !x), !evs')
              | j <= i -> popRec i (decMS2 x ms, evs')
            _ -> (ms, evs)

  printCol res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc464/tasks/abc464_c
main :: IO ()
main = runIO solve
