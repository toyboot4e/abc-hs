...
solve = do
  ...
  let undef :: Int = -(maxBound `div` 2)
  let bnd = zero2 n 2
  dp <- IxVector bnd <$> UM.replicate (2 * n) undef

  let costAt i c = bool (xs G.! i) 0 (BS.index s i == c)
  let toC S = 'S'
      toC R = 'R'
      toC _ = error "unreachable"

  -- 0: S, 1: R
  writeIV dp (0, S) $ negate $ costAt 0 (toC S)
  writeIV dp (0, R) $ negate $ costAt 0 (toC R)

  -- R -> R, S -> R, R -> S, S -> S
  for_ [1 .. n - 1] $ \i -> do
    for_ [0 .. 1] $ \s1 -> do
      prev <- readIV dp (i - 1, s1)
      for_ [0 .. 1] $ \s2 -> do
        let m = costAt i (toC s2)
        let p = bool 0 (ys G.! (i - 1)) (s1 == R && s2 == S)
        let !x' = prev + p - m
        modifyIV dp (max x') (i, s2)

  x1 <- readIV dp (n - 1, 0)
  x2 <- readIV dp (n - 1, 1)
  printBSB $ max x1 x2

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc464/tasks/abc464_d
main :: IO ()
main = runIO $ do
  n <- intP
  replicateM_ n solve
