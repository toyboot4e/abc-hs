...
solve = do
  (!h, !w, !q) <- ints3P
  !rcxs <- U.replicateM q ((,,) <$> int1P <*> int1P <*> charP)

  let xs = M.insert (w + 1) (-1, 'A') $ U.ifoldr' step s0 rcxs
        where
          s0 = M.empty
          step (dbgId -> i) (!_, !c, !x) m
            | isJust (M.lookupGE c m) = m
            | otherwise = M.insert c (i, x) m

  let ys = M.insert (h + 1) (-1, 'A') $ U.ifoldr' step s0 rcxs
        where
          s0 = M.empty
          step i (!r, !_, !x) m
            | isJust (M.lookupGE r m) = m
            | otherwise = M.insert r (i, x) m
  ...
  for_ [0 .. h - 1] $ \y -> do
    for_ [0 .. w - 1] $ \x -> do
      let (!_, (!iy, !cy)) = fromJust $ M.lookupGE y ys
      let (!_, (!ix, !cx)) = fromJust $ M.lookupGE x xs
      let c = bool cy cx $ ix < iy
      putBSB $ showBSB c
    printBSB ""

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc464/tasks/abc464_e
main :: IO ()
main = runIO solve
