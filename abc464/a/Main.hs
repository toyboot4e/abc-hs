...
solve = do
  !s <- lineP
  let e = length $ filter (== 'E') $ BS.unpack s
  printBSB $ bool "East" "West" $ not (e > BS.length s `div` 2)

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc464/tasks/abc464_a
main :: IO ()
main = runIO solve