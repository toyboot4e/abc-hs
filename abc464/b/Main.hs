...
solve = do
  ...
  let isSpan i y0 y1 x0 x1 = case i of
        0 -> and [gr @! (y0, x) | x <- [x0 .. x1]]
        1 -> and [gr @! (y1, x) | x <- [x0 .. x1]]
        2 -> and [gr @! (y, x0) | y <- [y0 .. y1]]
        3 -> and [gr @! (y, x1) | y <- [y0 .. y1]]

  let proceed :: Int -> Int -> Int -> Int -> Int -> (Int, Int, Int, Int)
      proceed i y0 y1 x0 x1 = case i of
        0 -> (y0 + 1, y1, x0, x1)
        1 -> (y0, y1 - 1, x0, x1)
        2 -> (y0, y1, x0 + 1, x1)
        3 -> (y0, y1, x0, x1 - 1)

  let proceedRec :: Int -> (Int, Int, Int, Int) -> (Int, Int, Int, Int)
      proceedRec i (dbgId -> (!y0, !y1, !x0, !x1))
        | isSpan i y0 y1 x0 x1 = proceedRec i (proceed i y0 y1 x0 x1)
        | otherwise = (y0, y1, x0, x1)

  let p = (0 :: Int, h - 1, 0 :: Int, w - 1)
  let (!y, !y', !x, !x') = proceedRec 3 . proceedRec 2 . proceedRec 1 $ proceedRec 0 p

  for_ [y .. y'] $ \r -> do
    for_ [x .. x'] $ \c -> do
      putBSB . showBSB . bool '#' '.' $ gr @! (r, c)
    printBSB ""

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc464/tasks/abc464_b
main :: IO ()
main = runIO solve
