...
solve = do
  ...
  let !dict = U.modify VAI.sort $ xs U.++ U.map fst qs
  let !len = G.length dict

  let !lmost = U.head dict
  let !rmost = U.last dict
  let !delta = rmost - lmost
  let !csum = csum1D $ U.accumulate (+) (U.replicate len (0 :: Int)) $ U.map ((,1 :: Int) . bindex dict) xs

  printBSB . unlinesBSB . (`U.map` qs) $ \(!x, !k) -> do
    fromJust . bisectR 0 delta $ \d ->
      let !l = x - d
          !r = x + d
          !l' =
            let l1 = fromMaybe 0 $ bsearchL dict (<= l)
             in if dict G.! l1 < l
                  then min (G.length dict - 1) (l1 + 1)
                  else l1
          !r' = bindex dict r
       in csum +! (l', r') < k

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc364/tasks/abc364_d
main :: IO ()
main = runIO solve
