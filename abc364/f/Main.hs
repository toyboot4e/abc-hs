...
instance SemigroupActionWithLength Op Acc where
  ...
  sactWithLength (Op !dx) a@(Acc s) len
    | not dx = a
    | otherwise = Acc len
...
solve = do
  (!n, !q) <- ints2'
  !lrcs <- U.modify (VAI.sortBy (comparing thd3)) <$> U.replicateM q ints110'

  stree <- buildLSTree $ U.replicate (n - 1) (Acc 0)
  res <-
    U.foldM'
      ( \acc (!l, !r, !c) -> do
          deltaCnt <- do
            Acc cnt <- foldAllWithLengthLSTree stree
            -- unify
            unless (l == r) $ do
              sactWithLengthLSTree stree l (r - 1) $ Op True
            Acc cnt' <- foldAllWithLengthLSTree stree
            return $ cnt' - cnt

          return $ acc + (deltaCnt + 1) * c
      )
      (0 :: Int)
      lrcs
