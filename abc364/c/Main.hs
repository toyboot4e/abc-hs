...
solve = do
  ...
  let !nx = fromMaybe (U.length xs) $ U.findIndex (> x) $ U.scanl' (+) (0 :: Int) $ U.modify (VAI.sortBy (comparing Down)) xs
  let !ny = fromMaybe (U.length ys) $ U.findIndex (> y) $ U.scanl' (+) (0 :: Int) $ U.modify (VAI.sortBy (comparing Down)) ys

  printBSB $ min nx ny

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc364/tasks/abc364_c
main :: IO ()
main = runIO solve
