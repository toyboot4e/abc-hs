...
getLineIntV = V.unfoldr (BS.readInt . BS.dropWhile isSpace) <$> BS.getLine

-- | Returns a vector of (original index, set)
readInput :: V.Vector Int -> V.Vector (Int, IS.IntSet)
readInput input = V.fromList $ inner IS.empty input'
  where
    input' = V.toList $ V.zip (vRange 0 (pred (VG.length input))) input
    inner !xs [] = []
    inner !is ((!i, !x) : ixs)
      | IS.member x is = inner is ixs
      | otherwise =
          let !is' = IS.insert x is
           in (i, is') : inner is' ixs

main :: IO ()
main = do
  [n] <- getLineIntList

  !xs <- readInput <$> getLineIntV
  !ys <- readInput <$> getLineIntV
  let !comparisonResults = V.map (\(!a, !b) -> snd a == snd b) $ V.zip xs ys

  let !_ = dbg (xs)
  let !_ = dbg (ys)

  let findMap !i !maps = fromJust $ bsearchL (0, VG.length maps - 1) \j -> fst (maps V.! j) <= i

  !nQueries <- readLn @Int
  !queries <- VU.replicateM nQueries (both pred <$> getTuple2)

  VU.forM_ queries $ \(!ix, !iy) -> do
    let !nx = findMap ix xs
    let !xmap = snd $ xs V.! nx

    let !ny = findMap iy ys
    let !ymap = snd $ ys V.! ny

    let !_ = dbg ("query", ix, iy)
    let !_ = dbg (ix, xmap)
    let !_ = dbg (iy, ymap)

    putStrLn . yn $ nx == ny && comparisonResults V.! nx
