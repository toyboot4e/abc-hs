          ...
          ( \(!cnt, !rmost) (!l, !len) -> do
              if l > rmost + 1
                then Nothing
                  ...
                  let r = l + len - 1
                      intersection = rmost - l + 1
                      rmost' = r + intersection
                      cnt' = cnt + len * (len - 1) `div` 2 + intersection * len
                      !_ = dbg ((l, len), r, intersection, rmost', cnt')
                   in Just (cnt', rmost')
          )
          (0 :: Int, -1 :: Int)
          . U.modify (VAI.sortBy (comparing fst))
          $ U.zip is ns
