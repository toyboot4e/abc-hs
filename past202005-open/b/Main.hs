...
solve = do
  ...
  !qs <- U.replicateM q $
    int' >>= \case
      ...
      2 -> (2 :: Word8,,) <$> int' <*> int'
      _ -> error "unreachable"
  problemScores <- UM.replicate m n
  personSolves <- VM.replicate n IS.empty

  let evalPersonScore iPerson = do
        (sum <$>) . mapM (GM.read problemScores) . IS.elems =<< GM.read personSolves iPerson

  res <- (`U.mapMaybeM` qs) $ \case
    (1, pred -> iPerson, !_) -> do
      Just <$> evalPersonScore iPerson
    (2, pred -> iPerson, pred -> !iProblem) -> do
      s <- GM.read problemScores iProblem
      GM.write problemScores iProblem $ s - 1
      GM.modify personSolves (IS.insert iProblem) iPerson
      pure Nothing
    _ -> error "unreachable"

  printBSB $ unlinesBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/past202005-open/tasks/past202005-open_b
main :: IO ()
main = runIO solve
