...
match :: BS.ByteString -> BS.ByteString -> Maybe Char
match bs1 bs2 = S.lookupMin $ S.intersection s1 s2
  where
    s1 = S.fromList $ BS.unpack bs1
    s2 = S.fromList $ BS.unpack bs2

solve :: StateT BS.ByteString IO ()
solve = do
  !n <- int'
  !rows <- V.replicateM n line'

  let matches = catMaybes . V.toList $ V.zipWith match rows (V.reverse rows)
  printBSB $ if length matches == n then showBSB matches else showBSB (-1 :: Int)

-- verification-helper: PROBLEM https://atcoder.jp/contests/past202005-open/tasks/past202005-open_f
main :: IO ()
main = runIO solve
