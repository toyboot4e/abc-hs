...
toDigitCols :: [String] -> [[String]]
toDigitCols =
  unfoldr
    (\cols -> if null cols then Nothing else Just $ splitAt 4 cols)
    . transpose
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/past202005-open/tasks/past202005-open_d
main :: IO ()
main = do
  !n <- fst . fromJust . BS.readInt <$> BS.getLine
  !rows <- replicateM 5 getLine
  let digitCols = init $ toDigitCols rows
  printBSB $ concatMap ((:[]) . intToDigit . fromJust . (`elemIndex` digits)) digitCols
