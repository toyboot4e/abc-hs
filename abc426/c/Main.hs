...
solve = do
  (!n, !q) <- ints2'
  !xys <- U.replicateM q ints2'

  -- (x, cnt)
  bh <- newMinBH (2 * (n + q))
  U.forM_ (U.generate n ((, 1 :: Int) . succ)) $ insertBH bh
  res <- U.forM xys $ \(!x, !y) -> do
    cnt <- flip fix (0 :: Int) $ \loop cnt -> do
      peekBH bh >>= \case
        Just (!a, !deltaCnt)
          | a <= x -> do
              deleteBH bh
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc426/tasks/abc426_c
main :: IO ()
main = runIO solve
