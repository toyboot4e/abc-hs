...
longest :: BS.ByteString -> Char -> Int
longest bs c =
  maximumOr 0
    . map snd
    . filter ((== c) . fst)
...
solve = do
  ...
  let n0 = BS.length $ BS.filter (== '0') s
  let n1 = BS.length $ BS.filter (== '1') s

  let len0 = longest s '0'
  let len1 = longest s '1'

  let res0 = (n - len0) + (n0 - len0)
  let res1 = (n - len1) + (n1 - len1)

  let !_ = dbg (s, (n0, n1), (len0, len1), (res0, res1))
  printBSB $ min res0 res1

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc426/tasks/abc426_d
main :: IO ()
main = runIO $ do
  n <- int'
  replicateM_ n solve
