...
import qualified Data.Ix.Enum as HT -- range for enum
import qualified Data.List.HT as HT -- groupBy
...
main = do
  ...
  let arr = accumArray @UArray (+) (0 :: Int) (0, 6) input
      input :: [(Int, Int)]
      input = zip (map indexIt [1 ..]) s

  -- let !_ = traceShow (arr) ()
  let nLands = length $ filter head . group $ map (> 0) $ elems arr

  putStrLn $ if head s == 0 && nLands > 1 then "Yes" else "No"
