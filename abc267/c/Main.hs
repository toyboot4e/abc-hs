...
main = do
  ...
  let result = maximum $ map snd $ scanl' step s0 [1 .. n - m]
      s0 = (edge0, acc0)
      edge0 = sum $ map (xs VU.!) [0 .. pred m]
      acc0 = sum $ map (\i -> succ i * xs VU.! i) [0 .. pred m]
      step (edge, acc) i =
        let x = xs VU.! (i + m - 1)
            edge' = edge + x - xs VU.! pred i
            -- !_ = traceShow (i, edge, edge') ()
            -- !_ = traceShow (i, acc, acc - edge + m * x) ()
         in (edge', acc - edge + m * x)
