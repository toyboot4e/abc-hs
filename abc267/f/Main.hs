...
main = do
  [n] <- getLineIntList
  !edges <- replicateM (pred n) (both pred <$> getTuple2)

  let !graph = accumArray @Array (flip (:)) [] (0, pred n) $
        concatMap (\(!v1, !v2) -> [(v1 ,v2), (v2, v1)]) edges

  [q] <- getLineIntList
  !queries <- replicateM q (first pred <$> getTuple2)

  forM_ queries $ \(!v1, !len) -> do
    putStrLn "TODO"
