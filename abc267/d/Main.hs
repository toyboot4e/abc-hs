...
main = do
  ...
  let result = foldl' step s0 input
      s0 = VU.generate (succ m) (\i -> if i == 0 then 0 else minBound @Int)
      step line x = VU.generate (succ m) f
        where
          f 0 = 0
          f 1 = max (line VU.! 1) (line VU.! (1 - 1) + 1 * x)
          f i
            | line VU.! (i - 1) == minBound = minBound
            | otherwise = max (line VU.! i) (line VU.! (i - 1) + i * x)
