...
solve = do
  ...
  let (!resCnt, !resBits) = dbgId $ U.foldl' f (0 :: Int, 0 :: Int) xs
        where
          f (!cnt, !acc) x
            | x .&. k == x = (cnt + 1, acc .|. x)
            | otherwise = (cnt, acc)

  printBSB . (\case 0 -> (-1); x -> x) $ bool resCnt (-1) (resBits /= k)

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0011/tasks/awc0011_c
main :: IO ()
main = runIO solve
