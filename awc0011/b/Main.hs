...
solve = do
  ...
  let bnd = zero2 (h * k) (w * k)
  let gr = createIV $ do
        vec <- IxVector bnd <$> UM.replicate (k * h * k * w) c1
        for_ [0 .. h - 1] $ \y0 -> do
          for_ [0 .. w - 1] $ \x0 -> do
            when (gr0 @! (y0, x0) == '.') $ do
              for_ [0 .. k - 1] $ \dy -> do
                for_ [0 .. k - 1] $ \dx -> do
                  writeIV vec (k * y0 + dy, k * x0 + dx) c2
        pure vec

  printBSB $ showGridBSB' gr

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0011/tasks/awc0011_b
main :: IO ()
main = runIO solve
