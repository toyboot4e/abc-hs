...
solve = do
  (!n, !m)<- ints2P
  (!s, !t) <- ints11P
  !pvs <- U.modify VAI.sort <$> U.replicateM m ((,) <$> int1P <*> intP)

  let (!ps, !vs) = U.unzip pvs
  let is = B.lowerBound ps (min s t)
  let it = B.upperBound ps (max s t)

  printBSB . U.sum . U.backpermute vs $ U.generate (it - is) (+ is)

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0011/tasks/awc0011_a
main :: IO ()
main = runIO solve
