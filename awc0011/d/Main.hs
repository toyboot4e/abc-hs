...
solve = do
  (!n, !q) <- ints2P
  !vs <- intsP
  !ps <- U.replicateM (n - 1) int1P
  qs <- U.replicateM q int1P

  let res = U.constructN n $ \sofar -> case G.length sofar of
        0 -> G.head vs
        len ->
          let p = ps G.! (len - 1)
           in sofar G.! p + vs G.! len

  printBSB . unlinesBSB $ U.backpermute res qs

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0011/tasks/awc0011_d
main :: IO ()
main = runIO solve
