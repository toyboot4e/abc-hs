...
knapsack :: Int -> U.Vector (Int, Int) -> V.Vector (U.Vector Int)
knapsack m = V.scanl' step s0 . U.convert
  where
    s0 = U.generate (m + 1) $ \case 0 -> (0 :: Int); _ -> (-1)
    -- s0 = U.replicate (m + 1) (0 :: Int)
    step !sofar (!dw, !dv) =
      U.accumulate max sofar $
        U.imapMaybe
          ( \w v ->
              if v == -1
                then Nothing
...
solve = do
  (!n, !m) <- ints2P
  wvs <- U.replicateM n ints2P

  let res1 = knapsack m wvs
  let maxV = U.maximum $ G.last res1

  if maxV == -1
    then do
      replicateM_ n (printYn False)
    else do
      let res2 = G.reverse . knapsack m $ G.reverse wvs
      let res' = G.zipWith3 f (U.convert wvs) res1 (G.tail res2)
            where
              f (!dw, !dv) vs1 vs2 = U.or $ U.zipWith (p dv) vs1 (U.drop dw (U.reverse vs2'))
                where
                  -- w2: w1 + w2 <= w - dw, w2 <= w - dw - w1
                  vs2' = U.scanl1' max vs2
              p dv v1 v2 = v1 /= -1 && v2 /= -1 && (v1 + dv + v2) == maxV
    
      G.forM_ res' printYn

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0011/tasks/awc0011_e
main :: IO ()
main = runIO solve
