...
solve = do
  !s <- line'
  let xs = U.fromList $ BS.unpack s
  let start = "(<["
  let end = ")>]"
  let match '(' = (== ')')
      match '[' = (== ']')
      match '<' = (== '>')
      match c = error $ "unreachable " ++ show c

  let (!rest, !res) = U.foldl' step s0 xs
        where
          s0 = ([], True)
          ...
          step ([], True) c
            | c `elem` start = ([c], True)
            | otherwise = ([], False)
          step (stack@(top : rest), True) c
            | c `elem` start = (c : stack, True)
            | match top c = (rest, True)
            | otherwise = ([], False)

  printYn $ res && null rest

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc394/tasks/abc394_d
main :: IO ()
main = runIO solve
