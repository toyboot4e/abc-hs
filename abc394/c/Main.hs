...
solve = do
  !s <- line'
  vec <- U.unsafeThaw . U.fromList $ BS.unpack s

  -- W+C -> A+C
  skip <- UM.replicate (BS.length s) False
  for_ [0 .. BS.length s - 2] $ \i -> do
    b <- GM.read skip i
    unless b $ do
      vec' <- U.drop i <$> U.unsafeFreeze vec
      let ws = U.takeWhile (== 'W') vec'
      when (U.length ws > 0) $ do
        let !_ = dbg (i, ws)
        if vec' G.!? U.length ws == Just 'A'
          then do
            GM.write vec i 'A'
            for_ [1 .. U.length ws] $ \j -> do
              GM.write vec (i + j) 'C'
          else do
            for_ [0 .. U.length ws - 1] $ \j -> do
              GM.write skip (i + j) True

  res <- U.unsafeFreeze vec
  printBSB $ U.toList res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc394/tasks/abc394_c
main :: IO ()
main = runIO solve
