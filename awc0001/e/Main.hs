...
solve = do
  ...
  let res = fst $ U.foldl' step s0 $ U.zip (U.replicate k h U.++ hs) hs
        where
          s0 = (0 :: Int, (1 :: Int, IM.singleton h k))
          step (!acc, !ms) (!del, !ins) = (max acc (maxX - minX), ms')
            where
              !ms' = decMS del $ incMS ins ms
              (!minX, !_) = IM.findMin $ innerMS ms'
              (!maxX, !_) = IM.findMax $ innerMS ms'

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0001/tasks/awc0001_e
main :: IO ()
main = runIO solve
