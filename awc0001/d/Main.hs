...
solve = do
  (!n, !m, !k) <- ints3P
  !abs <- U.replicateM n ints2P

  -- TODO: Don't use segment tree?
  -- segs[gain][pos]
  segs <- V.generateM (m + 1) $ \i ->
    if i == 0
      then Seg.build $ U.generate (n + 1) $ bool mempty (Max (0 :: Int)) . (== 0)
      else Seg.build $ U.replicate (n + 1) mempty

  U.iforM_ abs $ \((+ 1) -> iTo) (!gain, !loss) -> do
    for_ [0 .. m - loss] $ \powerFrom -> do
      let powerTo = powerFrom + loss
      -- REMARK: The initial position is arbitrary
      acc <- max (Max 0) <$> Seg.prod (segs G.! powerFrom) (max 0 (iTo - k)) iTo
      Seg.modify (segs G.! powerTo) (<> (acc + Max gain)) iTo

  -- TODO: foldMapM?
  printBSB . getMax . V.foldl' (<>) mempty =<< V.mapM Seg.allProd segs

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0001/tasks/awc0001_d
main :: IO ()
main = runIO solve
