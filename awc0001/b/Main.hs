...
headOr :: (U.Unbox a) => a -> U.Vector a -> a
headOr x0 xs
  | U.null xs = x0
  | otherwise = U.head xs
...
solve = do
  ...
  let res = headOr (-1) . U.map (succ . snd) . U.modify (VAI.sortBy (comparing (first Down))) . U.filter (\(!x, !_) -> inRange (l, r) x) $ U.imap (flip (,)) xs
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/awc0001/tasks/awc0001_b
main :: IO ()
main = runIO solve
