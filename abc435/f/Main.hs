...
solve = do
  ...
  seg <- Seg.build $ U.map (Max . swap) $ U.indexed xs

  let eval from l r
        | l == r = pure (1 :: Int)
        | l < 0 || r >= n || l > r = pure (0 :: Int)
        | otherwise = do
            Max (!_, !i) <- Seg.prod seg l (r + 1)
            let !_ = dbg (from, l, r, i)
            let d = abs $ i - from
            vl <- eval i l (i - 1)
            vr <- eval i (i + 1) r
            pure $ d + max vl vr

  Max (!_, !i0) <- Seg.prod seg 0 n
  h1 <- eval i0 0 (i0 - 1)
  h2 <- eval i0 (i0 + 1) (n - 1)
  printBSB $ max h1 h2

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc435/tasks/abc435_f
main :: IO ()
main = runIO solve
