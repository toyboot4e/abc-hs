...
solve = do
  (!n, !m) <- ints2P
  !es <- U.replicateM m ints11P
  q <- intP
  qs <- U.replicateM q ints2P

  -- rev
  let gr = Gr.build' n $ U.map swap es

  black <- UM.replicate n $ Bit False
  let paintDfs u = do
        Bit b <- GM.exchange black u $ Bit True
        unless b $ do
          U.forM_ (Gr.adj gr u) $ \v -> do
            paintDfs v
      ...
      (\(!p, pred -> !v) -> do
          if p == 1
            then do
              ...
              Just . unBit <$> GM.read black v
      )
      qs

  U.forM_ res printYn

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc435/tasks/abc435_d
main :: IO ()
main = runIO solve
