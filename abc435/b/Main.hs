...
import AtCoder.Extra.Math qualified as AEM
-- {{{ toy-lib import
  ...
  let cnts =
        U.length
          . U.filter
            ( \(!l, !r) ->
                let !s = csum +! (l, r)
                    !ds = IS.fromList . U.toList $ AEM.divisorsUnsorted s
                    !is = IS.fromList . U.toList . U.take (r + 1 - l) $ U.drop l xs
                    !_ = dbg (s, ds)
                 in IS.size (IS.intersection is ds) == 0
            )
          $ U.concatMap (\l -> U.generate (n - l) (\i -> (l, l + i))) (U.generate n id)

  printBSB cnts

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc435/tasks/abc435_b
main :: IO ()
main = runIO solve
