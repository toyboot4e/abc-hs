...
main = do
  ...
  let pis = map primeFactors $ VU.toList xs
  let pis23 = map (takeWhile (\(p, _) -> p == 2 || p == 3)) pis
  let pisElse = map (dropWhile (\(p, _) -> p == 2 || p == 3)) pis

  let compatible = (== 1) $ S.size $ S.fromList pisElse

  let (minI2, minI3) = foldl' step (maxBound @Int, maxBound @Int) pis23
      step (i2, i3) pi = case pi of
        [(2, i2'), (3, i3')] -> (min i2 i2', min i3 i3')
        [(2, i2')] -> (min i2 i2', 0)
        [(3, i3')] -> (0, min i3 i3')
        [] -> (0, 0)
        _ -> error "bug"

  let nOps = uncurry (+) $ foldl' step2 (0, 0) pis23
      step2 (i2, i3) pi = case pi of
        [(2, i2'), (3, i3')] -> (i2 + i2' - minI2, i3 + i3' - minI3)
        [(2, i2')] -> (i2 + i2' - minI2, i3)
        [(3, i3')] -> (i2, i3 + i3' - minI3)
        [] -> (i2, i3)
        _ -> error "bug"

  print $ if not compatible
    then (-1)
    else nOps
