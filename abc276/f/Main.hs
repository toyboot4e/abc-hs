...
main = do
  ...
  let !xMax = 200_001 :: Int
  !stree <- newSTree (\(!c1, !s1) (!c2, !s2) -> (c1 + c2, s1 + s2)) xMax (0 :: Int, modInt 0)

  -- take twice, get the expected value of `max (x, y)`
  let !m = VU.foldM step ([], modInt 0) xs
      step :: ([MyModInt], MyModInt) -> Int -> IO ([MyModInt], MyModInt)
      step (!res, s) !x = do
        (!nDown, !_) <- fromMaybe (0, modInt 0) <$> querySTree stree (1, x)
        (!_, !sUp) <- fromMaybe (0, modInt 0) <$> querySTree stree (succ x, xMax)

        -- NOTE: the order can be either (x, other) or (other, x)
        let !fromDown = modInt $ 2 * nDown * x
        let !fromUp = modInt 2 * sUp

        -- NOTE: can select `x` twice
        let !s' = s + fromDown + fromUp + modInt x

        !nAcc <- (\(!c, !_) -> c + 1) . fromJust <$> querySTree stree (0, xMax)
        let !p = s' / (modInt $ nAcc * nAcc)
        -- let !_ = dbg (s, (fromDown, fromSame, fromUp), (s', p), nAcc * nAcc)

        modifySTree stree (\(!c, !s) -> (succ c, modInt x + s)) x
