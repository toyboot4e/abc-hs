...
main = do
  [h, w] <- getLineIntList
  !input <- VU.fromList . concat <$> replicateM h getLine

  let bounds_ = ((0, 0), (pred h, pred w))

  let (sy, sx) = (`quotRem` w) . fromJust $ elemIndex 'S' input
  let !graph = listArray @UArray bounds_ $ map (== '.') input

  -- let !_ = traceShow (sy, sx) ()

  let dirs = [(1, 0), (-1, 0), (0, 1), (0, -1)]
  let neighbors (y, x) = filter (\x -> inRange bounds_ x && graph ! x) $! map (add2 (y, x)) dirs
  let indexIt (y, x) = y * w + x
