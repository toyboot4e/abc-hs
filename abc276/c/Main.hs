...
fact :: Int -> Int
fact 0 = 1
fact idx = idx * fact (idx - 1)

dictIndex :: [Int] -> Int
dictIndex (x : []) = x
dictIndex list@(x : xs) = i * fact (length xs) + dictIndex xs
  where
    i = fromJust . findIndex (== x) $ sort list

dictDigitsTemp :: Int -> Int -> [Int]
dictDigitsTemp index nDigits = map f [0 .. (nDigits - 1)]
  where
    f digit =
      let unit = fact digit
          -- n = (index `div` unit) `mod` nDigits
          n = (index `mod` ((digit + 1) * unit)) `div` unit
       in n

dictDigits :: [Int] -> Int -> [Int]
dictDigits is nDigits = f is [0..(nDigits - 1)]
  where
    f [i] [x] = [x]
    f (i : is) xs =
      let x = xs !! i
       in x : f is (delete x xs)

main :: IO ()
main = do
  [n] <- getLineIntList
  digits <- map (subtract 1) . map read . words <$> getLine
  print digits

  let index = dictIndex digits - 1
  print index

  let digitsTemp = reverse $ dictDigitsTemp index n
  print digitsTemp

  let result = dictDigits digitsTemp n
  putStrLn . unwords . map show $ map (+ 1) result
