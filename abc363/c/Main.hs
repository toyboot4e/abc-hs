...
solve = do
  (!n, !k) <- ints2'
  !xs <- U.fromList . map ord . BS.unpack <$> line'

  let p :: U.Vector Int -> Bool
      p !ys = U.and $ U.generate (n - k + 1) $ \i -> U.take k (U.drop i ys) /= U.take k (U.drop (n - k - i) ys')
        where
          !ys' = U.reverse ys

  printBSB $ V.length $ V.filter p $ lexPerms xs

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc363/tasks/abc363_c
main :: IO ()
main = runIO solve
