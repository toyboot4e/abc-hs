...
solve = do
  ...
  -- 0  1  2  => 2
  -- o--o--o
  let !maxLen = U.last xs - U.head xs

  -- 余った基地局をどう使うかが抜けている……
  let solve len
        | G.length groups > m = Nothing
        | otherwise = Just . V.sum $ V.map (\xs -> (last xs - head xs + 1) `div` 2) groups
        where
          !groups = dbgId . V.fromList . groupBy (\a b -> b - a <= len) $ U.toList xs

  let !res = fromJust . solve . fromJust . bisectR 0 maxLen $ \len -> isNothing (solve len)
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc414/tasks/abc414_d
main :: IO ()
main = runIO solve
