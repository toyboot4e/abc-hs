...
solve = do
  !n <- int'
  !cls <- U.replicateM n ((,) <$> char' <*> int')

  let lenSum = V.sum $ V.map (toInteger . snd) $ U.convert cls

  printBSB
    if lenSum > 100
      then showBSB "Too Long"
      else showBSB . concatMap (\(!c, !n) -> replicate n c) $ U.toList cls

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc414/tasks/abc414_b
main :: IO ()
main = runIO solve
