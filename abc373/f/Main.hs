...
solve = do
  ...
  -- -- low value items are preferred (sorted to the front)
  -- !wvs <- U.modify (VAI.sortBy (comparing snd)) <$> U.replicateM n ints2'

  -- I think this is wrong
  !wvs <- U.replicateM n ints2'

  res <- UM.generate (wMax + 1) $ \case 0 -> 0; _ -> (-1 :: Int)
  use <- VM.replicate (wMax + 1) IM.empty
  forM_ [0 .. wMax] $ \w -> do
    U.iforM_ wvs $ \iWvs (!dw, !dv) -> do
      let !prevW = w - dw
      when (prevW >= 0) $ do
        prevV <- GM.read res prevW
        when (prevV >= 0) $ do
          v <- GM.read res w
          !theUse <- GM.read use prevW
          let !nUse = fromMaybe 0 . IM.lookup iWvs $ theUse
          let !v' = prevV + dv - (2 * nUse + 1)
          when (v' > v) $ do
            GM.write res w v'
            GM.write use w $ IM.insert iWvs (nUse + 1) theUse

  res' <- U.unsafeFreeze res
  let !_ = dbg res'
  printBSB $ U.maximum res'

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc373/tasks/abc373_f
main :: IO ()
main = runIO solve
