...
solve = do
  -- let !dict = U.fromList $ map ord ['A' .. 'Z']
  !s <- U.fromList . map (subtract (ord 'A') . ord) . BS.unpack <$> line'
  let !is = U.update (U.replicate 26 (-1 :: Int)) $ U.imap (flip (,)) s
  let !ds = U.zipWith ((abs.).(-)) is (U.tail is)
  printBSB $ U.sum ds

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc373/tasks/abc373_b
main :: IO ()
main = runIO solve
