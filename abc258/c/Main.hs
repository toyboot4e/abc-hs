...
indexString = map (subtract 97 . ord)

main :: IO ()
main = do
  [nString, q] <- getLineIntList
  xs <- VU.fromList <$> getLine
  queries <- replicateM q ((\(n : ' ' : x) -> (digitToInt n, read @Int x)) <$> getLine)

  let results = reverse . snd $ foldl' step (0 :: Int, []) queries
      step (rot, acc) (1, x) = (rot + x, acc)
      step (rot, acc) (2, x) =
        -- let !_ = traceShow (rot, acc, x) () in
        (rot, xs VU.! ((pred x + VU.length xs - rot) `mod` VU.length xs) : acc)
      step _ _ = undefined

  forM_ results $ \x -> putStrLn [x]
