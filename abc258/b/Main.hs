...
deltas (y, x) (h, w) =
  [ (y, (x + 1) `rem` w),
    (y, (x + w - 1) `rem` w),
    ((y + 1) `rem` h, (x + 1) `rem` w),
    ((y + 1) `rem` h, (x + w - 1) `rem` w),
    ((y + 1) `rem` h, x),
    ((y + h - 1) `rem` h, x),
    ((y + h - 1) `rem` h, (x + 1) `rem` w),
    ((y + h - 1) `rem` h, (x + w - 1) `rem` w)
  ]

type Maze = UArray (Int, Int) Int

dfs :: Maze -> (Int, Int) -> [Int]
dfs maze start = next (0 :: Int, [], S.empty) (replicate (fst size) (0 :: Int)) start
  where
    size = bimap succ succ . snd $ bounds maze
    next :: (Int, [Int], S.Set (Int, Int)) -> [Int] -> (Int, Int) -> [Int]
    next (n, acc, vs) results x
      | n == snd size = max acc results
      | otherwise =
        let vs' = S.insert x vs
            -- TODO: more greedy filter choosing max
            nbs = filter (`S.notMember` vs') $ deltas x size
         in -- !_ = traceShow (x, maze ! x) ()
            foldl' (next (succ n, maze ! x : acc, vs')) results nbs

main :: IO ()
main = do
  [n] <- getLineIntList
  input <- concat <$> replicateM n (map digitToInt <$> getLine)

  -- looping
  let maze = listArray @UArray ((0, 0), (n - 1, n - 1)) input
  -- let !_ = traceShow (maze) ()
  let startPoints = range ((0, 0), (n - 1, n - 1))
  -- let startPoints = map fst . head . groupBy (\a b -> snd a == snd b) . sortBy (comparing snd) $ assocs maze

  let xs = map (dfs maze) startPoints
  let !_ = traceShow (xs) ()
  let ys = map (sum . snd . mapAccumL (\i x -> (10 * i, i * x)) (1 :: Int) . reverse) xs
  print $ maximum ys
