...
search :: Int -> (Int, Int) -> UArray (Int, Int) Int -> [[Int]]
search !n !i0 !grid = inner n [] i0
  where
    -- TODO: remove tail
    -- inner :: Int -> Int -> [[Int]]
    ...
    inner nRest !acc !i = concatMap (\i' -> inner (pred nRest) (grid ! i' : acc) i') $ get i

    get :: (Int, Int) -> [(Int, Int)]
    get !i =
      map (both (`mod` n) . add2 i) $ do
        iy <- [-1, 0, 1] :: [Int]
        ix <- [-1, 0, 1] :: [Int]
        guard $ not (iy == 0 && ix == 0)
        return (iy, ix)
...
main = do
  [n] <- getLineIntList
  !gridInput <- concat <$> replicateM n (map digitToInt <$> getLine)
  let !grid = listArray @UArray ((0, 0), (pred n, pred n)) gridInput
  let !_ = dbg (grid)

  let !maxValue = maximum $ elems grid
  let !maxIndices = map fst . filter ((== maxValue) . snd) $ assocs grid

  let solve :: (Int, Int) -> [Int]
      solve !i = map (\is -> read @Int $ concatMap show is) $ search n i grid
  let !result = maximum $ concatMap solve maxIndices
