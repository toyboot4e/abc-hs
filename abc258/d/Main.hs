...
main :: IO ()
main = do
  [nInput, nWin] <- getLineIntList
  xs <- VU.replicateM nInput ((\[a, b] -> (a, b)) <$> getLineIntList)

  let firstWins = VU.scanl' step (0 :: Int, maxBound @Int) xs
      step (acc, minSoFar) (a, b) = (acc + a + b, minSoFar `min` b)

  let durations = VU.imap f firstWins
      -- ignore
      f 0 _ = maxBound @Int
      -- concrete example
      f 1 (acc, minSoFar) = acc + (nWin - 1) * minSoFar
      -- more general answer
      f n (acc, minSoFar)
        | n == nWin = acc
        -- ignore
        | n > nWin = maxBound @Int
        | otherwise = acc + (nWin - n) * minSoFar

  -- let !_ = traceShow (durations) ()
  print $ VU.minimum durations
