...
getLineIntVG :: VG.Vector v Int => IO (v Int)
getLineIntVG = VG.unfoldr (BS.readInt . BS.dropWhile isSpace) <$> BS.getLine

getLineIntV :: IO (V.Vector Int)
getLineIntV = getLineIntVG

getLineIntVU :: IO (VU.Vector Int)
getLineIntVU = getLineIntVG
...
trivial :: Int -> [Int] -> IO ()
trivial answer queries = do
  forM_ queries $ \_ -> do
    print answer

main :: IO ()
main = do
  [nLoop, nQueries, _boundary] <- getLineIntList
  !ws <- getLineIntVU
  !queries <- replicateM nQueries (readLn @Int)

  let !_ = dbg ("weights", ws)

  -- ignore meaningless space
  let (!nIgnoreLoop, !boundary) = _boundary `divMod` (VU.sum ws)
  let !_ = dbg ("boundary", boundary)

  when (boundary == 0) $ do
    trivial (nIgnoreLoop * nLoop) queries
    exitSuccess

  -- REMARK: Here' we're assuming `boundary > 0` as in `succ i` in the `accum` function
  let !mapper = VU.accum (\_ i -> succ i `rem` nLoop) (VU.replicate nLoop (-1)) input
        where
          !csum2 = VU.scanl' (+) (0 :: Int) (ws VU.++ ws)
          !input = twoPointers (2 * nLoop) check
          !_ = dbg ("csum2", csum2)
          !_ = dbg ("shakutori", input)
          check (!l, !r)
            | l >= nLoop = False
            | l == r = True
            | otherwise = not $ s1 >= boundary && s2 >= boundary
            where
              s1 = csum2 VU.! r - csum2 VU.! l
              s2 = csum2 VU.! succ r - csum2 VU.! l
              !_ = dbg ((l, r), s1, s2)

  let !_ = dbg ("mapper", mapper)

  let doubled :: V.Vector (VU.Vector Int)
      !doubled = newDoubling mapper (\mapper' -> VU.map (mapper' VU.!) mapper')

  forM_ queries $ \nApply -> do
    -- let !i1 = times (pred nApply) (\i -> mapper VU.! i) 0
    -- let !i2 = times nApply (\i -> mapper VU.! i) 0
    let !i1 = applyDoubling doubled 0 (\i vec -> vec VU.! i) (pred nApply)
    let !i2 = applyDoubling doubled 0 (\i vec -> vec VU.! i) nApply

    let !_ = dbg ("nApply", nApply, i1, i2)
