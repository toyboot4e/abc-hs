...
extractCandicates :: IM.IntMap Int -> [Int]
extractCandicates !im = map fst . takeWhile ((== maxCount) . snd) $ IM.toDescList im
  where
    !maxCount = snd $ IM.findMax im

count :: [Int] -> IM.IntMap Int -> IM.IntMap Int -> Int -> Int
count !ms !im1 !im2 !nShift = n1 + n2
  where
    !n1 = sum $ map (\k -> fromMaybe 0 $ IM.lookup (k - nShift) im1) ms
    !n2 = sum $ map (\k -> fromMaybe 0 $ IM.lookup (k + nShift) im2) ms
...
main = do
  ...
  let !initials = VU.scanl' step 0 xs
        where
          step acc s = s - acc

  let (!evens, !odds) = VG.foldl' step (IM.empty, IM.empty) $ VG.zip (vRange 0 (pred n)) initials
        where
          step (!es, !os) (!i, !x)
            | i `rem` 2 == 0 = (IM.insertWith (+) x 1 es, os)
            | otherwise = (es, IM.insertWith (+) x 1 os)

  let !_ = dbg ("maps")
  let !_ = dbg (evens)
  let !_ = dbg (odds)

  let !shifts1 = foldl' (\acc from -> foldl' (\acc' x -> x -  from : acc') acc ms) [] $ IM.keys evens
  let !shifts2 = foldl' (\acc to -> foldl' (\acc' x -> to - x : acc') acc ms) shifts1 $ IM.keys odds

  let !_ = dbg ("shift candidates", shifts2)

  let !n = maximum $ map (count ms evens odds) $ nubSort shifts2
  print n
