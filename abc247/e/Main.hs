      ...
      | otherwise = do
          (!l', !lAcc') <-
            if isRightChild l
              then do
                ...
                !la' <- mact <$> (VUM.read ops l) <*> (VUM.read as l)
          ...
          (!r', !rAcc') <-
            if isLeftChild r
              then do
                -- Evaluate the target segmnent and append the result:
                !ra' <- mact <$> (VUM.read ops r) <*> (VUM.read as r)
...
-- solve :: Int -> (s -> (Int, Int) -> Bool) -> s -> (Int -> s) -> (s -> Int -> s) -> (s -> Int -> s) -> [(Int, Int)]
-- solve !n !check !state0 !singleton !onExpand !onShrink = inner (0, 0) state0
--   where
--     inner (!l, !r) !s | l >= n = []
--     inner (!l, !r) !s
--       | check s (l, r) =
--           let (!s', (!l', !r')) = until (not . peekCheck) (\(!s_, !lr) -> (onExpand (succ $ snd lr), second succ lr)) (s, (l, r))
--            in (l', r') : inner (succ l', max l' r') s'
--       -- FIXME: max (succ l) r is not proper.
--       | succ l > r = inner (succ l, succ l) (singleton l)
--       | otherwise = inner (succ l,  r) (onShrink s l)
--     peekCheck (!s, (!l, !r)) | r == pred n = False
--     peekCheck (!s, (!l, !r)) = check s (l, succ r)

-- main = do
  --   let !result = solve (VU.length xs) check (IM.singleton . (xs VU.!)) (IM.singleton $ xs VU.! 0) onExpand onShrink
  --         where
  --           check !im _ =
  --             let !xMax = snd $ IM.findMax im
  --                 !xMin = snd $ IM.findMin im
  --              in xMax <= maxLimit && xMin <= minLimit
  --           -- insert
  --           onExpand !im !r = IM.insertWith (+) (xs VU.! r) 1 im
  --           -- remove
  --           onShrink !im !l = case IM.lookup (xs VU.! l) im of
  --             Nothing -> error "bug: tried to remove non-existing value from the int map"
  --             Just 1 -> IM.delete (xs VU.! l) im
  --             Just n -> IM.insert (xs VU.! l) (pred n) im

add4 (!n1, !n2, !n3, !n4) (!m1, !m2, !m3, !m4) = (n1 + m1, n2 + m2, n3 + m3, n4 + m4)

sub4 (!n1, !n2, !n3, !n4) (!m1, !m2, !m3, !m4) = (n1 - m1, n2 - m2, n3 - m3, n4 - m4)

solveTrivial :: VU.Vector Int -> Int -> Int
solveTrivial !xs !x0 = sum . map (\n -> n * pred n `div` 2 + n) . map length . filter ((== x0) . head) $ groupBy (==) $ VU.toList xs

main :: IO ()
main = do
  [n, maxLimit, minLimit] <- ints
  !xs <- intsVU

  when (maxLimit == minLimit) $ do
    print $ solveTrivial xs maxLimit
    exitSuccess

  let !csums = VU.scanl' step s0 xs
        where
          -- (nX, nX', nY, nY')
          s0 = (0, 0, 0, 0) :: (Int, Int, Int, Int)
          step (!nX, nX', nY, nY') !x
            -- TODO: spray syntax for tuples?
            | x == maxLimit = (succ nX, nX', nY, nY')
            | x > maxLimit = (nX, succ nX', nY, nY')
            | x == minLimit = (nX, nX', succ nY, nY')
            | x < minLimit = (nX, nX', nY, succ nY')
            | otherwise = (nX, nX', nY, nY')

  let !_ = dbg ("input:")
  let !_ = dbg ((maxLimit, minLimit), xs)
  let !_ = dbg (csums)

  let !result = VU.sum $ VU.map f (rangeVG 0 (pred $ VU.length xs))
        where
          -- TODO: use `fromMaybe 0 $ do`
          f !l = case bsearchL (l, pred $ VU.length xs) g of
            Just r' ->
              case bsearchL (l, r') h of
                Just r -> r' - r
                Nothing -> r' - l
            Nothing -> 0
            where
              g !r' =
                let (!_, nX', _, nY') = (csums VU.! succ r') `sub4` (csums VU.! l)
                 in if
                        | nX' > 0 -> False
                        | nY' > 0 -> False
                        | otherwise -> True
              ...
              h !r =
                let (!nX, nX', nY, nY') = (csums VU.! succ r) `sub4` (csums VU.! l)
                 in if
                        | nX' > 0 -> error "unreachable 1"
                        | nY' > 0 -> error "unreachable 2"
                        | nX > 0 && nY > 0 -> False
                        | otherwise -> True
