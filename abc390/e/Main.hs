...
solve = do
  ...
  let dist2 = U.scanl' max (-1) . U.tail . calc $ U.mapMaybe (\(!v, !a, !c) -> if v == 2 then Just (a, c) else Nothing) vacs
  let dist3 = U.scanl' max (-1) . U.tail . calc $ U.mapMaybe (\(!v, !a, !c) -> if v == 3 then Just (a, c) else Nothing) vacs
  ...
  printBSB . fromMaybe 0 $ bisectL 0 (5000 * 2 * (10 ^ 5) :: Int) f

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc390/tasks/abc390_e
main :: IO ()
main = runIO solve
