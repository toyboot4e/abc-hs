...
ortho5 :: U.Vector (Int, Int)
ortho5 = U.fromList [(0, 0), (0, 1), (0, -1), (1, 0), (1, -1)]

solve :: StateT BS.ByteString IO ()
solve = do
  (!h, !w) <- ints2'
  !gr <- getGrid' h w
  (!sy, !sx, !ey, !ex) <- dbgId <$> ((,,,) <$> int1' <*> int1' <*> int1' <*> int1')
  ...
  --    for_ [0 .. w - 1] $ \x0 -> do
  --      when (gr @! (y0, x0) == '.') $ do
  --        U.forM_ ortho4 $ \(!dy, !dx) -> do
  --          let (!y, !x) = add2 (y0, x0) (dy, dx)
  --          when (gr @! (y, x) == '.') $ do
  --            unifyMUF_ uf (w * y0 + x0) (w * y +x)
  --
  --  rs <- rootMUF uf (w * sy + sx)
  --  re <- rootMUF uf (w * ey + ex)
  --
  --  buf <- newBuffer (16 * h * w)
  --  let onBreak r1 y0 x0 = do
  --        U.forM_ (U.map (add2 (y0, x0)) ortho5) $ \(!y, !x) -> do
  --          when (gr @! (y, x) == '.') $ do
  --            r2 <- rootMUF uf $ y * w + x
  --            when (r1 /= r2) $ do
  --              pushBack buf (r1, r2, 1 :: Int)
  --
  --  -- edges
  --  for_ [0 .. h - 1] $ \y0 -> do
  --    for_ [0 .. w - 1] $ \x0 -> do
  --      when (gr @! (y0, x0) == '.') $ do
  --        r1 <- rootMUF uf $ w * y0 + x0
  --        U.forM_ ortho4 $ \(!dy, !dx) -> do
  --          onBreak r1 (y0 + 1 * dy) (x0 + 1 * dx)
  --          onBreak r1 (y0 + 2 * dy) (x0 + 2 * dx)
  --
  --  es <- unsafeFreezeBuffer buf
  --  let gr = buildWSG (h * w) $ swapDupeW es

  let !bnd2 = zero2 h w
  let !res = genericBfs01 bnd2 grF (8 * h * w) $ U.singleton (sy, sx)
        where
          grF (!y0, !x0) =
            dbgId $
            U.concatMap
              ( \(!dy, !dx) ->
                  let pt1 = (y0 + 1 * dy, x0 + 1 * dx)
                      -- 1 on True
                      b1 = gr @!? pt1 /= Just '.'
                      d1 = bool 0 1 b1
                      pt2 = (y0 + 2 * dy, x0 + 2 * dx)
                      b2 = b1 || gr @!? pt2 /= Just '.'
                      d2 = bool 0 1 b2
                      -- !_ = dbg ((y0, x0), inRange bnd2 pt1, inRange bnd2 pt2, pt1, pt2)
                   in if
                        | inRange bnd2 pt2 -> U.fromListN 2 [(pt1, d1), (pt2, d2)]
                        | inRange bnd2 pt1 -> U.singleton (pt1, d1)
                        | otherwise -> U.empty
              )
              ortho4
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc400/tasks/abc400_d
main :: IO ()
main = runIO solve
