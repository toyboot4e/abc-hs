...
-- test :: Int -> Int -> Bool
-- test n d = r == 0 && n2 * n2 == n
--   where
--     (!q, !r) = n `divMod` d
--     n2 = fromMaybe 0 $ bisectL 0 q $ \i -> i * i <= q

-- solve :: StateT BS.ByteString IO ()
-- solve = do
--   !n <- dbgId <$> int'
--   let ds = dbgId $ U.unfoldr f 2
--         where
--           f d
--             | d > n = Nothing
--             | otherwise = Just (d, 2 * d)
--   let res = U.length $ U.filter (test n) ds
--   printBSB res
...
solve = do
  ...
  let count d =
        ...
        let !upTo = note (n, d) . fromMaybe (-1) $ bisectL 0 (10^9) $ \i -> toInteger d * toInteger (2 * i + 1) * toInteger (2 * i + 1) <= toInteger n
         in 1 + upTo

  let res = U.sum . dbgId $ U.map count twos
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc400/tasks/abc400_c
main :: IO ()
main = runIO solve
