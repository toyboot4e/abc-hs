...
solve = do
  ...
  let pCnt = U.create $ do
        vec <- UM.replicate (10 ^ 6 + 1) (0 :: Int)
        for_ [2 .. 10^6] $ \p -> do
          n <- GM.read vec p
          when (n == 0) $ do
            for_ [1 .. 10^6 `div` p] $ \dx -> do
              GM.modify vec (+ 1) $ p * dx
        pure vec

  -- 400 numbers:
  let candidates = U.map (\x -> x * x) $ U.findIndices (== 2) pCnt

  U.forM_ qs $ \x -> do
    printBSB $ (candidates G.!) . fromJust $ bisectL 0 (U.length candidates - 1) $ \i ->
      candidates G.! i <= x

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc400/tasks/abc400_e
main :: IO ()
main = runIO solve
