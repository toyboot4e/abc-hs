...
solve = do
  !n <- int'
  !tvs <- U.replicateM n ((,) <$> int1' <*> int')
  let maxT = fst $ U.last tvs
  let res = U.accumulate (+) (U.replicate (maxT + 1) (0 :: Int)) tvs
  let res' = U.foldl' (\acc dx -> dbgId $ max 0 (acc - 1) + dx) (0 :: Int) res
  printBSB res'

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc383/tasks/abc383_a
main :: IO ()
main = runIO solve
