...
solve = do
  ...
  let try y1 x1 y2 x2 = runST $ do
        vec <- IxVector bounds <$> UM.replicate (rangeSize bounds) False

        let paint y x = do
              for_ [-d .. d] $ \dy -> do
                for_ [-d .. d] $ \dx -> do
                  when (abs dx + abs dy <= d && inRange bounds (y + dy, x + dx)) $ do
                    unless (gr @! (y + dy, x + dx)) $ do
                      writeIV vec (y + dy, x + dx) True

        paint y1 x1
        paint y2 x2
        U.length . U.filter id <$> U.unsafeFreeze (vecIV vec)

  let res =
        maximum
          [ try y1 x1 y2 x2
            | i1 <- [0 .. rangeSize bounds - 1],
              not $ vecIV gr U.! i1,
              let (!y1, !x1) = i1 `divMod` w,
              i2 <- [0 .. rangeSize bounds - 1],
              not $ vecIV gr U.! i2,
              i1 /= i2,
              let (!y2, !x2) = i2 `divMod` w
          ]

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc383/tasks/abc383_b
main :: IO ()
main = runIO solve
