...
main = do
  [n, q] <- getLineIntList
  questions <- replicateM q getLineIntList

  let toIndex :: (Int, Int) -> Int
      toIndex (!a, !b) = a * (10 ^ 9) + b

  setRef <- newIORef IS.empty
  forM_ questions $ \[kind, a, b] -> do
    set <- readIORef setRef
    case kind of
      1 -> do
        let newSet = IS.insert (toIndex (a, b)) set
        writeIORef setRef newSet
      2 -> do
        let newSet = IS.delete (toIndex (a, b)) set
        writeIORef setRef newSet
      3 -> do
        let friend = IS.member (toIndex (a, b)) set && IS.member (toIndex (b, a)) set
        putStrLn $ if friend then "Yes" else "No"
  ...
  --     -- A follows B
  --     1 -> do
  --           case IM.lookup a map of
  --                 Just follows -> do
  --                       let newFollowMap = IS.insert b follows
  --                       writeIORef mapRef (IM.insert a newFollowMap map)
  --                 Nothing -> writeIORef mapRef (IM.insert a (IS.singleton b)

  --     -- A unfollows B
  --     2 -> do
  --           case IM.lookup a map of
  --                 Just follows -> do
  --                       let newFollowMap = IS.delete b follows
  --                       writeIORef mapRef (IM.insert a newFollowMap map)
  --                 Nothing -> pure ()

  --     -- Do A and B follow each other?
  --     3 -> do
  --           let friend = case (IM.lookup a map, IM.lookup b map) of
  --                 (Just as, Just bs) -> IS.member b as && IS.member a bs
  --                 _ -> False
  --           print $ if friend then "Yes" else "No"
