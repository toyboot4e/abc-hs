...
main = do
  ...
  [nqs] <- getLineIntList
  qs <- replicateM nqs getLineIntList

  -- (value, generation)
  vec <- VU.thaw (VU.zip as (VU.replicate n (0 :: Int)))
  bsRef <- newIORef (mempty @BSB.Builder)
  -- (value, generation)
  stateRef <- newIORef ((0, 0) :: (Int, Int))

  forM_ qs $ \(kind : rest) -> do
    (offset, generation) <- readIORef stateRef

    case kind of
      -- set all, but in $O(1)$
      1 -> do
        let [x] = rest
        writeIORef stateRef (x, generation + 1)
        return ()
      ...
      2 -> do
        ...
        (oldValue, oldGeneration) <- VUM.read vec i
        let newValue = addition + if oldGeneration == generation then oldValue else offset

        VUM.write vec i (newValue, generation)
        return ()
      ...
      3 -> do
        ...
        (value, gen) <- VUM.read vec i
        let x = if gen == generation then value else offset

        bs <- readIORef bsRef
        let bs' = bs <> (BSB.intDec x) <> (BSB.char7 '\n')
        writeIORef bsRef bs'
        return ()
