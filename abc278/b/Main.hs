...
toDigits x
  | x < 10 = [0, x]
  | True = [x `div` 10, x `mod` 10]

isConfusing :: [Int] -> Bool
isConfusing [h, m] = 0 <= h' && h' < 24 && 0 <= m' && m' < 60
  where
    [h0, h1] = toDigits h
    [m0, m1] = toDigits m
    h' = 10 * h0 + m0
...
findConfusing :: [Int] -> [Int]
findConfusing [h, m]
  | isConfusing [h, m] = [h, m]
  -- next minute
  | m < 59 = findConfusing [h, m + 1]
  -- next hour
  | h < 23 = findConfusing [h + 1, 0]
  -- next day
  | True = findConfusing [0, 0]

main :: IO ()
main = do
  [h, m] <- getLineIntList
  let [x, y] = findConfusing [h, m]
  putStr $ unwords . map show $ [x, y]
