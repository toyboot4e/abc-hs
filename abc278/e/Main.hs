...
-- Original: https://atcoder.jp/contests/abc278/submissions/36563884
main :: IO ()
main = do
  [gridH, gridW, n, h, w] <- getLineIntList

  -- Use 1-based index
  gridList <- replicateM gridH getLineIntList
  let grid :: AU.UArray (Int, Int) Int
      grid = AU.listArray ((1 :: Int, 1 :: Int), (gridH, gridW)) $ concat gridList

  -- REMARK: We cannot use an unboxed array of tuples
  let outOfBounds = ((gridH + 1, gridW + 1), (-1, - 1))
  let bounds :: Array Int ((Int, Int), (Int, Int))
      bounds = STA.runSTArray $ do
        -- (minPos, maxPos)
        arr <- MA.newArray (1, n) outOfBounds

        forM_ [1 .. gridH] $ \y -> do
          forM_ [1 .. gridW] $ \x -> do
            let v = grid IA.! (y, x)
            ((minY, minX), (maxY, maxX)) <- MA.readArray arr v
            let minPos = (minY `min` y, minX `min` x)
            let maxPos = (maxY `max` y, maxX `max` x)
            MA.writeArray arr v (minPos, maxPos)

        return arr

  forM_ [1 .. (gridH - (h - 1))] $ \y -> do
    rowValues <- forM [1 .. (gridW - (w - 1))] $ \x -> do
      let (minY, minX) = (y, x)
      let (maxY, maxX) = (y + h - 1, x + w - 1)
      return $ snd $ flip State.runState (0 :: Int) do
        forM_ [1 .. n] $ \v -> do
          let b@((boundsMinY, boundsMinX), (boundsMaxY, boundsMaxX)) = bounds IA.! v
          let rest =
                b /= outOfBounds
                  && boundsMinY < minY
                  || boundsMinX < minX
                  || boundsMaxX > maxX
                  || boundsMaxY > maxY
          State.modify' (+ (if rest then 1 else 0))

    putStrLn . unwords $ map show rowValues
