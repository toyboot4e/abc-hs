...
solve = do
  ...
  !qs <- U.map pred <$> intsU'
  -- value -> person
  let qs' = U.update (U.replicate n (-1)) $ U.imap (flip (,)) qs
  printVec $ U.map (+ 1) $ U.backpermute qs $ U.backpermute ps qs'

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc392/tasks/abc392_c
main :: IO ()
main = runIO solve
