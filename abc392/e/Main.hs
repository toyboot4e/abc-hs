...
solve = do
  ...
  rest <- newBuffer m
  uf <- newMUF n
  U.iforM_ es $ \i (!v1, !v2) -> do
    if v1 == v2
      then pushBack rest (i, v1, v2)
      else do
        unlessM (unifyMUF uf v1 v2) $ do
          pushBack rest (i, v1, v2)

  let pickNext m exclude = do
        exclude' <- rootMUF uf exclude
        let m' = S.delete exclude' m
        if S.null m'
          then pure (-1, m')
          else do
            let (!r, !m'') = S.deleteFindMin m'
            unifyMUF_ uf r exclude'
            r' <- rootMUF uf r
            let !mRes = S.insert r' $ S.delete r m''
            pure (r, mRes)

  result <- newBuffer m
  let run edges (dbgId -> m)
        | S.size m <= 1 = pure ()
        | Just ((!i, !u, !v), !es') <- U.uncons edges = do
          (!u', m') <- pickNext m u
          pushBack result (i, u, u')
          run es' m'
        | otherwise = pure ()
  ...
  let m0 = S.fromList $ U.toList roots
  edges <- unsafeFreezeBuffer rest
  run edges m0

  ops <- unsafeFreezeBuffer result
  printBSB $ U.length ops
  unless (U.null ops) $ do
    U.forM_ ops $ \(!i, !u, !u') -> do
      printBSB (i + 1, u + 1, u' + 1)

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc392/tasks/abc392_e
main :: IO ()
main = runIO solve
