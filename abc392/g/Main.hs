...
solve = do
  ...
  let len = 10 ^ 6 :: Int
  let cnts = U.accumulate (+) (U.replicate len (0 :: Int)) $ U.map (, 1 :: Int) xs

  res <- UM.replicate 1 (0 :: Int)
  U.iforM_ cnts $ \m xm -> do
    for_ [1 .. min m (len - 1 - m)] $ \d -> do
      -- let !_ = dbg (m, d, xm - d, xm + d)
      let !xl = cnts G.! (m - d)
      let !xr = cnts G.! (m + d)
      UM.modify res (+ (xl * xm * xr)) 0

  printBSB =<< UM.read res 0

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc392/tasks/abc392_g
main :: IO ()
main = runIO solve
