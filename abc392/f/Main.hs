...
solve = do
  ...
  let res = U.ifoldl' step s0 ps
        where
          s0 = Seq.empty
          step s i p = Seq.insertAt (p - 1) (i + 1) s

  printVec . U.fromList $ toList res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc392/tasks/abc392_f
main :: IO ()
main = runIO solve
