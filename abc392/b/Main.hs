...
solve = do
  (!n, !m) <- ints2'
  !xs <- IS.fromList . U.toList <$> intsU'
  let ys = U.filter (`IS.notMember` xs) $ U.generate n (+ 1)
  printBSB $ U.length ys
  unless (U.null ys) $ printVec ys

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc392/tasks/abc392_b
main :: IO ()
main = runIO solve
