...
match :: Int -> Int -> [(Int, Int)] -> [(Int, Int)] -> Double
match n m = inner
  where
    n_ = intToDouble n
    m_ = intToDouble m
    inner :: [(Int, Int)] -> [(Int, Int)] -> Double
    inner [] _ = 0.0
    inner _ [] = 0.0
    inner xss@((!kx, !x) : !xs) yss@((!ky, !y) : !ys) = case compare kx ky of
      EQ ->
        let !_ = dbg ((n, m), (kx, x), (ky, y), intToDouble x / n_, intToDouble y / m_) in
        intToDouble x / n_ * intToDouble y / m_ + inner xs ys
      LT -> inner xs yss
      GT -> inner xss ys
...
solve = do
  !n <- int'
  !lists <- V.replicateM n do
    m <- int'
    xs <- intsU'
    let ls = map (\v -> (U.head v, U.length v)) . U.group $ U.modify VAI.sort xs
    pure (U.length xs, ls)

  let res =
        maximum
          [ let (!la, !las) = lists V.! ia
                (!ra, !ras) = lists V.! ib
             in match la ra las ras
            | ia <- [0 .. n - 1],
              ib <- [ia + 1 .. n - 1]
          ]
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc392/tasks/abc392_d
main :: IO ()
main = runIO solve
