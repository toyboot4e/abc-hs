...
main = do
  ...
  let !scc = topScc graph
  -- let !_ = traceShow ("scc", scc) ()

  -- vertex -> SCC index
  let !vertIndex = VU.create $ do
        !vec <- VUM.replicate nVerts (-1 :: Int)
        forM_ (zip [0 ..] scc) $ \(!i, !vs) -> do
          forM_ vs $ \v -> do
            VUM.write vec v i
        return vec

  let !result = runST $ do
        -- SCC index -> can reach cycle
        !dp <- VUM.replicate nVerts False

        let !downSCC = reverse . map reverse $ scc
        !ss <- forM downSCC $ \vs -> do
          case vs of
            [] -> error "unreachable"
            [v] -> do
              let !i = vertIndex VU.! v
              !anyCycle <- anyM (VUM.read dp . (vertIndex VU.!)) $ graph ! v

              -- let !_ = traceShow (v, i, anyCycle) ()
              if anyCycle
                then do
                  VUM.write dp i True
                  return 1
