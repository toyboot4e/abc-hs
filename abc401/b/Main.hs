...
solve = do
  ...
  let os = V.fromList $ map BS.pack ["login", "logout", "public", "private"]
  !ss <- U.replicateM n $ do
    s <- line'
    pure . fromJust $ V.elemIndex s os

  let !_ = dbg ss
  let res = fst $ U.foldl' step s0 ss
        where
          !s0 = (0 :: Int, False)
          step (!acc, !st) o = (acc', stF o)
            where
              !_ = dbg st
              acc' = acc + bool 0 1 (not st && o == 3)
              stF 0 = True
              stF 1 = False
              stF _ = st
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc401/tasks/abc401_b
main :: IO ()
main = runIO solve
