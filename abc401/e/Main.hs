...
--   uf <- newMUF n
--
--   toDelete <- newMutVar S.empty
--   res <- U.forM (U.generate n id) $ \k -> do
--     for_ [0 .. k] $ \u -> do
--       modifyMutVar toDelete $ S.delete u
--       U.forM_ (gr `adj` u) $ \v -> do
--         if v > k
--           then do
--             modifyMutVar toDelete $ S.insert v
--           else do
--             unifyMUF_ uf u v
--
--     sz <- sizeMUF uf 0
--     delSize <- S.size <$> readMutVar toDelete
--     pure $ if k + 1 == sz then delSize else -1
--
...
solve = do
  ...
  let res = runST $ do
        ...
        delAt <- UM.replicate n False
        delCnt <- UM.replicate 1 (0 :: Int)

        let deleteAt i = do
              b <- GM.exchange delAt i True
              unless b $ do
                GM.modify delCnt (+ 1) 0
        let undoAt i = do
              b <- GM.exchange delAt i False
              when b $ do
                GM.modify delCnt (subtract 1) 0

        U.forM (U.generate n id) $ \k -> do
          let u = k
          undoAt u
          U.forM_ (gr `adj` u) $ \v -> do
            if v > k
          ...
          sz <- sizeMUF uf 0
          delSize <- GM.read delCnt 0
          pure $ if k + 1 == sz then delSize else -1

  printBSB $ unlinesBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc401/tasks/abc401_e
main :: IO ()
main = runIO solve
