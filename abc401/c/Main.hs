...
solve = do
  (succ -> !n, !k) <- ints2'
  !seg <- newSTree @(Sum (ModInt 1000000000)) $ max n k

  for_[0 .. k - 1] $ \i -> do
    writeSTree seg i $ Sum 1
  for_ [k .. n - 1] $ \i -> do
    -- x <- foldSTree seg (i - k) (i - 1)
    -- let !_ = dbg (i - k, i - 1, x)
    writeSTree seg i =<< foldSTree seg (i - k) (i - 1)

  Sum res <- readSTree seg (n - 1)
  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc401/tasks/abc401_c
main :: IO ()
main = runIO solve
