...
findCycles :: VU.Vector Int -> IS.IntSet -> Int -> (IS.IntSet, Maybe Int)
findCycles graph visited0 start
  | IS.member start visited0 = (visited0, Nothing)
  | otherwise = solve visited0 start
  where
    solve visited x =
      let next = graph VU.! x
          visited' = IS.insert x visited
       in if
              -- already visited
              | IS.member next visited0 -> (IS.union visited' visited0, Nothing)
              -- cycle
              | IS.member next visited' -> (IS.union visited' visited0, Just next)
              -- keep searching
              | otherwise -> solve visited' next
...
main = do
  ...
  -- functional graph (outdegree == 1)
  graph <- VU.map pred <$> getLineIntVec
  costs <- getLineIntVec

  let starts = snd $ foldl' step s0 [0 .. pred n]
      s0 = (IS.empty, [])
      step (set, acc) start =
        case findCycles graph set start of
          (set', Just x) -> (set', x : acc)
          (set', Nothing) -> (set', acc)

  -- let !_ = traceShow (starts) ()

  print $ sum $ map (findMinCycleBreak graph costs) starts
