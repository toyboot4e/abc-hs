...
main = do
  [n] <- getLineIntList
  input <- concatMap (\[a, b] -> [(a, 1 :: Int), (b, -1)]) <$> replicateM n getLineIntList

  let source = accumArray @UArray (+) (0 :: Int) (1, 2 * 10 ^ 5) input
  let csums = scanl1 (+) $ elems source
  let rangeInput = zipWith (curry (second (> 0))) [1..] csums
  let ranges = filter (snd . head) $ groupBy (\(_, a) (_, b) -> a == b) rangeInput

  forM_ ranges $ \range -> do
    let l = fst $ head range
    let r = succ $ fst $ last range
