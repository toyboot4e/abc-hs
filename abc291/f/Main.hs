...
main = do
  [n, m] <- getLineIntList
  !input <- forM [0 .. pred n] $ \v1 -> do
    map (\(!offset, _) -> (v1, v1 + offset)) . filter snd . zip [1 ..] . map (== '1') <$> getLine

  if m == 1
    then putStrLn $ unwords $ map show $ replicate (n - 2) (-1)
    else main' n m input

main' :: Int -> Int -> [[(Int, Int)]] -> IO ()
main' n m !input = do
  let !graph = accumArray (flip (:)) [] (0, pred n) $ concat input
  let !graph' = revGraph graph
  let !_ = dbg (graph)

  let !lenForwards = bfsVerts graph 0
  let !lenBackwards = bfsVerts graph' (pred n)

  let !results = flip map [1 .. (n - 2)] $ \vSkip ->
        minimum $
          flip map [(max 0 (vSkip + 1 - m)) .. (vSkip - 1)] $ \v1 ->
            let !v2s = filter (> vSkip) $ graph ! v1
                !lens = flip map v2s $ \v2 ->
                  maybe (maxBound) succ $ (+) <$> IM.lookup v1 lenForwards <*> IM.lookup v2 lenBackwards
                !_ = dbg (vSkip, (v1, v2s), lens)
             in case lens of
                  [] -> maxBound
                  xs -> minimum xs

  putStrLn $ unwords $ map show $ map (\x -> if x == maxBound then (-1) else x) results
