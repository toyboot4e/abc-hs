...
main :: IO ()
main = do
  [nCards] <- getLineIntList
  input <- replicateM nCards (tuple2 <$> getLineIntList)

  let bool01 b = if b then 1 else 0

  let result = uncurry (+) . snd3 $ foldl' step s0 (tail input)
      s0 = (True, (1 :: Int, 1 :: Int), head input)
      step :: (Bool, (Int, Int), (Int, Int)) -> (Int, Int) -> (Bool, (Int, Int), (Int, Int))
      step (False, _, _) _ = (False, (0, 0), (0, 0))
      step (True, (accN, accR), (xn, xr)) (yn, yr) =
        let accN' = (`rem` 998244353) $ accN * bool01 (xn /= yn) + accR * bool01 (xr /= yn)
            accR' = (`rem` 998244353) $ accN * bool01 (xn /= yr) + accR * bool01 (xr /= yr)
            -- !_ = traceShow ((xn, xr), (yn, yr), (accN, accR), (accN', accR')) ()
         in (accN' /= 0 || accR' /= 0, (accN', accR'), (yn, yr))

  print $ result `rem` 998244353
