...
main = do
  [ns] <- getLineIntList
  deltas <- map deltaVec <$> getLine

  let result = fst3 $ foldl' step s0 deltas
      s0 = (False, (0, 0), S.singleton (0, 0))
      step (b, (x, y), vis) (dx, dy) =
        let (x', y') = (x + dx, y + dy)
            b' = b || (x', y') `S.member` vis
            vis' = S.insert (x, y) vis
         in (b', (x', y'), vis')
