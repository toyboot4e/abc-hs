...
newtype UnionFind m = UnionFind (VM.MVector (PrimState m) UfNode)

-- | `Child parent | Root size`
-- | I failed to implement `Unbox` for this algebric data type..
data UfNode = Child {-# UNPACK #-} !Int | Root {-# UNPACK #-} !Int

root :: (PrimMonad m) => UnionFind m -> Int -> m Int
...
same :: (PrimMonad m) => UnionFind m -> Int -> Int -> m Bool
...
unite :: (PrimMonad m) => UnionFind m -> Int -> Int -> m ()
