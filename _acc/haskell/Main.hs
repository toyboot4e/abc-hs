...
-- | >         ok             ng
-- |
-- | In this case `bsearchHelper` returns the `(ok, ng)` = `(5, 6)` pair:
-- |
-- | > > let xs = [0..9] in do
-- | > >   print $ bsearchHelper (0, 9) (\i -> xs !! i <= 5)
-- | > (5, 6)
-- |
...
bsearchHelper :: (Int, Int) -> (Int -> Bool) -> (Int, Int)
bsearchHelper (ok, ng) isOk
  | abs (ok - ng) == 1 = (ok, ng)
  | isOk m = bsearchHelper (m, ng) isOk
  | otherwise = bsearchHelper (ok, m) isOk
  where
    m = (ok + ng) `div` 2

-- | Binary search for inclusive range (left to right only)
bsearch :: (Int, Int) -> (Int -> Bool) -> (Maybe Int, Maybe Int)
bsearch (low, top) isOk = bimap wrap wrap result
  where
    ok = low - 1
    ng = top + 1
    result = bsearchHelper (ok, ng) isOk'
