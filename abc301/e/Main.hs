...
-- TODO: clean
tsp :: Int -> UArray (Int, Int) Int -> VU.Vector (Int, Int) -> Int -> Int
tsp !nVerts !mat !bfs0 !lenLimit = runST $ do
  let !nSets = shift (1 :: Int) nVerts

  -- dp ! (set, v)
  let !goalVert = 0 :: Int

  -- TODO: maybe remove the STRef
  !result <- newSTRef (0 :: Int)

  let !undefDP = maxBound @Int

  -- FIXME: why such a type annotation is required?
  -- TODO: easier type annotation on `newArray`
  !dp <- newArray ((0, 0), (pred nSets, pred nVerts)) undefDP :: ST s (STUArray s (Int, Int) Int)
  writeArray dp (0, 0) 0

  let !_ = dbg ("DP", nSets, nVerts)

  -- start -> other vertex
  VU.forM_ bfs0 $ \(!v, !l) -> do
    let !s = shift (1 :: Int) v
    writeArray dp (s, v) l
  ...
  repM_ 1 (pred nSets) $ \s -> do
    repM_ 0 (pred nVerts) $ \v1 -> do
      ...
      when (len0 /= undefDP) $ do
        -- push
        forMS (rangeMS 0 (pred nVerts)) \v2 -> do
          when (not $ testBit s v2) $ do
            let s' = setBit s v2

            -- REMARK: some vertices are UNREACHABLE
            when (mat ! (v1, v2) /= undef) $ do
              let !len' = len0 + mat ! (v1, v2)
              modifyArray dp (min len') (s', v2)

              -- update the maximum score
              when (v2 == goalVert && len' <= lenLimit) $ do
                let !count = pred $ popCount s'
                let !_ = dbg (len0, (s, v1), "->", len', (s', v2), count)
                modifySTRef result (max count)

  readSTRef result

-- TODO: refactor the super dirty code.
main :: IO ()
main = do
  [h, w, nMove] <- ints
  !grid <- do
    !input <- replicateM h getLine
    return $ listArray @UArray ((0, 0), (pred h, pred w)) $ concat input

  -- [[(pos, "x")]]
  let !vs = groupBy (\a b -> snd a == snd b) . sortBy (comparing snd) $ filter ((`notElem` ".#") . snd) $ assocs grid

  let !start = fst . head . fromJust $ find ((== 'S') . snd . head) vs
  let !goal = fst . head . fromJust $ find ((== 'G') . snd . head) vs
  let !points = maybe [] (map fst) $ find ((== 'o') . snd . head) vs
  let !_ = dbg ("points", points, vs)

  let !bfs0 = bfsGrid grid start

  -- can reach?
  let !d0 = bfs0 ! goal
  when (d0 == -1 || d0 > nMove) $ do
    let !_ = dbg ("d0", d0)
    putStrLn "-1"
    exitSuccess

  -- all BFS, exclusing the start
  let !bfsGoal = bfsGrid grid goal
  let !bfsAll = M.fromList $ (goal, bfsGoal) : (map (\p -> (p, bfsGrid grid p))) points

  -- matrix graph, excluding the start
  let !verts = goal : points
  let !nVerts = length verts

  let !mat = listArray @UArray ((0, 0), (pred nVerts, pred nVerts)) $ concatMap f verts
        where
          f !yx1 = map (bfsAll M.! yx1 !) verts
