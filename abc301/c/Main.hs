...
main = do
  ...
  let xs = accumArray @UArray (+) (0 :: Int) (ord '@', ord 'z') $ map (\c -> (ord c, 1)) s
  let xt = accumArray @UArray (+) (0 :: Int) (ord '@', ord 'z') $ map (\c -> (ord c, 1)) t

  let !srest = VU.fromList $ map (\i -> max 0 (xs ! i - xt ! i)) [ord 'a' .. ord 'z']
  let !sat = xs ! ord '@'
  let !sAnyError = any (\i -> srest VU.! (i - ord 'a') > 0) $ filter (`notElem` (map ord "atcoder")) [ord 'a' .. ord 'z']

  let !trest = VU.fromList $ map (\i -> max 0 (xt ! i - xs ! i)) [ord 'a' .. ord 'z']
  let !tat = xt ! ord '@'
  let !tAnyError = any (\i -> trest VU.! (i - ord 'a') > 0) $ filter (`notElem` (map ord "atcoder")) [ord 'a' .. ord 'z']
  ...
  putStrLn $
    if not (sAnyError || tAnyError) && VU.sum srest <= tat && VU.sum trest <= sat
      then "Yes"
      else "No"
