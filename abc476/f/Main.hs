...
solve = do
  ...
  let rot :: (Int, Int) -> (Int, Int)
      rot (!x, !y) =
        let (!x', !y') = rot45 (x, y)
         in (x' + n, y' + n)
  let sAt x y = xs G.! x * ys G.! y `mod` m

  let sumByX' = U.accumulate (+) (U.replicate (3 * n) (0 :: Int)) $ U.generate (n * n) $ \i ->
        let (!y, !x) = i `divMod` n
            (!x', !_') = rot (x, y)
         in (x', sAt x y)

  let sumByY' = U.accumulate (+) (U.replicate (3 * n) (0 :: Int)) $ U.generate (n * n) $ \i ->
        let (!y, !x) = i `divMod` n
            (!_', !y') = rot (x, y)
         in (y', sAt x y)

  let contByX' = U.generate (n * 3) $ \i0 ->
        U.sum $
          U.imap (\i s -> s * abs (i - i0)) sumByX'

  let contByY' = U.generate (n * 3) $ \i0 ->
        U.sum $
          U.imap (\i s -> s * abs (i - i0)) sumByY'
  ...
  printBSB $ U.foldl1' xor res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc476/tasks/abc476_f
main :: IO ()
main = runIO solve
