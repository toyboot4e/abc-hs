...
solve = do
  (!_, !_, !k) <- ints3P
  -- n1: the number of 1-dollar bills, nK: the number of k-dollar bills
  (!n1, !nK) <- ints2P

  -- Who said we can sort them? But we can.
  --
  -- by1K: 1-dollar or K-dollar bills
  !by1K <- U.modify VAI.sort <$> intsP
  -- byK: K-dollary bills only
  !byK <- U.modify VAI.sort <$> intsP

  let csum1 = csum1D by1K

  -- Why not just search all?
  let result = U.maximum . dbgId $ U.izipWith f reqK' remK'
        where
          reqK' = U.scanl' (+) (0 :: Int) $ U.map (\x -> (x + (k - 1)) `div` k) byK
          remK' = U.scanl' (+) (0 :: Int) $ U.map (\x -> let r = x `mod` k in if r == 0 then 0 else k - r) byK
          f lenK reqK remK
            | nK < reqK = minBound :: Int
            | otherwise =
              let len1 = greedy (n1 + remK + (nK - reqK) * k)
                  !_ = dbg (lenK, len1)
               in lenK + len1
            where
              greedy rest = maxRight 0 (G.length by1K) $ \i ->
                i == 0
                  || csum1 +! (0, i - 1) <= rest

  printBSB $ result

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc476/tasks/abc476_d
main :: IO ()
main = runIO solve
