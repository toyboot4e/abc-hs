...
solve = do
  (!n, !m) <- ints2P
  ps <- U.map pred <$> intsP
  !lrs <- U.replicateM m ((,) <$> int1P <*> intP)

  segMax <- Seg.build $ U.map Max ps
  segMin <- Seg.build $ U.map Min ps
  valueToPos <- U.unsafeThaw $ U.accumulate max (U.replicate n (-1 :: Int)) $ U.imap (flip (,)) ps
  U.forM_ lrs $ \(!l, !r) -> do
    -- !_ <- dbgId <$> Seg.unsafeFreeze segMin
    Min minV <- Seg.prod segMin l r
    minI <- GM.read valueToPos minV

    Max maxV <- Seg.prod segMax l r
    maxI <- GM.read valueToPos maxV
    -- let !_ = dbg ("swap", (l, r), "->", (minI, minV), (maxI, maxV))

    GM.write valueToPos minV maxI
    GM.write valueToPos maxV minI
    Seg.write segMin minI $ Min maxV
    Seg.write segMin maxI $ Min minV
    Seg.write segMax minI $ Max maxV
    Seg.write segMax maxI $ Max minV

  vs <- Seg.unsafeFreeze segMin
  -- let !_ = dbg vs
  printRow $ U.map (\(Min x) -> x + 1) vs

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc476/tasks/abc476_e
main :: IO ()
main = runIO solve
