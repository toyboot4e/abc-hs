...
solve = do
  (!n, !d) <- ints2'
  !tls <- U.replicateM n ints2'

  let res = U.map f (U.generate d (+ 1))
        where
          f d = U.maximum $ U.map (\(!t, !l) -> t * (l + d)) tls

  printBSB $ unlinesBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc388/tasks/abc388_b
main :: IO ()
main = runIO solve
