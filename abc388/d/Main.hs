...
solve = do
  ...
  seg <- buildLSTree @(Sum Int) @(Sum Int) $ U.map Sum xs
  for_ [0 .. n - 2] $ \i -> do
    Sum cur <- readLSTree seg i
    when (cur > 0) $ do
      let rest = n - 1 - i
      let len = min rest cur
      when (len > 0) $ do
        sactAtLSTree seg i (Sum (- len))
        sactLSTree seg (i + 1) (i + len) $ Sum 1

  res <- U.mapM ((getSum <$>) . readLSTree seg) (U.generate n id)
  printVec res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc388/tasks/abc388_d
main :: IO ()
main = runIO solve
