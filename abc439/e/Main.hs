...
solve = do
  ...
  -- !abs <- U.modify (VAI.sortBy (comparing fst)) <$> U.replicateM n ints2P
  !abs <- U.modify (VAI.sortBy (comparing (second Down))) <$> U.replicateM n ints2P

  let (!_, !bs) = U.unzip abs
  let !dict = U.uniq $ U.modify VAI.sort bs
 
  -- maxLens <- Seg.build $ U.replicate (G.length dict) (Max (0 :: Int))
  -- for_ (G.groupBy ((==) `on` fst) abs) $ \sub -> do
  --   let bis = U.map (B.lowerBound dict . snd) sub
  --   ns <- U.mapM (fmap (Max (0 :: Int) <>) . Seg.prod maxLens 0) bis
  --   U.forM_ (U.zip bis ns) $ \(!bi, !n) -> do
  --     let !_ = dbg (bi, n + Max 1)
  --     Seg.modify maxLens (<> (n + Max 1)) bi

  maxLens <- Seg.build $ U.replicate (G.length dict) (Max (0 :: Int))
  U.forM_ abs $ \(!a, !b) -> do
    let bi = B.lowerBound dict b
    len <- (Max 0 <>) <$> Seg.prod maxLens 0 bi
    Seg.modify maxLens (<> (len +  Max 1)) bi

  printBSB . getMax =<< Seg.allProd maxLens

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc439/tasks/abc439_e
main :: IO ()
main = runIO solve
