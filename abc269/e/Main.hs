...
askIO :: Int -> Int -> Int -> Int -> IO Int
askIO !y1 !y2 !x1 !x2 = do
  putStrLn $ unwords $ "?" : map show [y1, y2, x1, x2]
  hFlush stdout
  read @Int <$> getLine
...
main = do
  ...
  let m = foldM stepM s0m [1 .. 10]
      s0m = (1, n)
      stepM (!y1, !y2) _
        | y1 == y2 = return (y1, y2)
        | otherwise = do
          let !ym = (y1 + y2) `div` 2
          !nItems <- askIO y1 ym 1 n
          return $
            if ym - y1 == nItems
              then (y1, ym)
              else (succ ym, y2)
  ...
  let m2 = foldM stepM2 s0m2 [1 .. 10]
      s0m2 = (1, n)
      stepM2 (!x1, !x2) _
        | x1 == x2 = return (x1, x2)
        | otherwise = do
          let !xm = (x1 + x2) `div` 2
          !nItems <- askIO 1 n x1 xm
          return $
            if xm - x1 == nItems
              then (x1, xm)
              else (succ xm, x2)
