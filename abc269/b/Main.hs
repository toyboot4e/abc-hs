...
main :: IO ()
main = do
  xs <- concat <$> replicateM 10 getLine
  let i1 = fromJust $ findIndex (== '#') xs
  let i2 = fromJust $ findIndex (== '#') (reverse xs)

  let (x1, y1) = (1 + i1 `rem` 10, 1 + i1 `div` 10)
  let (x2, y2) = (10 - i2 `rem` 10, 10 - i2 `div` 10)
