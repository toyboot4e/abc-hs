...
connectEdge !uf !groupCount !isPowered !v1 !v2 = do
  !r1 <- rootMUF uf v1
  !r2 <- rootMUF uf v2
  !g1 <- VUM.read groupCount r1
  !g2 <- VUM.read groupCount r2
  ...
  if r1 == r2
    then return 0
      ...
      uniteMUF uf v1 v2
      r' <- rootMUF uf v1
      VUM.write groupCount r' (g1 + g2)

      !p1 <- VUM.read isPowered r1
      !p2 <- VUM.read isPowered r2

      when (p1 || p2) $ do
        VUM.write isPowered r' True

      -- return the number of newly powered cities
      return $
        case (p1, p2) of
          (True, False) -> g2
          (False, True) -> g1
...
main :: IO ()
main = do
  [nVerts1, nVerts2, nEdges] <- getLineIntList
  let !nVerts = nVerts1 + nVerts2
  !edges <- VU.replicateM nEdges (both pred <$> getTuple2)

  [nQueries] <- getLineIntList
  !queries <- concat <$> replicateM nQueries (map pred <$> getLineIntList)

  let !queriedEdges = IS.fromList queries
  let !initialEdges = filter (`IS.notMember` queriedEdges) [0 .. pred nEdges]

  uf <- newMUF nVerts
  groupCount <- VU.unsafeThaw $ VU.generate nVerts (\i -> if i < nVerts1 then 1 else 0)
  isPowered <- VU.unsafeThaw $ VU.generate nVerts (\i -> i >= nVerts1)

  !acc0 <-
    sum
      <$> ( forM initialEdges $ \iEdge -> do
              let (!v1, !v2) = edges VU.! iEdge
              connectEdge uf groupCount isPowered v1 v2
          )
  let !_ = dbg (acc0)

  let m = foldM stepM s0M $ reverse queries
      s0M = (acc0, [] :: [Int])
      stepM (!nAcc, !acc) !iEdge = do
        let (!v1, !v2) = edges VU.! iEdge
        !delta <- connectEdge uf groupCount isPowered v1 v2
        let !_ = dbg ((v1, v2), delta)
        let nAcc' = delta + nAcc
        -- REMARK: accumulate the `nAcc` before adding the edge (i.e., after removing the edge)
        return (nAcc', nAcc : acc)
  -- TODO: count connected vertices
