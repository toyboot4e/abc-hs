...
main = do
  ...
  let !result = fst $ VU.foldl' step s0 (vRange 1 k)
      -- vector range: [0, n + m - 1]
      s0 = (modInt 0, VU.generate (n + m) (\case 0 -> modInt 1; _ -> modInt 0))
      step (!acc, !vec) count = (acc', vec')
        where
          -- mirroring
          !contribs = VU.imap (\i x -> if | i < n -> x + fromMaybe 0 (vec VU.!? (n + (n - i))) | otherwise -> 0) vec
          !csum = VU.scanl' (+) (modInt 0) contribs
          !vec' = VU.generate (n + m) $ \i ->
            (csum VU.! i - csum VU.! (max 0 (i - m))) / modInt m
          !acc' = acc + vec' VU.! n
          -- !_ = dbg (count, vec, csum, vec', (acc, acc'))
          !_ = dbg (count, acc', vec')
