...
main = do
  ...
  let bounds_ = ((0, 0), (8, 8))
  let arr = listArray @UArray bounds_ input
  ...
  let pos =
         range bounds_ >>=
           ...
           (\context ->
              -- let !_ = traceShow (context) () in
                return [a, b])))

  let posMap [a, b] =
        let (dx, dy) = sub2 b a
            d = (dy, -dx)
         in [ [a, b, add2 a d, add2 b d],
              [a, b, sub2 a d, sub2 b d]
            ]

  let possibilities = nubSort $ concatMap (map sort . posMap) pos
  let result = filter (all (\i -> inRange bounds_ i && arr ! i)) possibilities
