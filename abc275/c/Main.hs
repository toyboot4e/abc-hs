...
main :: IO ()
main = do
  -- 9x9 grid indexed by y then x
  grid <- V.replicateM 9 (VU.fromList . map (== '#') <$> getLine)

  let xs = combinations 2 [0 .. 8]
  let ys = combinations 2 [0 .. 8]

  let sxys = [(s, x, y) | s <- [2 .. 8], x <- [1 .. (s - 1)], let y = s - x]

  -- [(Int, (Int, Int))]
  let squares = map (\(s, x, y) -> (s, concat $ squares x y)) sxys

  -- [(Int, Int)]
  let points = concatMap (\(s, pts) -> concatMap (\pt -> withOffset (9 - s)) pts) squares

  print $ points

squares :: Int -> Int -> [[(Int, Int)]]
squares x y
  | x == y = [[(x, 0), (x + y, x), (y, x + y), (0, y)]]
  | otherwise =
    [ [(x, 0), (x + y, x), (y, x + y), (0, y)],
      -- reflected
      [(y, 0), (y + x, y), (x, y + x), (0, x)]
...
withOffset :: Int -> (Int, Int) -> [(Int, Int)]
withOffset ofsRange (x, y) = [(ofsX + x, ofsY + y) | ofsX <- [0 .. ofsRange], ofsY <- [0 .. ofsRange]]

-- -- y -> xs
-- let ixs = V.map (\y -> VU.map (\x -> grid V.! y VU.! x) VU.fromList [0..8]) $ V.fromList [0..8]
-- -- x -> ys
-- let iys = V.map (\x -> VU.map (\y -> grid V.! y VU.! x) VU.fromList [0..8]) $ V.fromList [0..8]
