...
solve = do
  !gr <- getGrid' 8 8
  vec <- IxVector (zero2 8 8) <$> UM.replicate (8 * 8) True

  forM_ [0 .. 7] $ \y -> do
    forM_ [0 .. 7] $ \x -> do
      when (gr @! (y, x) == '#') $ do
        forM_ [0 .. 7] $ \y' -> do
          writeIV vec (x, y') False
        forM_ [0 .. 7] $ \x' -> do
          writeIV vec (x', y) False

  gr' <- unsafeFreezeIV vec
  printBSB $ U.length $ U.filter id $ vecIV gr'

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc377/tasks/abc377_b
main :: IO ()
main = runIO solve
