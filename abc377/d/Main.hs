...
solve = do
  (!n, !m) <- ints2'
  !lrs <- U.replicateM n ints11'

  let !lrs' = IM.fromListWith min . U.toList $ U.modify (VAI.sortBy (comparing (Down . fst))) $ U.filter inc lrs
        where
          inc (!y, !x) = inRange (0, m - 1) y && inRange (0, m - 1) x
  let !res = fst $ U.foldl' step s0 $ U.generate m (\di -> m - 1 - di)
        where
          s0 = (0, m)
          step (!acc, !rOut) l = (acc + dAcc, rOut')
            where
              !rOut' = case lrs' IM.!? l of
                Nothing -> rOut
                Just r' -> min rOut r'
              count x1 x2 = max 0 $ x2 - x1 + 1
              dAcc = count l (rOut' - 1)
              !_ = dbgId (l, rOut', dAcc)

  printBSB res

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc377/tasks/abc377_d
main :: IO ()
main = runIO solve
