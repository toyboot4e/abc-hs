...
main = do
  ...
  let f' :: Int -> Double
      f' n =
        let n' = fromIntegral n :: Double
         in b - (a / 2.0) * (g + n') ** (- 3.0 / 2.0)

  let nMax = a' `div` b'

  let nf'0 = fst $ bsearch (0, nMax) (\n -> f' (fromIntegral n) <= 0.0)
  let result = case nf'0 of
        Just x -> minimum [f 0, f x, f (x + 1), f nMax]
        Nothing -> minimum [f 0, f nMax]
