...
main = do
  [n, m] <- getLineIntList
  !swapCommands <- VU.map pred <$> getLineIntVec

  let !x0 = VU.foldl' swap0 0 swapCommands

  -- vector that represents a replacement
  !repl <- VU.unsafeThaw $ VU.generate n id

  let !m = VU.foldM step s0 $ VU.reverse swapCommands
      !s0 = ([], x0)
      step (!acc, !i) swapIndex = do
        -- undo the last swap
        let !i' = swap0 i swapIndex
        !res <- VUM.read repl i'
        let !_ = dbg ((i, i'), res, swapIndex)
        -- advance the replacement
        VUM.swap repl swapIndex (succ swapIndex)
        return (res : acc, i')

  !result <- map succ . fst <$> m
  forM_ result print
