...
solve = do
  ...
  let popK k0 = do
        let inner !acc 0 = pure acc
            inner !acc k = do
              (!x, !dn) <- readFront buf 0
              case compare dn k of
                LT -> do
                  popFront_ buf
                  inner (acc + x * dn) (k - dn)
                EQ -> do
...
-- verification-helper: PROBLEM https://atcoder.jp/contests/abc413/tasks/abc413_c
main :: IO ()
main = runIO solve
