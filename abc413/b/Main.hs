...
solve = do
  !n <- int'
  !ss <- V.replicateM n line'

  let strings = S.fromList $ concat [[s1 <> s2, s2 <> s1] | [!s1, !s2] <- combs 2 (V.toList ss)]
  printBSB $ S.size strings

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc413/tasks/abc413_b
main :: IO ()
main = runIO solve
