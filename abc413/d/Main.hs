...
-- testSeq :: V.Vector (Rational Int) -> Rational Int -> Bool
testSeq xs' =
  let (!r1, !rest) = fromJust . V.uncons $ V.zipWith (%) (U.convert xs') (U.convert (U.tail xs'))
      !_ = dbg (r1, rest)
   in G.all (== r1) rest

-- il :: (U.Unbox a) => U.Vector a -> (U.Vector a, U.Vector a)
il xs =
  let seq = U.convert . V.concat . V.toList $ G.zipWith (\x y -> G.fromListN 2 [x, y]) (U.convert xs) $ U.convert (G.reverse xs)
      n = G.length xs
   -- in (G.take n seq, G.take n (G.reverse seq))
   in G.take n seq
...
solve = do
  !n <- int'
  !xs <- intsU'
  let !xs' = dbgId $ U.modify VAI.sort xs
  -- let (!il1, !il2) = dbgId $ il xs'
  let il1 = il xs'
  let il2 = il $ G.reverse xs'
  printYn $ testSeq xs' || testSeq il1 || testSeq il2

-- verification-helper: PROBLEM https://atcoder.jp/contests/abc413/tasks/abc413_d
main :: IO ()
main = runIO $ do
  t <- int'
  replicateM_ t solve
